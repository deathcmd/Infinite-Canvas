...
def model_dump_compat(model, **kwargs):
    """Return a Pydantic model as a plain dict on v1 and v2.

    The project intentionally keeps ``pydantic`` unpinned for easy local
    installation.  Pydantic v2 deprecates ``BaseModel.dict()``; centralising
    the compatibility call avoids deprecation noise while retaining support
    for older desktop environments that still ship v1.
    """
...
class ConnectionManager:
    ...
    async def connect(self, websocket: WebSocket, client_id: str = None):
        ...
        # A browser reconnect can arrive before the old socket's disconnect
        # callback.  Evict the stale mapping first; otherwise the old socket
        # remains in ``active_connections`` forever and every broadcast keeps
        # retrying a dead transport.  ``_forget_connection`` is idempotent and
        # deliberately preserves a newer mapping when called later.
        if client_id:
            previous = self.user_connections.get(client_id)
            if previous is not None and previous is not websocket:
                self._forget_connection(previous, client_id)
        if websocket in self.active_connections:
            self._forget_connection(websocket, client_id)
    ...
    def _forget_connection(self, websocket, client_id: str = None):
        """Remove a websocket from every connection index.

        Broadcast failures used to remove only ``active_connections``.  The
        reverse indexes then kept a dead socket counted as online and
        ``send_personal_message`` retried it forever.  A client ID may have
        already been rebound to a newer socket, so only delete that mapping
        when it still points at the socket being evicted.
        """
        try:
            self.active_connections.remove(websocket)
        except ValueError:
            # Disconnect and broadcast cleanup can race at an await boundary;
            # cleanup is intentionally idempotent.
            pass
        mapped_client = self.connection_clients.pop(websocket, None)
        for candidate in (mapped_client, client_id):
            if candidate and self.user_connections.get(candidate) is websocket:
                self.user_connections.pop(candidate, None)
    ...
    async def broadcast_new_image(self, image_data: dict):
        # WebSocket messages bypass HTTP middleware; apply the same public
        # boundary policy before broadcasting task/history payloads.
        safe_image = sanitize_public_runtime_value(image_data)
        data = json.dumps({"type": "new_image", "data": safe_image})
        for connection in self.active_connections[:]:
            try:
                await connection.send_text(data)
            except Exception as e:
    ...
    async def send_personal_message(self, message: dict, client_id: str):
        ...
        if ws:
            try:
                ...
                self._forget_connection(ws, client_id)

manager = ConnectionManager()
GLOBAL_LOOP = None
APP_VERSION = "2026.06.03"
# 发行版保持本地源码模式：不在运行时探测或拉取任何远程仓库。
# 这些空值保留给旧版升级函数的兼容路径，避免历史调用产生 NameError。
...
def _schedule_background(coro):
    """Schedule a broadcast coroutine without leaking it across loop lifetimes.

    A number of persistence helpers are synchronous because they are also used
    by migration scripts and unit tests.  Previously they unconditionally
    called ``run_coroutine_threadsafe`` on ``GLOBAL_LOOP``.  Once a test client
    (or a development server reload) closed that loop, the stale reference
    raised ``RuntimeError: Event loop is closed`` and the freshly-created
    coroutine was left un-awaited.  Prefer the currently running loop, fall
    back to the live application loop from worker threads, and close the
    coroutine when no usable loop exists.
    """
    if coro is None:
        return None
    scheduled = False
    try:
        ...
        if running is not None and not running.is_closed():
            # ``create_task`` is safe and avoids crossing threads when the
            # synchronous helper is called from an async request/test.
        ...
        # Loop shutdown can race the checks above.  Treat notifications as
        # best-effort; persistence has already completed at this point.
        print(f"后台通知调度跳过: {exc}")
    finally:
        if not scheduled:
...
async def startup_event():
    ...
    # Cache-busting is applied to the HTTP response by ``static_html_response``
    # and must never rewrite the checked-in HTML files on disk.  The previous
    # startup rewrite replaced semantic asset tags (for example ``libtv11``)
    # with ``VERSION.mtime`` values every time the server restarted, which
    # polluted the working tree and made source-level cache contracts flaky.
    # Keep the source files stable; browsers still receive a fresh query key.
...
async def app_lifespan(_app: FastAPI):
    """Run one-time migrations through FastAPI's supported lifespan API.

    ``@app.on_event('startup')`` is deprecated by current FastAPI releases and
    emits a warning on every import.  Keep the migration body in the named
    ``startup_event`` helper for backwards-compatible tests/tools, but attach
    it to the application's router lifespan context so startup and shutdown
    are managed by a single supported hook.
    """
    global GLOBAL_LOOP
    ...
    try:
        ...
        # Do not leave a closed event loop in the module-level notification
        # bridge after a reload/test client shutdown.  The identity check keeps
        # a newer server loop intact if an overlapping worker replaced it.
        if GLOBAL_LOOP is loop:
            GLOBAL_LOOP = None


# ``FastAPI`` was instantiated above so middleware can be registered before
# route declarations.  Replacing Starlette's default lifespan context here is
# equivalent to passing ``lifespan=app_lifespan`` to the constructor while
# avoiding a large, error-prone reorder of the module's setup code.
...
def load_storage_settings():
    raw = {}
    # Settings are shared by the asset manager and storage panel.  Serialize
    # reads with writes so a concurrent PATCH cannot expose a half-written
    # JSON document (or apply a stale directory snapshot).
    with STORAGE_SETTINGS_LOCK:
        ...
        if not isinstance(raw, dict):
            # Valid JSON with the wrong top-level shape is still a corrupt
            # settings document; treat it as empty rather than calling
            # ``.get`` on a list/string and returning a 500 from every GET.
            raw = {}
    dirs = {}
...
def save_storage_settings(payload):
    with STORAGE_SETTINGS_LOCK:
        ...
        # Use the same crash-safe replacement primitive as canvas/project
        # documents.  The helper is defined later in this module and is
        # available by the time the HTTP endpoint can invoke this function.
        _atomic_json_write(STORAGE_SETTINGS_FILE, dirs)
...
# Batch mutation endpoints use dedicated locks so a multi-item request writes
# its index atomically without serialising unrelated generation work.  The
# existing single-item routes remain backwards compatible; these locks also
# make concurrent bulk requests deterministic.
ASSET_LIBRARY_LOCK = RLock()
PROMPT_LIBRARY_LOCK = RLock()
ASSET_CLASSIFICATION_PROMPT_LOCK = RLock()
WORKFLOW_FILE_LOCK = Lock()
STORAGE_DELETE_LOCK = Lock()
LOCAL_UPLOAD_DELETE_LOCK = Lock()
NEXT_TASK_ID = 1
UPDATE_LOCK = Lock()
...
# The public build intentionally does not expose the Grok/Sub2API route.  Keep
# the check centralized so a provider loaded from an old local config, an
# imported workflow, or a freshly fetched model list cannot re-introduce that
# route through a different UI entry point.  Matching is deliberately narrow:
# arbitrary custom OpenAI-compatible providers remain supported unless their
# identity/model is explicitly labelled as a disabled vendor or paid tier.
OPEN_SOURCE_BLOCKED_PROVIDER_MARKERS = ("grok", "sub2api")
OPEN_SOURCE_BLOCKED_MODEL_MARKERS = ("grok",)
# Do not blanket-match words such as ``pro`` or ``plus``: those are common
# model names and are not, by themselves, a billing control.  These markers
# are matched as standalone ASCII tokens (``seedance25-vip`` matches) plus a
# small set of unambiguous Chinese billing labels.  The policy hides/rejects
# these labels but deliberately leaves the user's provider file untouched.
OPEN_SOURCE_BILLING_MODEL_MARKERS = frozenset({
    "vip",
    "premium",
    "paid",
    "subscription",
...
OPEN_SOURCE_BILLING_TEXT_RE = re.compile(r"(?:会员|付费|充值|订阅)", re.I)
OPEN_SOURCE_BILLING_TOKEN_RE = re.compile(
    r"(?<![a-z0-9])(?:vip|premium|paid|subscription|membership)(?![a-z0-9])",
    re.I,
)
# Keep this response vendor-neutral and stable so disabled legacy billing
# commands never echo upstream account/credit details to a client.
OPEN_SOURCE_NO_BILLING_MESSAGE = "本地开源版本不提供积分/钱包功能"
JIMENG_CREDIT_COMMAND_RE = re.compile(
    r"(?:credit|balance|quota|point|coin|wallet|积分|余额|额度|钱包)",
    re.I,
)

# Public provider payloads can contain data imported from old RunningHub
# configurations.  Treat that data as untrusted: credentials may be nested in
# ``raw``/``workflowJson`` and a stale app/workflow title can otherwise make a
# disabled vendor visible again.  These helpers are deliberately kept next to
# the central open-source provider policy so every public serialization path
# uses the same rules.
OPEN_SOURCE_BLOCKED_TEXT_RE = re.compile(
    r"(?:grok|sub2api|会员|付费|充值|订阅|"
    # A dot is excluded from the left boundary here so a legitimate host
    # suffix such as ``https://apistudio.vip`` is not stripped by the generic
    # public JSON sanitizer.  Model/provider labels still use the stricter
    # token matcher above, where dots remain valid separators when appropriate.
    r"(?<![a-z0-9.])(?:vip|premium|paid|subscription|membership)(?![a-z0-9]))",
    re.I,
)
_PUBLIC_SECRET_KEY_MARKERS = (
    "apikey",
    "token",
    "password",
    "passwd",
...
_PUBLIC_METADATA_KEYS = {
    # These are server-generated, masked compatibility hints.  Remove any
    # copies supplied by imported provider data before adding fresh values.
    "hastoken",
    "tokenpresent",
    "haskey",
    "keypreview",
    ...
    "walletkeypreview",
    "walletkeyenv",
    "hasvolcengineaccesskey",
    "volcengineaccesskeypreview",
    "volcengineaccesskeyenv",
    "hasvolcenginesecretkey",
    "volcenginesecretkeypreview",
    "volcenginesecretkeyenv",
}
_PUBLIC_BOOLEAN_METADATA_KEYS = {
    "hastoken",
    "tokenpresent",
    "haskey",
    "haswalletkey",
...
_PUBLIC_PREVIEW_METADATA_KEYS = {
    "keypreview",
    "walletkeypreview",
    "volcengineaccesskeypreview",
    "volcenginesecretkeypreview",
}
_PUBLIC_ENV_METADATA_KEYS = {
    "keyenv",
    "walletkeyenv",
    "volcengineaccesskeyenv",
    "volcenginesecretkeyenv",
}
_PUBLIC_MASKED_PREVIEW_RE = re.compile(r"^••••••••.{0,4}$", re.S)
_PUBLIC_ENV_NAME_RE = re.compile(r"^[A-Z][A-Z0-9_]{1,127}$")
_PUBLIC_ENV_ALLOWED_PREFIXES = (
    "API_PROVIDER_",
    "COMFLY_",
    "MODELSCOPE_",
    "RUNNINGHUB_",
...
_PUBLIC_ENTRY_ID_FIELDS = {"id", "appid", "workflowid"}
_PUBLIC_DROP = object()

# Upstream workflow exports sometimes serialize a complete JSON payload into
# a string (for example ``raw.data.prompt``), and media URLs may carry auth
# material in their query string.  Key-based recursion cannot see inside those
# scalar strings, so redact the common inline forms before a value crosses a
# public API boundary.  The replacement is intentionally deterministic and
# contains no part of the original secret.
_PUBLIC_INLINE_SECRET_NAMES = (
    "rh-comfy-auth",
    "rh-identify",
    "api-key",
    "api_key",
...
_PUBLIC_INLINE_SECRET_NAME_RE = "|".join(re.escape(item) for item in _PUBLIC_INLINE_SECRET_NAMES)
_PUBLIC_QUERY_SECRET_RE = re.compile(
    rf"(?i)([?&](?:{_PUBLIC_INLINE_SECRET_NAME_RE}|api%5fkey|x%2dapi%2dkey)=)(?!\[redacted\])[^&#\[\]\s\"'<>}},]+"
)
_PUBLIC_ASSIGNMENT_SECRET_RE = re.compile(
    # Start at the key name rather than trying to enumerate every escaping
    # style used by JSON-in-a-string (``\"apiKey\":\"...`` included).
    rf"(?i)((?<![A-Za-z0-9])(?:{_PUBLIC_INLINE_SECRET_NAME_RE})[\\\"'\s]{{0,8}}[:=][\\\"'\s]{{0,3}})(?!\[redacted\])[^,}}&\[\]\s\\\"'<>]+"
)
_PUBLIC_BEARER_RE = re.compile(r"(?i)(\bbearer\s+)[A-Za-z0-9._~+/=-]{8,}")
_PUBLIC_USERINFO_SECRET_RE = re.compile(r"(?i)(https?://[^/\s:@]+:)[^@\s/]+(@)")
_PUBLIC_AUTH_HEADER_PREFIX_RE = re.compile(
    r"(?i)\b(?:proxy-)?authorization(?:\\?[\"']\s*)?\s*[:=]\s*\\?[\"']?"
)
_PUBLIC_COOKIE_HEADER_PREFIX_RE = re.compile(
    r"(?i)\b(?:set-)?cookie(?:\\?[\"']\s*)?\s*[:=]\s*\\?[\"']?"
)
_PUBLIC_HTTP_URL_RE = re.compile(r"https?://[^\s\"'<>]+", re.I)
_PUBLIC_SIGNED_QUERY_NAME_RE = re.compile(
    r"(?i)(?:signature|sig|credential|accesskey|security.?token|session.?token|"
    r"auth(?:orization|entication)?|api.?key|secret|private.?key|x.?amz|x.?oss|"
    r"googleaccessid|key.?pair.?id|policy|jwt|cookie|bearer|accessid)"
)


def _public_key_compact(key: Any) -> str:
    """Normalize a JSON field name for secret/marker comparisons."""
    return re.sub(r"[^a-z0-9]", "", str(key or "").strip().lower())


def _sanitize_public_metadata(key: Any, value: Any):
    """Keep only the small, server-generated provider status hints.

    The final API middleware sees arbitrary upstream JSON, so simply allowing
    a key such as ``key_preview`` would let an imported payload smuggle a full
    credential under that compatibility name.  Presence flags must be real
    booleans; previews must have the exact masked shape emitted by
    :func:`mask_secret`; environment hints are names only.
    """
    compact = _public_key_compact(key)
    if compact in _PUBLIC_BOOLEAN_METADATA_KEYS:
        return value if isinstance(value, bool) else _PUBLIC_DROP
    if compact in _PUBLIC_PREVIEW_METADATA_KEYS:
        if not isinstance(value, str):
            return _PUBLIC_DROP
        text = value.strip()
        if not text:
            return ""
        return text if _PUBLIC_MASKED_PREVIEW_RE.fullmatch(text) or text == "[redacted]" else _PUBLIC_DROP
    if compact in _PUBLIC_ENV_METADATA_KEYS:
        if not isinstance(value, str):
            return _PUBLIC_DROP
        text = value.strip()
        if not text:
        ...
        return (
            text
            if _PUBLIC_ENV_NAME_RE.fullmatch(text)
            and any(text.startswith(prefix) for prefix in _PUBLIC_ENV_ALLOWED_PREFIXES)
            else _PUBLIC_DROP
        )
    return _PUBLIC_DROP


def _public_entry_has_secret_semantics(value: Any) -> bool:
    """Detect node cards that hide a secret field behind fieldName/value.

    RunningHub commonly represents inputs as ``{fieldName, fieldValue}``.
    Key-based recursion cannot classify the value in that shape because the
    literal key is only ``fieldValue``; drop the complete public card when its
    semantic field name is credential-shaped.
    """
    if not isinstance(value, dict):
        return False
    for name in ("fieldName", "field_name", "inputName", "input_name", "parameter", "param", "key", "name"):
        candidate = value.get(name)
        if isinstance(candidate, str) and candidate.strip() and _is_public_semantic_secret_name(candidate):
            return True
    return False


def _is_signed_query_name(name: Any) -> bool:
    compact = _public_key_compact(name)
    return bool(compact) and bool(_PUBLIC_SIGNED_QUERY_NAME_RE.search(compact))


def _redact_inline_header_values(text: str, pattern: re.Pattern) -> str:
    """Replace a complete Authorization/Cookie header value in free text.

    A simple ``Bearer`` regex leaves Basic/Digest/OAuth tails behind, and a
    JSON-escaped header may contain quotes inside the value.  Match only the
    header prefix, then consume either the quoted value or the rest of the
    line/object so every authentication scheme is covered while preserving
    the surrounding JSON quote when present.
    """
    ...
    for match in matches:
        ...
        if remainder.startswith("[redacted]"):
            output.append(prefix)
            cursor = start
            continue
        if prefix.endswith(('"', "'")):
            quote = prefix[-1]
            # The prefix's final quote is an opening quote even when escaped;
            # locate its matching closing quote without treating \\" as one.
            end = len(text)
            index = start
            if prefix.endswith("\\" + quote):
                # In a scalar containing escaped JSON (\\"Authorization\\":
                # \\"Basic ...\\"), the closing quote is likewise escaped in
                # the outer text.  Preserve that token as the suffix.
                token = "\\" + quote
...
def _redact_public_url(value: str) -> str:
    """Redact credential/signature query parameters from one or more URLs."""
    def redact_component(component):
        if not component:
            return component, False
        pieces = re.split(r"([&;])", component)
        changed = False
        safe_pieces = []
        for piece in pieces:
            if piece in {"&", ";"}:
            ...
            if not piece:
                safe_pieces.append(piece)
                continue
            key_raw, separator, value_raw = piece.partition("=")
            key_decoded = urllib.parse.unquote_plus(key_raw)
            value_decoded = urllib.parse.unquote_plus(value_raw) if separator else ""
            # Some upstream serializers percent-encode the equals sign itself
            # (``Signature%3D...``), so recover that shape before classifying.
            if not separator and "=" in key_decoded:
                key_decoded, embedded = key_decoded.split("=", 1)
                value_decoded = embedded
                separator = "="
            if _is_signed_query_name(key_decoded):
                encoded_key = urllib.parse.quote(key_decoded, safe="!$'()*+,-._~")
                safe_pieces.append(f"{encoded_key}=[redacted]")
                changed = True
                continue
            # A few signed URL producers put a nested ``Signature=...`` in a
            # generic parameter value.  Remove that entire value rather than
            # allowing the credential to survive under an innocent key.
            if separator and _PUBLIC_SIGNED_QUERY_NAME_RE.search(value_decoded) and "=" in value_decoded:
                safe_pieces.append(f"{key_raw}=[redacted]")
                changed = True
                continue
            safe_pieces.append(piece)
        return "".join(safe_pieces), changed
    ...
    def replace(match):
        ...
        while candidate and candidate[-1] in ".,;:!?)}":
            # ``]`` is part of the deterministic ``[redacted]`` sentinel,
            # not prose punctuation.  Stopping here keeps repeated sanitizer
            # passes idempotent (no ``[redacted]]`` growth).
        ...
        try:
            ...
            if parsed.scheme.lower() not in {"http", "https"} or not parsed.netloc:
                return original
            safe_path, path_changed = redact_component(parsed.path)
            safe_query, query_changed = redact_component(parsed.query)
            safe_fragment, fragment_changed = redact_component(parsed.fragment)
            if not path_changed and not query_changed and not fragment_changed:
                return original
            return urllib.parse.urlunsplit((parsed.scheme, parsed.netloc, safe_path, safe_query, safe_fragment)) + trailing
        except Exception:
            return original

    return _PUBLIC_HTTP_URL_RE.sub(replace, str(value or ""))


def _is_public_secret_key(key: Any) -> bool:
    """Return whether a nested provider field may carry a credential.

    The check intentionally accepts camelCase, snake_case and hyphenated
    spellings (``apiKey``, ``api_key``, ``client-secret`` ...).  Public
    ``has_key``/preview/env hints are stripped from imported data too and are
    re-created from the server-side masked values below.
    """
    compact = _public_key_compact(key)
    if not compact:
        return False
    # These are compatibility/status hints, not the credential values
    # themselves.  ``sanitize_public_provider_data`` removes imported copies
    # before re-adding server-generated hints; runtime response sanitization
    # must preserve the latter so the settings UI can show key presence.
    if compact in _PUBLIC_METADATA_KEYS:
        return False
    if any(marker in compact for marker in _PUBLIC_SECRET_KEY_MARKERS):
        return True
    if compact in {"auth", "cookie", "session", "jwt", "bearer", "sig", "policy", "accessid", "googleaccessid"}:
        return True
    if compact.endswith(("auth", "cookie", "session", "jwt")):
        return True
    if compact.endswith("sig") and len(compact) > 3:
        return True
    if "accessid" in compact or "googleaccessid" in compact:
        return True
    if compact.endswith("key") and any(
        prefix in compact for prefix in ("api", "access", "secret", "private", "client", "custom", "google", "security", "session", "comfy", "rh")
    ):
        return True
    return False


_PUBLIC_SEMANTIC_SECRET_NAMES = {
    "key",
    "apikey",
    "accesskey",
    "accesskeyid",
...
def _is_public_semantic_secret_name(value: Any) -> bool:
    """Classify a RunningHub semantic field name as credential-shaped.

    This is intentionally stricter than the dictionary-key heuristic above:
    names such as ``privateKey``/``comfyAuth``/``x-oss-signature`` must be
    blocked even though their enclosing JSON key is merely ``fieldValue``.
    """
    compact = _public_key_compact(value)
    if not compact:
        return False
    if compact in _PUBLIC_SEMANTIC_SECRET_NAMES:
        return True
    if any(term in compact for term in (
        ...
        "authorization", "authentication", "credential", "password",
        "passwd", "cookie", "session", "jwt", "signature",
    )):
        return True
    # Catch compact/camel-case custom names such as ``customKey`` or
    # ``comfyAuth`` without treating arbitrary dictionary keys as secrets.
    return compact.endswith(("key", "token", "secret", "auth")) and len(compact) > 3


def _redact_public_string(value: str) -> str:
    """Remove credentials embedded in URLs, headers, or JSON strings."""
    text = str(value or "")
    if not text:
        return text
    text = _redact_public_url(text)
    text = _redact_inline_header_values(text, _PUBLIC_AUTH_HEADER_PREFIX_RE)
    text = _redact_inline_header_values(text, _PUBLIC_COOKIE_HEADER_PREFIX_RE)
    text = _PUBLIC_USERINFO_SECRET_RE.sub(r"\1[redacted]\2", text)
    text = _PUBLIC_BEARER_RE.sub(r"\1[redacted]", text)
    text = _PUBLIC_ASSIGNMENT_SECRET_RE.sub(r"\1[redacted]", text)
    text = _PUBLIC_QUERY_SECRET_RE.sub(r"\1[redacted]", text)
    return text


_PUBLIC_OPAQUE_ID_RE = re.compile(r"^[A-Za-z0-9][A-Za-z0-9._:/-]{0,255}$")
_PUBLIC_OPAQUE_URL_RE = re.compile(r"^[A-Za-z][A-Za-z0-9+.-]{1,31}://", re.I)
_PUBLIC_OPAQUE_SECRET_MARKER_RE = re.compile(
    r"(?:^|[-_:/\.])(?:api[-_]?key|access[-_]?key|private[-_]?key|secret|credential|password|auth|bearer|token|jwt|signature|cookie|session)(?:$|[-_:/\.])",
    re.I,
)


def sanitize_public_opaque_id(value: Any) -> str:
    """Return an opaque task/file identifier only when it has a safe shape."""
    text = str(value or "").strip()
    if (
        not text
        or not _PUBLIC_OPAQUE_ID_RE.fullmatch(text)
        or ".." in text
        or _PUBLIC_OPAQUE_URL_RE.match(text)
        or _PUBLIC_OPAQUE_SECRET_MARKER_RE.search(text)
        or _redact_public_string(text) != text
        or _is_public_secret_key(text)
    ):
        return ""
    return text


def _is_public_blocked_text_field(key: Any) -> bool:
    compact = _public_key_compact(key)
    if not compact:
        return False
    if compact in _PUBLIC_BLOCKED_TEXT_FIELDS:
        return True
    # Cover common nested spellings such as provider_name/model_id/title_text
    # without treating arbitrary prompt/content fields as vendor identities.
    return any(compact.startswith(prefix) for prefix in ("title", "name", "note", "model", "provider"))


def _contains_open_source_blocked_text(value: Any) -> bool:
    """Search a marker field's complete value, including nested maps/lists."""
    if isinstance(value, str):
        return bool(OPEN_SOURCE_BLOCKED_TEXT_RE.search(value))
    if isinstance(value, dict):
        return any(
            _contains_open_source_blocked_text(key)
            or _contains_open_source_blocked_text(item)
            for key, item in value.items()
        )
    if isinstance(value, (list, tuple)):
        return any(_contains_open_source_blocked_text(item) for item in value)
    return False


def _public_entry_contains_blocked_marker(value: Any) -> bool:
    """Return whether an app/workflow entry advertises a disabled route."""
    if isinstance(value, dict):
        # RunningHub field cards commonly split the semantic key and its
        # display value into ``fieldName``/``fieldValue`` (or ``key``/``value``)
        # instead of using a literal ``model`` property.  Treat those pairs as
        # marker-bearing metadata too, while leaving arbitrary prompt values
        # untouched.
        semantic_key = ""
        for name in ("fieldName", "field_name", "key", "parameter", "param"):
            candidate = value.get(name)
            if isinstance(candidate, str) and candidate.strip():
                semantic_key = candidate.strip()
                break
        semantic_value = next(
            (
                value.get(name)
                for name in ("fieldValue", "field_value", "value", "default")
                if name in value
            ),
            None,
        )
        if semantic_key and _is_public_blocked_text_field(semantic_key):
            if _contains_open_source_blocked_text(semantic_value):
                return True
        for key, item in value.items():
            compact = _public_key_compact(key)
            if _is_public_blocked_text_field(key):
                if _contains_open_source_blocked_text(item):
                    return True
            elif compact in _PUBLIC_ENTRY_ID_FIELDS and isinstance(item, str):
                if OPEN_SOURCE_BLOCKED_TEXT_RE.search(item):
                    return True
            if _public_entry_contains_blocked_marker(item):
                return True
    elif isinstance(value, (list, tuple)):
        return any(_public_entry_contains_blocked_marker(item) for item in value)
    return False


def _contains_blocked_text_anywhere(value: Any) -> bool:
    """Find a disabled-vendor marker in any scalar/key of a nested value."""
    if isinstance(value, str):
        return bool(OPEN_SOURCE_BLOCKED_TEXT_RE.search(value))
    if isinstance(value, dict):
        return any(
            _contains_blocked_text_anywhere(key)
            or _contains_blocked_text_anywhere(item)
            for key, item in value.items()
        )
    if isinstance(value, list):
        return any(_contains_blocked_text_anywhere(item) for item in value)
    return False
...
def _sanitize_public_value(
    value: Any,
    *,
    drop_blocked_entries: bool = False,
    drop_blocked_text: bool = False,
    drop_public_metadata: bool = False,
):
    """Recursively redact secrets and disabled-vendor entry metadata.

    ``drop_blocked_entries`` is enabled only for ``rh_apps`` and
    ``rh_workflows`` lists.  A blocked title/model then removes the whole app
    or workflow card, preventing a marker from surviving in another nested
    field.  Other dictionaries retain their shape while offending fields are
    omitted, which keeps imported workflow payloads usable.
    """
    if isinstance(value, dict):
        if _public_entry_has_secret_semantics(value):
            return _PUBLIC_DROP
        if drop_blocked_entries and (
            _public_entry_contains_blocked_marker(value)
            or _contains_blocked_text_anywhere(value)
        ):
            return _PUBLIC_DROP
        result = {}
        for key, item in value.items():
            compact_key = _public_key_compact(key)
            if compact_key in _PUBLIC_METADATA_KEYS:
                if drop_public_metadata:
                    continue
                safe_metadata = _sanitize_public_metadata(key, item)
                if safe_metadata is not _PUBLIC_DROP:
                    result[key] = safe_metadata
                continue
            if drop_blocked_text and _contains_open_source_blocked_text(key):
                continue
            if _is_public_secret_key(key):
                continue
            if _is_public_blocked_text_field(key) and _contains_open_source_blocked_text(item):
                # ``model_names``/``model_protocols`` are maps whose keys are
                # model ids, not a single display label.  Keep the map here so
                # ``public_provider`` can filter only the disabled entries
                # after the model lists have been normalized.  Dropping the
                # complete map would make otherwise valid custom providers
                # lose their labels.  A scalar/list label field is omitted as
                # soon as it advertises a disabled route.
                if _public_key_compact(key) not in {"modelnames", "modelprotocols"}:
                    # At the top level this is a field omission; for an app or
                    # workflow the early check above drops the complete entry.
                    continue
            child_drop = _public_key_compact(key) in {"rhapps", "rhworkflows"}
            cleaned = _sanitize_public_value(
                item,
                drop_blocked_entries=child_drop,
                drop_blocked_text=drop_blocked_text,
                drop_public_metadata=drop_public_metadata,
            )
            if cleaned is not _PUBLIC_DROP:
                result[key] = cleaned
        return result
    if isinstance(value, (list, tuple)):
        result = []
        for item in value:
            if drop_blocked_entries and (
                (isinstance(item, str) and OPEN_SOURCE_BLOCKED_TEXT_RE.search(item))
                or _public_entry_contains_blocked_marker(item)
            ):
                continue
            cleaned = _sanitize_public_value(
                item,
                drop_blocked_entries=False,
                drop_blocked_text=drop_blocked_text,
                drop_public_metadata=drop_public_metadata,
            )
            if cleaned is not _PUBLIC_DROP:
        ...
        return result
    if isinstance(value, str):
        if drop_blocked_text and _contains_open_source_blocked_text(value):
            return _PUBLIC_DROP
        # A few upstream endpoints put JSON in a scalar field.  Parse only
        # object/array-looking strings, then serialize the sanitized result;
        # ordinary prompts and data URLs retain their original representation.
        ...
        if stripped[:1] in {"{", "["} and stripped[-1:] in {"}", "]"}:
            ...
            if isinstance(parsed, (dict, list)):
                cleaned = _sanitize_public_value(
                    parsed,
                    drop_blocked_entries=drop_blocked_entries,
                    drop_blocked_text=drop_blocked_text,
                    drop_public_metadata=drop_public_metadata,
                )
                if cleaned is _PUBLIC_DROP:
                    return _PUBLIC_DROP
                try:
                    return json.dumps(cleaned, ensure_ascii=False, separators=(",", ":"))
                except (TypeError, ValueError):
                    pass
        return _redact_public_string(value)
    return value


def sanitize_public_provider_data(provider: Any) -> Any:
    """Return a recursively sanitized copy suitable for public JSON output."""
    cleaned = _sanitize_public_value(provider, drop_public_metadata=True, drop_blocked_text=True)
    return cleaned if isinstance(cleaned, dict) else {}


def sanitize_public_workflow_config(config: Any) -> Dict[str, Any]:
    """Sanitize a saved/fetched workflow before returning it to a browser.

    Workflow JSON is user-extensible, so a shallow allow-list would either
    leak credentials hidden in ``raw`` or break custom node fields.  The
    recursive sanitizer removes credential-shaped keys and drops disabled
    vendor text while retaining all unrelated workflow data.  This function
    is used only at API response/write boundaries; internal execution can
    continue using the original normalized config when needed.
    """
    cleaned = _sanitize_public_value(config, drop_blocked_text=True)
    if not isinstance(cleaned, dict):
        return {}
    # Keep the response shape predictable for older clients.  Missing maps
    # are represented as empty objects rather than ``null``.
    if not isinstance(cleaned.get("workflowJson"), dict):
        cleaned["workflowJson"] = {}
    if not isinstance(cleaned.get("raw"), dict):
        cleaned["raw"] = {}
    if isinstance(cleaned.get("fields"), list):
        cleaned["fields"] = [
            item for item in cleaned["fields"] if isinstance(item, dict)
        ]
    return cleaned


def sanitize_public_runtime_value(value: Any) -> Any:
    """Sanitize an arbitrary upstream payload before exposing it in JSON."""
    cleaned = _sanitize_public_value(value, drop_blocked_text=True)
    return {} if cleaned is _PUBLIC_DROP else cleaned


@app.middleware("http")
async def sanitize_public_api_json(request: Request, call_next):
    """Apply the public sanitizer at the final JSON response boundary.

    RunningHub and legacy provider adapters have many historical return paths.
    Keeping this last-pass guard here prevents a newly added raw field or an
    exception detail from bypassing the same credential/vendor policy.  It
    It covers the legacy ``/generate`` JSON endpoint as well as ``/api/``;
    file, image, HTML, and WebSocket responses are left byte-for-byte
    unchanged.
    """
    ...
    if (
        not (path.startswith("/api/") or path == "/generate")
    ...
    try:
        ...
        # Rebuild the response because a streaming iterator may already have
        # been consumed while collecting the body.
        return Response(
            content=body,
    ...
    cleaned = sanitize_public_runtime_value(payload)
...
def is_open_source_paid_label(value: Any) -> bool:
    """Return whether *value* carries an explicit paid-tier label.

    This intentionally uses token boundaries instead of a substring check:
    ``seedance25-vip`` and ``premium_model`` are suppressed, while ordinary
    custom names such as ``flux-pro`` or a host like ``apistudio.vip`` remain
    usable.  Chinese labels are unambiguous enough to match directly.
    """
    normalized = str(value or "").strip().lower()
    if not normalized:
        return False
    if OPEN_SOURCE_BILLING_TEXT_RE.search(normalized):
        return True
    return any(
        token.lower() in OPEN_SOURCE_BILLING_MODEL_MARKERS
        for token in OPEN_SOURCE_BILLING_TOKEN_RE.findall(normalized)
    )


def is_open_source_blocked_model(value: Any) -> bool:
    """Return whether a model name is unavailable in the public build.

    Disabled vendor routes (currently Grok) and explicit paid-tier labels are
    both hidden from public model lists and rejected at request boundaries.
    """
    normalized = str(value or "").strip().lower()
    if not normalized:
        return False
    return bool(
        any(marker in normalized for marker in OPEN_SOURCE_BLOCKED_MODEL_MARKERS)
        or is_open_source_paid_label(normalized)
    )


def is_open_source_blocked_provider(provider: Any) -> bool:
    """Return whether a provider identity is disabled in the public build.

    This function intentionally does *not* inspect the model lists.  A normal
    custom provider may advertise many models; individual disabled/paid model
    entries are removed by ``model_list_from_values`` while the provider itself
    stays usable for its remaining models.  Provider billing labels are only
    checked in the explicit id/name fields, not in ``base_url``; this avoids
    treating a legitimate host such as ``apistudio.vip`` as a paid selector.
    """
    if hasattr(provider, "dict"):
        try:
            provider = model_dump_compat(provider)
        except Exception:
            provider = {}
    ...
    if any(marker in identity for marker in OPEN_SOURCE_BLOCKED_PROVIDER_MARKERS):
        return True
    # Keep the billing check narrow and avoid URL/TLD false positives.
    provider_label = " ".join(
        str(provider.get(key) or "").strip().lower() for key in ("id", "name")
    )
    return is_open_source_paid_label(provider_label)


def blocked_provider_error() -> HTTPException:
    """Use a vendor-neutral error so disabled legacy names are not promoted."""
    return HTTPException(status_code=400, detail="该平台或模型在开源版本中不可用，请选择其他平台或模型。")


def is_jimeng_credit_command(command: Any) -> bool:
    """Return whether a legacy Jimeng command queries account/billing data."""
    return bool(JIMENG_CREDIT_COMMAND_RE.search(str(command or "").strip()))


def jimeng_no_billing_response(command: Any = "") -> Dict[str, Any]:
    """Neutral response used by disabled Jimeng credit/help compatibility APIs."""
    return {
        "success": False,
        "disabled": True,
        # Do not echo a disabled command name (for example ``user_credit``)
        # back into a public response; old clients only need the neutral state.
        "command": "",
        "message": OPEN_SOURCE_NO_BILLING_MESSAGE,
        "text": OPEN_SOURCE_NO_BILLING_MESSAGE,
        "raw": None,
    }


# The distribution build has no billing surface.  Keep the legacy
# ``useWallet`` request fields for old clients, but always resolve them to the
# normal provider key so an imported workflow cannot select an account wallet.
OPEN_SOURCE_NO_BILLING = True
...
MODELSCOPE_CHAT_MODELS = list(dict.fromkeys([
    m for m in [*MODELSCOPE_DEFAULT_CHAT_MODELS, *_MODELSCOPE_CONFIGURED_CHAT_MODELS]
    if m and not is_open_source_blocked_model(m)
]))
...
# 版本号提升后会清理旧版本自动注入的默认 LoRA，避免已删除的第三方条目再次出现。
MODELSCOPE_DEFAULTS_VERSION = 4
CHAT_MODEL = os.getenv("CHAT_MODEL", "gpt-4o-mini")
IMAGE_MODEL = os.getenv("IMAGE_MODEL", "gpt-image-2")
# Keep scalar legacy defaults safe as well as the list-valued settings below.
# A user may have an old environment value that names a disabled model; it
# must not become the fallback used by an endpoint that receives no model.
if is_open_source_blocked_model(CHAT_MODEL):
    CHAT_MODEL = "gpt-4o-mini"
if is_open_source_blocked_model(IMAGE_MODEL):
...
def model_list(env_name, primary, defaults):
    ...
    deduped = []
    for value in values:
        if value and not is_open_source_blocked_model(value) and value not in deduped:
...
def reload_env_globals():
    ...
    # Video engines are opt-in: keep the open-source default local/mock and
    # read model names only from an explicit VIDEO_MODELS environment value.
    VIDEO_MODELS = model_list("VIDEO_MODELS", "", [])
    _configured = [m.strip() for m in os.getenv("MODELSCOPE_CHAT_MODELS", "").split(",") if m.strip()]
    MODELSCOPE_CHAT_MODELS = list(dict.fromkeys([
        m for m in [*MODELSCOPE_DEFAULT_CHAT_MODELS, *_configured]
        if m and not is_open_source_blocked_model(m)
    ]))
...
# No vendor model is bundled in the public default.  Configure one through
# VIDEO_MODELS (or an individual provider) when a user wants remote video.
VIDEO_MODELS = model_list("VIDEO_MODELS", "", [])
    ...
    if ms_default:
        ...
        if not current:
            ...
            if seeded_version < MODELSCOPE_DEFAULTS_VERSION:
                ...
                # 旧版本的默认 LoRA 已移除；升级时清掉旧种子，之后由用户自行添加。
                loras = normalize_ms_loras(MODELSCOPE_DEFAULT_LORAS)
...
def model_list_from_values(values):
    ...
    for value in values or []:
        ...
        if item and not is_open_source_blocked_model(item) and item not in deduped:
...
def normalize_provider(item):
    if is_open_source_blocked_provider(item):
        raise blocked_provider_error()
    ...
    allowed_models = set(image_models) | set(chat_models) | set(video_models)
    model_names = {
        str(key): str(value)
        for key, value in normalize_model_name_map(item.get("model_names")).items()
        if str(key) in allowed_models and not is_open_source_blocked_model(value)
    }
    model_protocols = {
        str(key): str(value)
        for key, value in normalize_model_protocols(item.get("model_protocols")).items()
        if str(key) in allowed_models
    }
    return {
        "id": provider_id,
        "name": name,
...
def load_api_providers():
    ...
    if not os.path.exists(API_PROVIDERS_FILE):
        merged_defaults = merge_default_api_providers(defaults)
        return [item for item in merged_defaults if not is_open_source_blocked_provider(item)]
    try:
        ...
        for item in raw if isinstance(raw, list) else []:
            if not isinstance(item, dict):
                continue
            # Skip disabled legacy identities before normalization so one
            # stale entry cannot abort loading the remaining safe providers.
            if is_open_source_blocked_provider(item):
                continue
            normalized = normalize_provider(item)
            # Keep legacy settings on disk untouched, but never make a
            # disabled provider available to runtime callers/UI consumers.
            if is_open_source_blocked_provider(normalized):
                continue
            providers.append(normalized)
        merged = merge_default_api_providers(providers or defaults, inject_missing=not bool(providers))
        return [item for item in merged if not is_open_source_blocked_provider(item)]
    except Exception as e:
        print(f"加载 API 平台配置失败: {e}")
        return [item for item in defaults if not is_open_source_blocked_provider(item)]

def _read_persisted_provider_snapshot():
    """Read the previous provider file without exposing it to a response.

    Public/runtime provider objects intentionally omit disabled models and
    provider identities.  A settings save should not silently erase those
    entries from a user's local migration data, though, so the writer merges
    them back into the on-disk snapshot below.  Fail closed on malformed or
    non-list data; the caller can still write the new safe provider list.
    """
    try:
        with open(API_PROVIDERS_FILE, "r", encoding="utf-8") as f:
            raw = json.load(f)
...
def _persistence_copy_without_credentials(value):
    """Copy a hidden legacy provider while dropping credential-shaped keys."""
    if isinstance(value, dict):
        return {
            key: _persistence_copy_without_credentials(item)
            for key, item in value.items()
            if not _is_public_secret_key(key)
        }
    if isinstance(value, list):
        return [_persistence_copy_without_credentials(item) for item in value]
    if isinstance(value, tuple):
        return [_persistence_copy_without_credentials(item) for item in value]
    return value


def _merge_hidden_provider_catalog(providers, previous, extra_hidden_models=None):
    """Preserve disabled model/provider records on disk, never in runtime.

    ``providers`` contains the normalized runtime list and is therefore safe
    for the public build.  ``previous`` is the old local file.  We retain
    explicit paid/vendor model ids and blocked provider records only in the
    local file so a later migration can recover them; all public serialization
    still passes through ``public_provider`` and filters them again.
    """
    current = [dict(item) for item in (providers or []) if isinstance(item, dict)]
    ...
    extra_hidden_models = extra_hidden_models or {}
    merged = []
    for item in current:
        provider_id = str(item.get("id") or "").strip().lower()
        prior = previous_by_id.get(provider_id) or {}
        item = dict(item)
        for model_key in ("image_models", "chat_models", "video_models"):
            visible_models = [
                str(model).strip()
                for model in (item.get(model_key) or [])
                if str(model or "").strip()
            ]
            hidden_models = []
            for source in (
                prior.get(model_key) if isinstance(prior, dict) else [],
                (extra_hidden_models.get(provider_id) or {}).get(model_key),
            ):
                for model in source or []:
                    model = str(model or "").strip()
                    if model and is_open_source_blocked_model(model) and model not in hidden_models:
                        hidden_models.append(model)
            item[model_key] = list(dict.fromkeys([*visible_models, *hidden_models]))
            if hidden_models:
                # Keep labels/protocol hints for a future migration, while
                # ensuring they can never become a runtime selection.
                for map_key in ("model_names", "model_protocols"):
                    prior_map = prior.get(map_key) if isinstance(prior, dict) else {}
                    if not isinstance(prior_map, dict):
                        continue
                    current_map = dict(item.get(map_key) or {})
                    for model in hidden_models:
                        if model in prior_map and model not in current_map:
                            current_map[model] = prior_map[model]
                    item[map_key] = current_map
        merged.append(item)
    # A provider whose id/name was explicitly blocked is not shown or used by
    # runtime callers, but keeping a credential-free copy avoids destructive
    # migration saves.  Do not append ordinary providers the user intentionally
    # removed from the settings UI.
    for prior in previous_items:
        provider_id = str(prior.get("id") or "").strip().lower()
        if provider_id in current_ids or not is_open_source_blocked_provider(prior):
            continue
        merged.append(_persistence_copy_without_credentials(prior))
    return merged


def save_api_providers(providers, extra_hidden_models=None):
    os.makedirs(DATA_DIR, exist_ok=True)
    with GLOBAL_CONFIG_LOCK:
        previous = _read_persisted_provider_snapshot()
        persisted = _merge_hidden_provider_catalog(providers, previous, extra_hidden_models)
        # Provider settings are edited from multiple tabs and contain the
        # user's model catalog.  A plain ``open(..., 'w')`` can leave a
        # truncated file if the process is interrupted during a save; use the
        # same atomic replacement primitive as canvas/project persistence.
        _atomic_json_write(API_PROVIDERS_FILE, persisted)
...
def mutate_static_runninghub_provider(mutator):
    ...
    if changed is False:
        return False
    # This template is user-editable state despite living under ``static/``.
    # Keep the update crash-safe so a browser refresh cannot leave the provider
    # catalog as a zero-byte/truncated JSON file.
    _atomic_json_write(STATIC_RUNNINGHUB_API_PROVIDERS_FILE, raw)
    return True
...
def public_provider(provider):
    if not isinstance(provider, dict) or is_open_source_blocked_provider(provider):
        # Public responses must never carry a stale/disabled provider through
        # even when a compatibility caller bypasses ``public_api_providers``.
        return None
    # ``load_api_providers`` normally returns normalized dictionaries without
    # credentials.  RunningHub can merge an imported workflow store after
    # normalization, however, so merge first and then recursively sanitize the
    # complete object.  This strips credentials from raw/workflowJson as well
    # as stale Grok/Sub2API app and workflow cards.
    provider = dict(provider)
    if provider.get("id") == "runninghub":
        try:
            ...
            pass
    provider = sanitize_public_provider_data(provider)
    for model_key in ("image_models", "chat_models", "video_models"):
        provider[model_key] = model_list_from_values(provider.get(model_key) or [])
    if isinstance(provider.get("model_names"), dict):
        allowed_models = set(
            provider.get("image_models", [])
            + provider.get("chat_models", [])
            + provider.get("video_models", [])
        )
        provider["model_names"] = {
            str(key): str(value)
            for key, value in provider["model_names"].items()
            if str(key) in allowed_models and not is_open_source_blocked_model(value)
            and not OPEN_SOURCE_BLOCKED_TEXT_RE.search(str(value or ""))
        }
    if isinstance(provider.get("model_protocols"), dict):
        allowed_models = set(
            provider.get("image_models", [])
            + provider.get("chat_models", [])
            + provider.get("video_models", [])
        ...
        provider["model_protocols"] = {
            str(key): str(value)
            for key, value in provider["model_protocols"].items()
            if str(key) in allowed_models and not OPEN_SOURCE_BLOCKED_TEXT_RE.search(str(value or ""))
        }
    ...
    if provider.get("id") == "runninghub" and not OPEN_SOURCE_NO_BILLING:
...
def public_api_providers():
    return [
        item
        for item in (public_provider(p) for p in load_api_providers())
        if item is not None
    ]
...
def get_primary_provider_id(providers=None):
    ...
    providers = [
        item for item in (providers if providers is not None else load_api_providers())
        if isinstance(item, dict) and not is_open_source_blocked_provider(item)
    ]
...
def get_api_provider(provider_id="comfly"):
    ...
    if target and is_open_source_blocked_provider({"id": target}):
        raise blocked_provider_error()
...
def global_modelscope_token_value() -> str:
    """Read the legacy global token for server-side use only.

    The value is intentionally never returned by an HTTP route.  Keeping this
    fallback lets the legacy Z-Image/Angle pages send an empty ``api_key`` and
    still use a token saved by an older build after ``/api/config/token`` was
    changed to a presence-only response.
    """
    ...
    try:
        ...
        if isinstance(config, dict):
            return strip_auth_scheme(config.get("modelscope_token"), "Bearer")
    except Exception:
        pass
    return ""
...
def modelscope_api_key(explicit_key: str = ""):
    return (
        ...
        or global_modelscope_token_value()
    )
...
def fetch_remote_update_notes(url: str, version: str = "", timeout: float = 5.0) -> Dict[str, Any]:
    ...
    try:
        resp = requests.get(
            ...
            headers={"User-Agent": "Canvas-Lab-Updater"},
            timeout=timeout,
...
def sync_static_html_versions():
    ...
    Older local launchers called this helper to rewrite every static HTML file
    in place.  That made a server restart mutate source files and destroyed
    human-readable cache labels.  ``versioned_static_html`` now performs the
    same transformation only while constructing an HTTP response, so this
    function intentionally does nothing for callers that still import it.
    """
    return None
...
def app_info():
    ...
    return {
        ...
        "tree_url": "",
        "sources": {},
        "update_enabled": REMOTE_UPDATE_ENABLED,
        "update_mode": "local-source",
...
def check_update():
    ...
    return {
        "current": current,
        "latest": {},
        "update_notes": read_local_update_notes(current),
...
def github_bytes(url: str) -> bytes:
    resp = github_get(url, headers={"User-Agent": "Canvas-Lab-Updater"}, timeout=60)
    return resp.content
...
def modelscope_update_file_list() -> List[str]:
    ...
    resp = github_get(MODELSCOPE_TREE_URL, headers={"User-Agent": "Canvas-Lab-Updater"}, timeout=30)
    payload = json.loads(resp.content.decode("utf-8", errors="replace"))
...
def write_update_backup_manifest(backup_dir: str, payload: Dict[str, Any]) -> None:
    """Persist a restore-point manifest without exposing a partial JSON file.

    Update/rollback code can be interrupted while copying a large static
    tree.  The old hand-written ``.tmp`` path was atomic at the final replace,
    but it did not flush the temporary file and could leave a stale sibling
    behind when serialization failed.  Reuse the common fsync + cleanup
    primitive so readers see either the previous manifest or a complete new
    one, even across an abrupt process shutdown.
    """
    path = update_backup_manifest_path(backup_dir)
    _atomic_json_write(path, payload if isinstance(payload, dict) else {})
...
class CanvasVideoRequest(BaseModel):
    ...
    # Remote video execution is opt-in. A caller must name both the saved
    # provider and the provider's model; the public default never silently
    # chooses a commercial endpoint.
    provider_id: str = ""
    model: str = ""
    duration: int = 5
...
class ConversationBatchDeleteRequest(BaseModel):
    ...
    ``ids`` are conversation IDs, never filesystem paths.  Media cleanup is
    opt-in because an assistant response may be referenced by a canvas or the
    generation history index.
    """

    ids: List[str] = Field(default_factory=list)
    remove_unreferenced_media: bool = False
...
class CanvasSaveRequest(BaseModel):
    ...
    # ``settings`` was historically a required/default-empty field.  Older
    # canvas clients do not send it on every save, though, and treating an
    # omitted value as ``{}`` silently erased settings that were already
    # persisted on the canvas.  Keep omission distinguishable from an
    # intentional empty object so partial/legacy saves remain lossless.
...
class DeleteCanvasLogRequest(BaseModel):
    """Request used by the generation-log modal's safe cleanup action."""
...
class CanvasBatchDeleteRequest(BaseModel):
    """批量处理画布：移入回收站、恢复或永久清理。"""
...
class HistoryBatchDeleteRequest(BaseModel):
    """按历史记录 timestamp 批量删除生成记录。"""

    timestamps: List[float] = Field(default_factory=list)
    remove_unreferenced_media: bool = True
...
def save_to_history(record):
    # History is a durable public-facing store (and is also broadcast to
    # connected canvases).  Sanitize at write time so a provider response or a
    # user-supplied signed URL cannot remain on disk even if a legacy route
    # later bypasses the HTTP response middleware.
    safe_record = sanitize_public_runtime_value(record)
    ...
    with HISTORY_LOCK:
        ...
        if os.path.exists(HISTORY_FILE):
            try:
                # Windows editors and older exports may leave a UTF-8 BOM;
                # accepting it avoids treating an otherwise valid history as
                # corrupt.  More importantly, never replace an unreadable
                # existing file with ``[]``: the previous implementation did
                # so and destroyed every older history entry on the next save.
                with open(HISTORY_FILE, 'r', encoding='utf-8-sig') as f:
                    loaded = json.load(f)
                if not isinstance(loaded, list):
                    raise ValueError("历史文件不是列表")
                history = loaded
            except Exception as exc:
                print(f"读取历史文件失败，保留原文件: {exc}")
                return False
        if "timestamp" not in record:
            record["timestamp"] = time.time()
        history.insert(0, record)
        _atomic_json_write(HISTORY_FILE, history[:5000])
        return True
...
def conversation_path(user_id, conversation_id):
    # Conversation IDs are generated UUID/opaque record IDs, not filenames.
    # Silently stripping punctuation used to turn a typo such as ``abc!`` into
    # ``abc`` and let a single-item GET/DELETE target a different record.  The
    # bulk endpoints already validate this shape; keep the legacy routes on the
    # same strict contract instead of normalising caller input.
    ...
    if (
        ...
        or not re.fullmatch(r"[A-Za-z0-9][A-Za-z0-9_-]{0,127}", cleaned)
    ):
        raise HTTPException(status_code=400, detail="无效的对话 ID")
...
def _safe_project_int(value, default=0):
    """Coerce persisted project counters/timestamps without raising.

    Project indexes are user-editable JSON and older versions did not enforce
    numeric types.  A single value such as ``"nan"`` or ``"1st"`` must not
    make the entire ``/api/projects`` response fail while sorting records.
    ``bool`` is deliberately treated as a number-compatible legacy value,
    matching Python's normal ``int`` semantics.
    """
    try:
        numeric = float(value)
...
# IDs/names received by bulk mutation endpoints are treated as opaque values.
# Do not silently strip path separators or other characters: doing so can make
# a typo target a different record and makes audit logs impossible to trust.
BATCH_DELETE_MAX_ITEMS = 1000
BATCH_RECORD_ID_RE = re.compile(r"^[A-Za-z0-9][A-Za-z0-9_-]{0,127}$")


def _strict_batch_ids(values, label="ID", *, pattern=BATCH_RECORD_ID_RE,
                      max_items=BATCH_DELETE_MAX_ITEMS):
    """Validate and de-duplicate a bounded list of record identifiers.

    The function deliberately rejects non-string values, surrounding
    whitespace, empty values, path separators and overlong lists.  Returning a
    stable de-duplicated list lets callers report exactly what was requested
    while avoiding repeated filesystem work.
    """
    if not isinstance(values, list):
        raise HTTPException(status_code=400, detail=f"{label}列表格式不正确")
    if not values:
        raise HTTPException(status_code=400, detail=f"没有选择{label}")
    if len(values) > max_items:
        raise HTTPException(status_code=400, detail=f"单次最多选择 {max_items} 个{label}")
    invalid = []
    result = []
    seen = set()
    for raw in values:
    ...
    if invalid:
        raise HTTPException(
            status_code=400,
            detail={"message": f"包含非法{label}", "invalid": invalid[:50]},
        )
    if not result:
        raise HTTPException(status_code=400, detail=f"没有选择{label}")
    return result
...
def _batch_skip(item_id, reason, detail=""):
    """Build a stable skipped-entry payload shared by bulk endpoints."""
    result = {"id": str(item_id), "reason": str(reason)}
    if detail:
        result["detail"] = str(detail)
    return result


def _strict_batch_timestamps(values, max_items=BATCH_DELETE_MAX_ITEMS):
    """Validate a bounded list of finite history timestamps."""
    if not isinstance(values, list):
        raise HTTPException(status_code=400, detail="时间戳列表格式不正确")
    if not values:
        raise HTTPException(status_code=400, detail="没有选择历史记录")
    if len(values) > max_items:
        raise HTTPException(status_code=400, detail=f"单次最多选择 {max_items} 条历史记录")
    result = []
    seen = set()
    invalid = []
    for raw in values:
        # bool is an int subclass but never represents a meaningful timestamp.
        try:
            numeric = float(raw)
        ...
        if isinstance(raw, bool) or not isinstance(raw, (int, float)) or not finite:
            ...
            continue
        value = numeric
        # Preserve the existing route's 1ms matching tolerance while
        # de-duplicating equivalent values supplied by a multi-select UI.
        key = round(value, 3)
        if key not in seen:
            seen.add(key)
            result.append(value)
    if invalid:
        raise HTTPException(
            status_code=400,
            detail={"message": "包含非法时间戳", "invalid": invalid[:50]},
        )
    if not result:
        raise HTTPException(status_code=400, detail="没有选择历史记录")
    return result
...
def save_conversation(user_id, conversation):
    with CONVERSATION_LOCK:
        ...
        # Conversation lists and chat tabs read these files without a shared
        # database transaction.  Use the same crash-safe replacement primitive
        # as canvases so a reader sees either the old or the complete new JSON,
        # never a transient zero-byte/truncated document.
        _atomic_json_write(path, conversation)
...
def load_conversation(user_id, conversation_id):
    with CONVERSATION_LOCK:
        ...
        try:
            ...
            # A malformed record should produce a stable API error rather than
            # leaking a decoder traceback as a 500.  Callers can then recover
            # by deleting/recreating the damaged conversation.
            raise HTTPException(status_code=409, detail="对话数据无法读取") from exc
        if not isinstance(conversation, dict) or str(conversation.get("id") or "") != conversation_id:
            raise HTTPException(status_code=409, detail="对话记录 ID 与文件名不一致")
        return conversation

def _list_conversations_unlocked(user_id):
    records = []
    ...
    for filename in os.listdir(root):
        ...
        # Do not expose a mismatched file under a caller-visible ID; a damaged
        # legacy record can otherwise make list/get/delete act on different
        # objects depending on which route was used.
        if record_id != filename[:-5] or not BATCH_RECORD_ID_RE.fullmatch(record_id):
            continue
...
def list_conversations(user_id):
    # Keep directory scans consistent with save/delete operations.  Atomic
    # writes protect readers from truncation; this lock also prevents a batch
    # delete from returning a half-updated list in the same process.
    with CONVERSATION_LOCK:
        return _list_conversations_unlocked(user_id)

def canvas_path(canvas_id):
    # Canvas IDs are opaque record identifiers, not user-provided filenames.
    # Do not silently strip punctuation/path characters: ``canvas-one!`` used
    # to resolve to ``canvas-one.json`` and could make a single-item rename,
    # delete or GET operate on the wrong canvas.  Keep the single-item routes
    # aligned with the strict bulk-ID contract.
...
def _atomic_json_write(path, payload):
    ...
    Canvas documents are read by the list endpoint, editor tabs, and the
    websocket-triggered refresh path without holding ``CANVAS_LOCK``.  A
    regular ``open(path, 'w')`` therefore leaves a small window in which a
    reader can observe an empty or truncated document.  Writing and flushing
    a sibling temporary file before replacing the destination gives readers a
    complete old or complete new snapshot, never a partial JSON document.

    The helper deliberately does not acquire a lock: callers that mutate
    canvas/project state already hold ``CANVAS_LOCK`` and the non-reentrant
    lock must remain safe for compound operations.  It is also useful for
    project-index writes, which share the same persistence guarantees.
    """
    ...
    try:
        with tempfile.NamedTemporaryFile(
            ...
            handle.flush()
            # fsync is cheap for these small metadata documents and makes a
            # completed API response survive an abrupt process/OS restart.
...
def _atomic_text_write(path, value, *, encoding="utf-8"):
    """Write a text document with the same crash-safety guarantees as JSON.

    A few user-editable settings are intentionally stored as plain text (for
    example the asset-classification prompt), so encoding them as JSON would
    change the public file format.  Keep the write protocol identical to
    ``_atomic_json_write``: flush and fsync a sibling temporary file, then
    replace the destination in one operation and clean up on every failure.
    """
    target = os.path.abspath(path)
    directory = os.path.dirname(target) or os.getcwd()
    os.makedirs(directory, exist_ok=True)
...
def _write_canvas_locked(canvas):
    """Persist a canvas while the caller owns ``CANVAS_LOCK``.

    Keeping the write primitive separate lets compound operations (for
    example deleting a generation log and resetting its result node) remain
    atomic without recursively acquiring the non-reentrant lock.  The
    monotonic version is updated before the atomic replacement so optimistic
    concurrency checks remain valid across same-millisecond saves.
    """
    previous = int(canvas.get("updated_at") or 0)
    # Keep the optimistic-concurrency version strictly increasing even when
    # two saves happen in the same millisecond (or the system clock moves
    # backwards).  A non-monotonic timestamp can make a newer save look stale
    # to another editor and break log cleanup/version checks.
    canvas["updated_at"] = max(now_ms(), previous + 1)
    _atomic_json_write(canvas_path(canvas["id"]), canvas)


def save_canvas(canvas):
    with CANVAS_LOCK:
        _write_canvas_locked(canvas)
...
def _save_projects_unlocked(projects):
    os.makedirs(DATA_DIR, exist_ok=True)
    _atomic_json_write(PROJECTS_PATH, {"projects": projects})

def save_projects(projects):
    # Project index updates share the canvas data directory.  Keep a stable
    # lock order (PROJECT_LOCK -> CANVAS_LOCK) so project and canvas mutations
    # cannot overwrite each other's read/modify/write snapshots.
    with PROJECT_LOCK:
        with CANVAS_LOCK:
            os.makedirs(DATA_DIR, exist_ok=True)
            _atomic_json_write(PROJECTS_PATH, {"projects": projects})
...
def project_record(p):
    ...
    return {
        "id": str(p.get("id") or ""),
        "name": (name or "未命名项目")[:60],
        "order": _safe_project_int(p.get("order"), 0),
        "created_at": _safe_project_int(p.get("created_at"), 0),
        "updated_at": _safe_project_int(p.get("updated_at"), 0),
    }

def _ensure_default_project_unlocked():
...
def ensure_default_project():
    with PROJECT_LOCK:
        return _ensure_default_project_unlocked()

def new_project(name="新项目"):
    with PROJECT_LOCK:
        projects = _ensure_default_project_unlocked()
        ts = now_ms()
        clean = (str(name or "").strip() or "新项目")[:60]
        order = max([_safe_project_int(p.get("order"), 0) for p in projects], default=0) + 1
...
def list_projects():
    ...
    for p in sorted(
        projects,
        key=lambda x: (
            _safe_project_int(x.get("order"), 0),
            _safe_project_int(x.get("created_at"), 0),
        ),
    ):
...
def load_canvas(canvas_id):
    ...
    try:
        ...
        raise HTTPException(status_code=409, detail="画布数据无法读取") from exc
    if not isinstance(canvas, dict) or str(canvas.get("id") or "") != str(canvas_id):
...
def canvas_record(data):
    ...
    return {
        ...
        "title": str(data.get("title") or "未命名画布")[:80],
        "icon": str(data.get("icon") or "🧩")[:32],
        ...
        "created_at": safe_timestamp(data.get("created_at", 0)),
        "updated_at": safe_timestamp(data.get("updated_at", 0)),
        "deleted_at": safe_timestamp(data.get("deleted_at", 0)),
...
def iter_canvas_records(include_deleted=False):
    ...
    for filename in os.listdir(CANVAS_DIR):
        ...
        try:
            ...
            continue
        # A hand-edited or interrupted record can still be valid JSON but not
        # a canvas object (for example ``[]`` or a scalar).  The list endpoint
        # must skip that entry instead of calling ``.get`` and turning one bad
        # file into a 500 for every project.  Keep the filename/ID relationship
        # strict as well; otherwise a copied record could be exposed under the
        # wrong opaque URL.
        if not isinstance(data, dict):
            continue
        expected_id = os.path.splitext(filename)[0]
        if str(data.get("id") or "") != expected_id:
            continue
...
def selected_model(requested, fallback):
    ...
    if is_open_source_blocked_model(model):
        # Do not echo the disabled vendor/model name back to callers.  This
        # keeps imported workflows and direct API requests from turning the
        # legacy route into a discoverable feature.
        raise blocked_provider_error()
    return model
...
def extract_task_id(data):
    ...
    if raw_id and (raw_id.startswith("task") or "-" in raw_id):
        return raw_id
...
async def delete_storage_files(payload: Dict[str, Any]):
    ...
    # Validate the storage root up front; this also keeps the endpoint scoped
    # to the three configured roots and never to an arbitrary filesystem path.
    storage_kind_dir(kind)
    raw_items = payload.get("items")
    if not isinstance(raw_items, list):
        raise HTTPException(status_code=400, detail="文件列表格式不正确")
    if not raw_items:
        raise HTTPException(status_code=400, detail="请选择要删除的文件")
    if len(raw_items) > BATCH_DELETE_MAX_ITEMS:
        raise HTTPException(status_code=400, detail=f"单次最多选择 {BATCH_DELETE_MAX_ITEMS} 个文件")
    ...
    with STORAGE_DELETE_LOCK:
        for raw in raw_items:
            if not isinstance(raw, str) or raw != raw.strip() or not raw:
                skipped.append(_batch_skip(raw, "invalid_path", "文件路径不合法"))
                continue
            rel = raw.replace("\\", "/")
            parts = rel.split("/")
            if (
                ...
                continue
            if rel in seen:
                skipped.append(_batch_skip(rel, "duplicate", "重复选择"))
                continue
            seen.add(rel)
            try:
                path = storage_file_path(kind, rel)
            except HTTPException:
                skipped.append(_batch_skip(rel, "invalid_path", "文件路径不合法"))
                continue
            if not path or not os.path.isfile(path):
                skipped.append(_batch_skip(rel, "not_found", "文件不存在"))
                continue
            # Do not delete media still referenced by a canvas, conversation,
            # asset library, or history record.  This is especially important
            # for generated files that appear in several editable surfaces.
            try:
                referenced = persisted_json_references_media_path(path)
            except Exception:
                referenced = True
            if history_unreadable:
                referenced = True
            elif not referenced and history_value is not None:
                referenced = json_references_media_path(history_value, path)
            if referenced:
                skipped.append(_batch_skip(rel, "referenced", "文件仍被其他内容引用"))
                continue
            try:
                os.remove(path)
                removed_items.append(rel)
            except OSError as exc:
                skipped.append(_batch_skip(rel, "remove_failed", f"删除失败：{exc}"))
    return {
        # Keep the legacy integer field for existing clients.
        "removed": len(removed_items),
        "removed_count": len(removed_items),
        "removed_items": removed_items,
        "deleted": removed_items,
...
def media_preview_cache_paths(path: str, width: int):
    stat = os.stat(path)
    key = hashlib.sha1(
        # Canonicalize the source path so Windows 8.3 aliases and their long
        # forms address the same preview cache entry.  Cleanup resolves paths
        # canonically as well; using ``abspath`` here alone left orphaned
        # previews when a URL was resolved through a different alias.
...
# ===== Generation-log media cleanup ========================================
#
# A log record can own files that are also referenced by result nodes,
# another canvas, the asset library, or conversation history.  Keep cleanup
# conservative: remove only generated-output files that have no remaining
# persisted reference, and leave unreadable documents untouched.

def collect_local_media_urls(value: Any) -> List[str]:
    """Collect local media URLs from an arbitrarily nested JSON value."""
    urls: List[str] = []
    if isinstance(value, str):
        text = value.strip()
...
def local_media_path_from_url(url: str) -> Optional[str]:
    """Resolve a local media URL using the currently mounted directories."""
    if isinstance(url, dict):
        url = url.get("url", "")
    if not url:
        return None
    clean = urllib.parse.unquote(str(url).split("?", 1)[0].split("#", 1)[0]).replace("\\", "/")
    if clean.startswith("/api/storage-files/"):
        rest = clean[len("/api/storage-files/"):].lstrip("/")
        kind, _, rel = rest.partition("/")
        return storage_file_path(kind, rel) if kind and rel else None
    ...
    if clean.startswith("/assets/"):
        ...
        # /output is the legacy mount.  The generated storage directory is
        # also accepted for installations that point the mount at assets.
        roots = [OUTPUT_DIR, OUTPUT_OUTPUT_DIR]
...
def generated_media_path_from_url(url: str) -> Optional[str]:
    """Resolve only files inside a generated-output directory.

    In particular, an ``/assets/input`` upload must never become deletable
    just because a log happens to mention it.
    """
    try:
...
def json_references_media_path(value: Any, target_path: str) -> bool:
    """Return whether a JSON-compatible value points at ``target_path``."""
...
def persisted_json_references_media_path(target_path: str) -> bool:
    """Scan persisted documents before deleting generated media.

    A malformed JSON file may be in the middle of a write.  Treat that as a
    live reference (safe failure) instead of deleting a file that another
...
def prune_generation_history_for_media(paths: List[str]) -> int:
    """Remove history cards that point at media already being deleted."""
    ...
    try:
        with HISTORY_LOCK:
            ...
            kept = [
                ...
                if not any(
                    json_references_media_path(record, path)
                    for path in paths
            ...
            if removed:
                # Never truncate the live index in place.  A process crash or
                # disk error during a regular ``open(..., 'w')`` would leave
                # an empty/corrupt history file, so all history mutations use
                # the same sibling-temp + replace primitive as save_to_history.
                _atomic_json_write(HISTORY_FILE, kept)
            return removed
...
def smart_owned_result_items(images: List[Any], paths: List[str]) -> List[Any]:
    """Return generated smart-image results, including legacy unmarked ones."""
    return [
        item
        for item in images
        if isinstance(item, dict)
...
def expand_canvas_generated_media_paths(canvas: Dict[str, Any], paths: List[str]) -> List[str]:
    """Include all generated results owned by result nodes touched by a log."""
    expanded = list(paths)
...
def reset_canvas_result_nodes_for_media(canvas: Dict[str, Any], paths: List[str]) -> List[str]:
    """Clear generated media while preserving prompts, references and links."""
    reset_ids: List[str] = []
    updated_nodes = []
    for raw_node in list(canvas.get("nodes") or []):
...
def delete_media_preview_cache(path: str) -> int:
    """Delete derived image/JPEG previews associated with a source file."""
    try:
        stat = os.stat(path)
    except OSError:
        return 0
    ...
    removed = 0
    seen = set()
    # JPEG previews historically accepted any width, while the newer WebP/
    # PNG endpoint uses the same hash for widths 64..2048.  Iterate the
    # bounded range to clean caches produced by either endpoint.
    for width in range(0, 4097):
        ...
        if 64 <= width <= 2048:
            try:
                webp, png = media_preview_cache_paths(path, width)
                names.extend((os.path.basename(webp), os.path.basename(png)))
            except OSError:
                pass
        for name in names:
            if name in seen:
...
def normalize_asset_library(lib):
    ...
    # A partially written/hand-edited index may contain scalar entries.  Drop
    # those records before normalizing so one damaged category cannot turn a
    # library GET or startup migration into an AttributeError/500.
    libraries = [library for library in libraries if isinstance(library, dict)]
    if not libraries:
        libraries = [{
            "id": "default",
            "name": "默认资产库",
    ...
    for library in libraries:
        ...
        cats = [category for category in cats if isinstance(category, dict)]
        ...
        for cat in cats:
            ...
            # Keep only object-shaped items; all downstream CRUD paths rely on
            # ``item.get`` and should remain usable after a malformed import.
            cat["items"] = [item for item in items if isinstance(item, dict)]
...
def load_asset_library():
    # The library index is read by the asset manager while mutation endpoints
    # may be writing it.  Pair the read with the writer lock so malformed
    # transient data cannot become a new in-memory snapshot.  RLock is used
    # because first-run initialization below persists the default in the same
    # critical section.
...
def cleanup_asset_library_file_if_unreferenced(item) -> str:
    ...
    返回清理结果（``deleted``、``referenced``、``not_found`` 或
    ``remove_failed``）。画布、会话、资产库及历史记录仍引用该文件时，
    保留物理文件，避免单项删除绕过批量删除的引用保护。
    """
    try:
        url = item.get("url") if isinstance(item, dict) else ""
        path = output_file_from_url(url)
        ...
        if os.path.isfile(HISTORY_FILE):
            try:
                ...
                # A malformed history file is treated as a live reference.
                return "referenced"
...
def load_asset_classification_prompt():
    with ASSET_CLASSIFICATION_PROMPT_LOCK:
        try:
...
def save_asset_classification_prompt(text):
    ...
    with ASSET_CLASSIFICATION_PROMPT_LOCK:
        os.makedirs(DATA_DIR, exist_ok=True)
        _atomic_text_write(ASSET_CLASSIFICATION_PROMPT_FILE, value)
    return value
    ...
    try:
        ...
        if match:
            try:
                ...
                # Provider captions are best-effort input.  A response that
                # contains braces but is not valid JSON should not turn an
                # otherwise successful upload/classification request into a
                # 500; return the same normalized empty shape as any other
                # unusable caption.
                data = {}
        else:
            data = {}
...
def _write_local_upload_classification(filename, classification):
    ...
    _atomic_json_write(path, normalize_asset_classification(classification))
...
def migrate_asset_library_into_dirs():
    ...
    # Startup migration runs in a worker thread and can overlap a browser
    # mutation during hot reload.  Keep the complete snapshot migration under
    # the same lock used by CRUD routes so it cannot write an older index over
    # a user's edit.  There are no awaits in this synchronous helper.
    with ASSET_LIBRARY_LOCK:
        try:
...
def save_asset_library(lib):
    with ASSET_LIBRARY_LOCK:
        ...
        os.makedirs(DATA_DIR, exist_ok=True)
        # A settings tab can save while another process is restarting the
        # server.  Replace the complete JSON document instead of truncating
        # it with ``open(..., 'w')``; readers then observe an old or new valid
        # index only.
        _atomic_json_write(ASSET_LIBRARY_PATH, lib)
        _schedule_background(manager.broadcast_asset_library_updated(int(lib["updated_at"])))
        return lib
...
def shared_folders_save(data):
    with SHARED_FOLDERS_LOCK:
        ...
        _atomic_json_write(
            SHARED_FOLDERS_FILE,
            data if isinstance(data, dict) else {"folders": []},
        )
...
def save_prompt_libraries(data):
    with PROMPT_LIBRARY_LOCK:
        ...
        os.makedirs(DATA_DIR, exist_ok=True)
        # Prompt libraries are user-authored editable content.  Atomic replace
        # prevents a reload or concurrent tab from observing a truncated index.
        _atomic_json_write(PROMPT_LIBRARY_PATH, data)
        return data
...
def runninghub_api_headers(provider, use_wallet=False):
...
def runninghub_json_headers(provider, use_wallet=False):
...
def runninghub_api_key(provider=None, use_wallet=False, prefer_wallet=False):
    # The public build has no billing path.  Legacy callers may still pass
    # ``use_wallet``/``prefer_wallet`` from imported workflows, but those flags
    # must never select an account-balance credential at runtime.
    if OPEN_SOURCE_NO_BILLING:
        use_wallet = False
        prefer_wallet = False
    ...
    wallet_key = "" if OPEN_SOURCE_NO_BILLING else (str((provider or {}).get("wallet_api_key") or "").strip() or runninghub_wallet_key_value())
...
async def runninghub_store_remote_output(client, remote):
    ...
    if not response.is_success:
        # Signed upstream URLs are not safe public fallbacks.  Callers can
        # report an empty result and let the user retry the download.
        return ""
...
def runninghub_error_detail(message, raw=None, **extra):
    safe_message = sanitize_public_runtime_value(str(message or "RunningHub 请求失败"))
    detail = {"message": safe_message if isinstance(safe_message, str) else "RunningHub 请求失败"}
    for key, value in extra.items():
        if value in (None, ""):
            continue
        safe_value = sanitize_public_runtime_value(value)
        if safe_value is not _PUBLIC_DROP:
            detail[key] = safe_value
    if raw is not None:
        detail["raw"] = sanitize_public_runtime_value(raw)
    return detail
...
def log_runninghub_error(stage, raw=None, **extra):
    try:
        ...
        if raw is not None:
            payload["raw"] = raw
        safe_payload = sanitize_public_runtime_value(payload)
        print(f"RunningHub error: {json.dumps(safe_payload, ensure_ascii=False)[:4000]}")
    except Exception:
...
async def runninghub_models_payload(provider=None):
    ...
    payload = sanitize_model_payload(runninghub_registry_payload(registry))
...
async def runninghub_upload_reference(client, provider, ref):
    ...
    # Reference uploads follow the same non-billing key policy as generation.
    # Keep the legacy request shape, but never select the account-wallet key.
    headers = {"Authorization": bearer_auth_value(runninghub_api_key(provider, use_wallet=False)), "Accept": "application/json"}
    with open(path, "rb") as fh:
...
async def delete_local_assets(payload: dict, request: Request):
    ...
    if not isinstance(names, list):
        raise HTTPException(status_code=400, detail="素材列表格式不正确")
    if not names:
        raise HTTPException(status_code=400, detail="没有选择素材")
    if len(names) > BATCH_DELETE_MAX_ITEMS:
        raise HTTPException(status_code=400, detail=f"单次最多选择 {BATCH_DELETE_MAX_ITEMS} 个素材")
    deleted = []
    skipped = []
    seen = set()
    history_value = None
    ...
    if os.path.isfile(HISTORY_FILE):
        try:
            ...
            # A malformed history index may be in the middle of a write.  It
            # is safer to leave every candidate in place than to risk deleting
            # a file which the index still points at.
            history_value = None
            history_unreadable = True

    with LOCAL_UPLOAD_DELETE_LOCK:
        for name in names:
            if not isinstance(name, str) or name != name.strip() or not name.strip():
                skipped.append(_batch_skip(name, "invalid_path", "素材路径不合法"))
                continue
            raw_name = name.replace("\\", "/")
            parts = raw_name.split("/")
            if any(part in {"", ".", ".."} for part in parts) or raw_name.startswith("/"):
                skipped.append(_batch_skip(name, "invalid_path", "素材路径不合法"))
                continue
            try:
                rel, path = _local_upload_safe_path(raw_name)
            except HTTPException:
                skipped.append(_batch_skip(name, "invalid_path", "素材路径不合法"))
                continue
            if rel in seen:
            ...
            seen.add(rel)
            if not os.path.isfile(path):
                skipped.append(_batch_skip(rel, "not_found", "素材不存在"))
                continue

            # Local uploads can be referenced by an asset-library item, a
            # canvas node, a conversation attachment, or a history record.
            # Keep the source file when any persisted document still points at
            # it; otherwise deleting the upload would silently break an
            # existing editable document.  A malformed persisted document is
            # treated as a reference by ``persisted_json_references...``.
            try:
                referenced = persisted_json_references_media_path(path)
            except Exception:
                referenced = True
            if history_unreadable:
                ...
                referenced = json_references_media_path(history_value, path)
            if referenced:
                skipped.append(_batch_skip(rel, "referenced", "素材仍被其他内容引用"))
                continue
            try:
                os.remove(path)
                ...
                skipped.append(_batch_skip(rel, "remove_failed", "删除素材失败"))
    return {
        "deleted": deleted,
        "removed": deleted,
...
async def runninghub_app_info(webappId: str = ""):
    webapp_id = sanitize_public_opaque_id(webappId)
    if not webapp_id:
    ...
    async with httpx.AsyncClient(timeout=httpx.Timeout(connect=20.0, read=120.0, write=30.0, pool=20.0)) as client:
        try:
            ...
            raise HTTPException(status_code=exc.response.status_code, detail="RunningHub 应用信息请求失败") from exc
        except Exception as exc:
            raise HTTPException(status_code=502, detail="请求 RunningHub 应用信息失败，请稍后重试。") from exc
    if response.status_code >= 400:
        raise HTTPException(
            status_code=response.status_code,
            detail=sanitize_public_runtime_value(raw),
        )
    if isinstance(raw, dict) and raw.get("code") not in (0, "0", None):
        message = raw.get("msg") or f"RunningHub 查询失败 code={raw.get('code')}"
        raise HTTPException(status_code=400, detail=sanitize_public_runtime_value(message))
    data = raw.get("data") if isinstance(raw, dict) else {}
    return {"success": True, "data": sanitize_public_runtime_value(data or {})}

@app.post("/api/runninghub/submit")
async def runninghub_submit(payload: RunningHubSubmitRequest):
    webapp_id = sanitize_public_opaque_id(payload.webappId)
    if not webapp_id:
        raise HTTPException(status_code=400, detail="webappId 必填")
    provider = runninghub_provider()
    use_wallet = False if OPEN_SOURCE_NO_BILLING else bool(payload.useWallet)
    api_key = runninghub_api_key(provider, use_wallet=use_wallet)
    body = {
        "apiKey": api_key,
        "webappId": webapp_id,
    ...
    async with httpx.AsyncClient(timeout=httpx.Timeout(connect=20.0, read=180.0, write=120.0, pool=20.0)) as client:
        try:
            response = await client.post(url, headers=runninghub_app_headers(True, use_wallet), json=body)
            raw = response.json()
        except Exception as exc:
    ...
    if isinstance(raw, dict) and raw.get("code") in (0, "0"):
        task_id = sanitize_public_opaque_id(
            raw.get("data", {}).get("taskId") if isinstance(raw.get("data"), dict) else ""
        )
        if not task_id:
        ...
        return {
            "success": True,
            "data": {"taskId": task_id, "raw": sanitize_public_runtime_value(raw)},
        }
...
async def runninghub_workflow_info(workflowId: str = ""):
    workflow_id = runninghub_workflow_store_key(workflowId)
    if not workflow_id:
        raise HTTPException(status_code=400, detail="workflowId 必填")
    provider = runninghub_provider()
    api_key = runninghub_api_key(provider)
    ...
    async with httpx.AsyncClient(timeout=httpx.Timeout(connect=20.0, read=180.0, write=60.0, pool=20.0)) as client:
        try:
            ...
            raise HTTPException(status_code=502, detail="拉取 RunningHub 工作流参数失败，请稍后重试。") from exc
    if response.status_code >= 400:
        raise HTTPException(
            status_code=response.status_code,
            detail=sanitize_public_runtime_value(raw),
    ...
    if not isinstance(raw, dict) or raw.get("code") not in (0, "0"):
        safe_msg = (raw.get("msg") if isinstance(raw, dict) else "") or "RunningHub 工作流参数拉取失败"
        raise HTTPException(status_code=400, detail=sanitize_public_runtime_value(safe_msg))
    ...
    if isinstance(prompt, str) and prompt.strip():
        try:
            ...
            raise HTTPException(status_code=502, detail="RunningHub 工作流 JSON 解析失败，请检查上游返回。") from exc
    elif isinstance(prompt, dict):
        workflow_json = prompt
    node_info_list = runninghub_workflow_node_info_list(workflow_json)
    safe = sanitize_public_workflow_config({
        "workflowId": workflow_id,
        "workflowJson": workflow_json,
        "raw": raw,
    })
    safe_nodes = _sanitize_public_value(node_info_list, drop_blocked_text=True)
    if not isinstance(safe_nodes, list):
        safe_nodes = []
    return {
        "success": True,
        "data": {
            "workflowId": workflow_id,
            "nodeInfoList": safe_nodes,
            # Keep diagnostics available, but never return upstream keys or
            # disabled-vendor metadata from the raw response.
            "raw": safe.get("raw") or {},
        },
    }
...
def list_runninghub_workflows():
    ...
    for workflow_id, cfg in merged.items():
        ...
        safe_item = sanitize_public_workflow_config(item)
        if not _contains_blocked_text_anywhere(safe_item) and safe_item.get("title"):
            items.append(safe_item)
    items.sort(key=lambda item: str(item.get("title") or item.get("workflowId") or ""))
...
def get_runninghub_workflow(workflow_id: str):
    ...
    return {"workflow": sanitize_public_workflow_config(cfg)}
...
async def fetch_runninghub_workflow(payload: RunningHubWorkflowConfig):
    ...
    async with httpx.AsyncClient(timeout=httpx.Timeout(connect=20.0, read=180.0, write=60.0, pool=20.0)) as client:
        try:
            ...
            raise HTTPException(status_code=502, detail="Failed to fetch RunningHub workflow parameters. Please try again.") from exc
    if response.status_code >= 400:
        raise HTTPException(
            status_code=response.status_code,
            detail=sanitize_public_runtime_value(raw),
    ...
    if not isinstance(raw, dict) or raw.get("code") not in (0, "0"):
        message = (raw.get("msg") if isinstance(raw, dict) else "") or "RunningHub workflow fetch failed"
        raise HTTPException(status_code=400, detail=sanitize_public_runtime_value(message))
    data = raw.get("data") if isinstance(raw.get("data"), dict) else {}
    prompt = data.get("prompt")
    workflow_json = {}
    if isinstance(prompt, str) and prompt.strip():
        try:
            ...
            raise HTTPException(status_code=502, detail="Failed to parse RunningHub workflow JSON returned by the provider.") from exc
    elif isinstance(prompt, dict):
        workflow_json = prompt
    ...
    safe_fields = _sanitize_public_value(fields, drop_blocked_text=True)
    safe["fields"] = safe_fields if isinstance(safe_fields, list) else []
...
def save_runninghub_workflow(workflow_id: str, payload: RunningHubWorkflowConfig):
    ...
    # Store the same sanitized representation that is returned to clients so
    # an imported workflow cannot leave credentials or disabled-vendor text in
    # the local workflow store and later reappear through another route.
    cfg = sanitize_public_workflow_config(cfg)
    cfg["workflowId"] = key
...
async def runninghub_query(taskId: str = "", useWallet: bool = False):
    task_id = sanitize_public_opaque_id(taskId)
    if not task_id:
        raise HTTPException(status_code=400, detail="taskId 必填")
    provider = runninghub_provider()
    use_wallet = False if OPEN_SOURCE_NO_BILLING else bool(useWallet)
    api_key = runninghub_api_key(provider, use_wallet=use_wallet)
    ...
    async with httpx.AsyncClient(timeout=httpx.Timeout(connect=20.0, read=240.0, write=30.0, pool=20.0)) as client:
        try:
            response = await client.post(url, headers=runninghub_app_headers(True, use_wallet), json={"apiKey": api_key, "taskId": task_id})
            raw = response.json()
        except Exception as exc:
        ...
        if code in (0, "0"):
            ...
            for remote in runninghub_extract_outputs(raw.get("data")):
                try:
                    ...
                    local_url = ""
                # Never fall back to a signed upstream URL in a public
                # response.  A successful download is represented by a local
                # /output or /assets URL; failed downloads are omitted.
        ...
        return {
            ...
            "data": {
                ...
                "failReason": sanitize_public_runtime_value(runninghub_fail_reason(raw)),
                "code": code,
                "raw": sanitize_public_runtime_value(raw),
            },
        }
...
async def runninghub_upload_asset(payload: RunningHubUploadAssetRequest):
    ...
    async with httpx.AsyncClient(timeout=httpx.Timeout(connect=20.0, read=240.0, write=240.0, pool=20.0), follow_redirects=True) as client:
        ...
        if path:
            ...
            raise HTTPException(status_code=400, detail="不支持的素材地址，请使用本地素材或可访问的 http(s) 地址。")
        if not content:
        ...
        try:
            ...
            # Upstream exceptions can contain URLs, provider names, or other
            # implementation details.  Keep the public error neutral and log
            # the original only on the server side.
            log_runninghub_error("upload-asset-request", extra_error=str(exc))
            raise HTTPException(status_code=502, detail="上传素材到 RunningHub 失败，请稍后重试。") from exc
    if response.status_code >= 400:
        raise HTTPException(
            status_code=response.status_code,
            detail=sanitize_public_runtime_value(raw),
    ...
    if isinstance(raw, dict) and raw.get("code") in (0, "0") and isinstance(raw.get("data"), dict) and raw["data"].get("fileName"):
        file_name = sanitize_public_opaque_id(raw["data"].get("fileName"))
        if file_name:
            return {"success": True, "data": {"fileName": file_name, "fileType": raw["data"].get("fileType") or content_type}}
        raise HTTPException(status_code=502, detail="RunningHub 返回了无效素材标识，请稍后重试。")
    safe_reason = sanitize_public_runtime_value(
        raw.get("msg") if isinstance(raw, dict) else ""
    )
    if not isinstance(safe_reason, str) or not safe_reason.strip():
        safe_reason = "RunningHub 上传失败，请检查服务配置后重试。"
    raise HTTPException(status_code=400, detail=safe_reason)
...
async def jimeng_status():
    ...
    # Older builds used ``user_credit`` as an implicit login probe.  That
    # command exposes account/billing data, so the open-source build reports
    # installation/version only; authentication is checked when a generation
    # command actually runs.
    return {
        "installed": True,
        "logged_in": None,
        "message": "即梦 CLI 已安装；登录状态将在执行任务时验证。",
...
async def jimeng_credit():
    # Keep the legacy route for old clients, but never execute or return the
    # account-credit command in the distribution build.
    return jimeng_no_billing_response("credit")
...
async def jimeng_login_status():
    ...
    # Do not probe login state with ``user_credit``.  A completed login
    # subprocess with a zero exit status is sufficient for the in-flight QR
    # flow; when no flow is active the state is intentionally unknown.
    logged_in = None
    if not running and proc is not None:
        return_code = getattr(proc, "returncode", None)
        lowered = text.lower()
        failed = any(marker in lowered for marker in ("failed", "error", "cancel", "失败", "错误"))
        logged_in = return_code == 0 and not failed
    return {
        "success": True,
        "running": running,
        "logged_in": logged_in,
...
def sanitize_jimeng_help_text(value: Any) -> str:
    """Remove account/billing lines from legacy CLI help output."""
    text = str(value or "")
    kept = [line for line in text.splitlines() if not is_jimeng_credit_command(line)]
    if len(kept) != len(text.splitlines()):
        kept.append(OPEN_SOURCE_NO_BILLING_MESSAGE)
    return "\n".join(kept).strip()

@app.post("/api/jimeng/help")
async def jimeng_help(payload: JimengHelpRequest):
    command = str(payload.command or "").strip()
    if is_jimeng_credit_command(command):
        return jimeng_no_billing_response(command)
    allowed = {"", "login", "logout", "text2image", "image2image", "image_upscale", "text2video", "image2video", "multimodal2video", "frames2video", "multiframe2video", "list_task", "query_result"}
    if command not in allowed:
    ...
    stdout = sanitize_jimeng_help_text(raw.get("_stdout") if isinstance(raw, dict) else raw)
    stderr = sanitize_jimeng_help_text(raw.get("_stderr") if isinstance(raw, dict) else "")
    text = "\n".join(part for part in (stdout, stderr) if part).strip()
    # Return only sanitized text fields; an upstream CLI may include the
    # blocked command in an arbitrary nested raw object.
    return {
        "success": True,
        "command": command,
        "text": text,
        "raw": {"_stdout": stdout, "_stderr": stderr},
    }
...
@app.get("/api/config")
async def ai_config():
    safe_chat_models = model_list_from_values(CHAT_MODELS)
    safe_image_models = model_list_from_values(IMAGE_MODELS)
    safe_video_models = model_list_from_values(VIDEO_MODELS)
    safe_ms_chat_models = model_list_from_values(MODELSCOPE_CHAT_MODELS)
    preferred_chat_model = next((m for m in safe_chat_models if m == "gpt-5.5"), safe_chat_models[0] if safe_chat_models else "")
    providers = public_api_providers()
    return {
        "base_url": AI_BASE_URL,
        "chat_model": preferred_chat_model,
        "image_model": safe_image_models[0] if safe_image_models else "",
        "chat_models": safe_chat_models,
        "image_models": safe_image_models,
        "video_models": safe_video_models,
        "comfy_instances": COMFYUI_INSTANCES,
        "api_providers": providers,
        "has_api_key": bool(AI_API_KEY),
        "ms_chat_models": safe_ms_chat_models,
...
async def save_providers(payload: List[ApiProviderPayload]):
    providers = []
    env_updates = {}
    # The browser intentionally receives a filtered model catalog.  Preserve
    # any disabled/vendor or paid-tier ids that arrive from an older client in
    # the local provider file, without allowing them into the runtime object.
    hidden_model_catalog = {}
    # 收集每个 item 的 primary 字段
    raw_primary_flags = [bool(getattr(item, "primary", False)) for item in payload]
    for item in payload:
        raw_item = model_dump_compat(item, exclude={"api_key"})
        if is_open_source_blocked_provider(raw_item):
            # A stale page may still submit a provider that was hidden by the
            # runtime filter.  Reject it without naming the disabled vendor.
            raise blocked_provider_error()
        raw_provider_id = str(raw_item.get("id") or "").strip().lower()
        hidden_model_catalog[raw_provider_id] = {
            model_key: [
                str(model).strip()
                for model in (raw_item.get(model_key) or [])
                if str(model or "").strip() and is_open_source_blocked_model(model)
            ]
            for model_key in ("image_models", "chat_models", "video_models")
        }
        provider = normalize_provider(raw_item)
        ...
        if provider["id"] == "runninghub" and not OPEN_SOURCE_NO_BILLING:
    ...
    if primary_indices:
        ...
        for i, p in enumerate(providers):
            p["primary"] = (i == winner)
    save_api_providers(providers, hidden_model_catalog)
...
# --- ModelScope Token (只公开是否存在，不下发 Token 内容) ---

@app.get("/api/config/token")
async def get_global_token():
    # 优先读 env，回退到 global_config.json（兼容旧数据）。 旧版页面曾
    # 直接读取 ``token`` 并把它放进浏览器响应；开源版本只返回状态，实际
    # 请求由后端的 modelscope_api_key() 在服务端解析，避免凭据下发。
    token_present = bool(modelscope_api_key())
    if token_present:
        return {"has_token": True, "token_present": True}
    if os.path.exists(GLOBAL_CONFIG_FILE):
        try:
            with open(GLOBAL_CONFIG_FILE, 'r', encoding='utf-8') as f:
                config = json.load(f)
                token_present = bool(str(config.get("modelscope_token") or "").strip())
        except:
            pass
    return {"has_token": token_present, "token_present": token_present}
...
def protocol_from_payload(payload):
    ...
    payload_base_url = str(getattr(payload, "base_url", "") or "").strip()
    if is_open_source_blocked_provider({"id": provider_id, "base_url": payload_base_url}):
        raise blocked_provider_error()
    if provider_id == "volcengine":
        return "volcengine"
    if provider_id == "runninghub":
    ...
    if provider_id == "jimeng":
        return "jimeng"
    base_url = payload_base_url.lower()
...
def api_key_from_payload(payload, protocol: str = ""):
    ...
    provider_id = str(getattr(payload, "provider_id", "") or "").strip().lower()
    if is_open_source_blocked_provider({
        "id": provider_id,
...
def sanitize_model_payload(payload):
    """Remove disabled model names from a model-discovery response.

    Upstream registries are user-controlled data, so provider-level filtering
    alone is not enough: a normal custom endpoint can still return a Grok
    entry.  Keep the response shape stable while ensuring every list consumed
    by the settings picker is safe.
    """
    if not isinstance(payload, dict):
        return payload
    result = dict(payload)
    for key in ("image_models", "chat_models", "video_models", "all"):
        values = result.get(key)
        if isinstance(values, list):
            result[key] = model_list_from_values(values)
    if isinstance(result.get("model_names"), dict):
        allowed = set(result.get("all") or [])
        result["model_names"] = {
            str(key): str(value)
            for key, value in result["model_names"].items()
            if str(key) in allowed and not is_open_source_blocked_model(value)
        }
    if isinstance(result.get("all"), list):
        result["all"] = model_list_from_values(result["all"])
        result["model_count"] = len(result["all"])
        if "total" in result:
            result["total"] = len(result["all"])
    return result
...
def parse_upstream_models(raw, protocol="openai"):
    ...
    for it in items:
        if isinstance(it, str):
            ...
            mid = ""
        if mid and not is_open_source_blocked_model(mid):
            mid = str(mid)
...
async def fetch_upstream_models(provider_id: str):
    ...
    # The open-source build only accepts the regular service key. Never read
    # or select an account-balance credential during model discovery.
    api_key = provider_env_key_value(provider["id"])
    if not api_key:
...
async def build_online_image_result(payload: OnlineImageRequest):
    ...
    _schedule_background(manager.broadcast_new_image(result))
    return result
...
async def query_image_task(payload: ImageTaskQueryRequest):
    provider = get_api_provider(payload.provider_id)
    task_id = sanitize_public_opaque_id(payload.task_id)
    if not task_id:
    ...
    if is_runninghub_provider(provider):
        ...
        try:
                ...
                if code in (0, "0"):
                    ...
                    for remote in runninghub_extract_outputs(raw.get("data")):
                        try:
                            ...
                            local_url = ""
                        if str(local_url or "").startswith(("/output/", "/assets/")):
                            local_urls.append(local_url)
                    ...
                    result = sanitize_public_runtime_value(result)
                    save_to_history(result)
                    _schedule_background(manager.broadcast_new_image(result))
                    return result
                if code in (805, "805"):
                    return sanitize_public_runtime_value({
                        "status": "failed",
                        "task_id": task_id,
                        "provider_id": provider["id"],
                        "provider_name": provider.get("name") or provider["id"],
            ...
            raise HTTPException(status_code=exc.response.status_code, detail="查询 RunningHub 任务失败，请稍后重试。") from exc
        except httpx.HTTPError as exc:
            raise HTTPException(status_code=502, detail="查询 RunningHub 任务失败，请稍后重试。") from exc
    timeout = httpx.Timeout(connect=20.0, read=300.0, write=60.0, pool=20.0)
    try:
        async with httpx.AsyncClient(timeout=timeout, follow_redirects=True) as client:
...
def video_task_url_candidates(provider, base_url, task_id, submit_url=""):
    ...
    v1_videos = f"{base_url}/v1/videos/{task_id}"
    return [v1_videos, v1_task, v1_generic_task, v2_task]
...
async def canvas_video(payload: CanvasVideoRequest):
    ...
    if not provider_id:
        raise HTTPException(status_code=400, detail="请先选择视频 API 平台；未配置时可使用本地预览。")
    if provider_id in {"comfyui", "openai_local"}:
        raise HTTPException(status_code=400, detail="本地引擎不会通过远程视频接口提交，请使用 ComfyUI/本地工作流节点。")
    provider = get_api_provider_exact(provider_id)
    requested_model = str(payload.model or "").strip()
    if not requested_model:
        raise HTTPException(status_code=400, detail="请先选择或手动填写该平台的模型标识。")
    requested_model = selected_model(requested_model, "")
    # The provider-specific adapters below historically read ``payload.model``
    # directly.  Keep that legacy call shape, but seed it with the validated
    # user selection so no adapter can silently fall back to a bundled model.
    ...
    is_agnes = is_agnes_provider(provider, requested_model)
    ...
    # Provider-specific adapters may normalize aliases, but the initial value
    # always comes from the explicit request.
    if is_tudou_provider(provider) and is_tudou_video_model(requested_model):
        try:
            ...
            # Keep the provider error branch self-contained.  Older upstream
            # code called an undefined friendly-error helper, which replaced
            # the useful HTTP error with a ``NameError``.  The
            # bounded response body is enough context for a local preview/log.
            text = (exc.response.text or "").strip()
            detail = f"土豆视频接口错误：{text[:1200]}" if text else "土豆视频接口返回错误"
            raise HTTPException(status_code=exc.response.status_code, detail=detail) from exc
        except httpx.HTTPError as exc:
    ...
    try:
        async with httpx.AsyncClient(timeout=VIDEO_POLL_TIMEOUT) as client:
            ...
            if is_apimart:
                ...
                if is_volcengine and not volc_is_proxy:
                    ...
                    grok_video = "grok-imagine-video" in str(requested_model or payload.model or "").lower() or str((provider or {}).get("id") or "").strip().lower() == "sub2api"
                    image_limit = 7 if grok_video else 4
                    image_payload = []
                    grok_http_urls = []
                    first_url = ""
                    last_url = ""
                    for idx, ref in enumerate(payload.images[:image_limit]):
                        if not ref.url:
                            continue
                        role = str(getattr(ref, "role", "") or "").strip().lower()
                        raw_url = str(ref.url or "").strip()
                        http_url = ("http://127.0.0.1:3000" + raw_url) if raw_url.startswith("/assets/") else raw_url
                        if grok_video and http_url.startswith("http"):
                            grok_http_urls.append(http_url)
                            if idx == 0 or role in {"first_frame", "first"}:
                                first_url = first_url or http_url
                            if role in {"last_frame", "last"} or (idx == 1 and not last_url):
                                last_url = http_url
                        else:
                    ...
                    if grok_video and grok_http_urls:
                        body["image_url"] = first_url or grok_http_urls[0]
                        if last_url and last_url != body["image_url"]:
                            body["last_frame_url"] = last_url
                        body["image_urls"] = grok_http_urls[:7]
                        body["reference_image_urls"] = grok_http_urls[:7]
                    elif image_payload:
                        body["images"] = image_payload
                        if grok_video:
                            body["reference_images"] = image_payload
                            body["ref_images"] = image_payload
                    if payload.videos:
        ...
        if "channel not found" in text or "model_not_found" in text:
            hint = (
                ...
                f"原因：当前凭据或接口没有该模型的访问权限，或者上游尚未开放这个端点。\n\n"
                f"解决方法：\n"
                f"  1. 检查「API 设置」中的 Base URL、模型名称和凭据是否与服务文档一致；\n"
                f"  2. 或改用当前接口已公开的其他视频模型。"
            )
            raise HTTPException(status_code=exc.response.status_code, detail=hint) from exc
...
async def delete_conversation(conversation_id: str, request: Request, x_user_id: str = Header(default="")):
    user_id = safe_user_id(x_user_id, request)
    # Serialize the single-item route with chat saves and bulk deletion.  The
    # previous unlocked ``exists``/``remove`` pair could delete a conversation
    # that had just been recreated by another request after its existence check.
    # Holding the lock through the check and removal gives the operation a
    # deterministic boundary; a subsequent save intentionally recreates it.
    with CONVERSATION_LOCK:
        path = conversation_path(user_id, conversation_id)
        if os.path.exists(path):
            try:
                ...
                # Another process may have removed it; preserve the legacy
                # idempotent response rather than surfacing a spurious 500.
                pass
    return {"ok": True}
...
async def batch_delete_conversations(
    ...
    """批量删除当前用户的对话，不允许跨用户访问文件。

    Conversation IDs are validated before joining the user-specific directory.
    Optional media cleanup only removes generated-output files that have no
    remaining persisted reference; input uploads and shared files stay intact.
    """
    ids = _strict_batch_ids(payload.ids, "对话 ID")
    user_id = safe_user_id(x_user_id, request)
    changed = []
    skipped = []
    media_candidates = []
    ...
    with CONVERSATION_LOCK:
        # ``user_dir`` sanitises the user ID and creates only that directory;
        # never accept a caller-supplied path as a replacement.
        root = user_dir(user_id)
        for conversation_id in ids:
            path = os.path.join(root, f"{conversation_id}.json")
            if not os.path.isfile(path):
                skipped.append(_batch_skip(conversation_id, "not_found", "对话不存在"))
                continue
            try:
                with open(path, "r", encoding="utf-8") as handle:
                    conversation = json.load(handle)
            except (OSError, UnicodeError, json.JSONDecodeError):
                skipped.append(_batch_skip(conversation_id, "malformed_record", "对话数据无法读取"))
                continue
            if not isinstance(conversation, dict) or str(conversation.get("id") or "") != conversation_id:
                skipped.append(_batch_skip(conversation_id, "id_mismatch", "对话记录 ID 与文件名不一致"))
                continue
            if payload.remove_unreferenced_media:
                for url in collect_local_media_urls(conversation):
                    candidate = generated_media_path_from_url(url)
            ...
            try:
                ...
                skipped.append(_batch_skip(conversation_id, "remove_failed", f"删除失败：{exc}"))
                continue
    ...
    skipped_files = []
    removed_previews = 0
    if payload.remove_unreferenced_media and media_candidates:
        # The history index is not part of persisted_json_references_media_path
        # because generation-log cleanup prunes it first.  For conversation
        # deletion it may still pin a generated file, so check it explicitly.
        history_value = None
        if os.path.isfile(HISTORY_FILE):
            try:
                with open(HISTORY_FILE, "r", encoding="utf-8-sig") as handle:
        ...
        for path in media_candidates:
            referenced = persisted_json_references_media_path(path)
            if not referenced and history_value is not None:
                referenced = json_references_media_path(history_value, path)
            if referenced:
...
async def update_project(project_id: str, payload: ProjectUpdateRequest):
    # Hold the project lock across the complete read/modify/write transaction;
    # two settings tabs updating different projects must not overwrite each
    # other's changes with stale snapshots.
    with PROJECT_LOCK:
        projects = _ensure_default_project_unlocked()
...
async def delete_project(project_id: str):
    ...
    # Keep project removal and canvas reassignment in one ordered critical
    # section.  Persisting the project index with a plain ``open('w')`` before
    # moving canvases could leave a truncated index or orphaned project IDs on
    # a crash; every document now uses the atomic writer.
    with PROJECT_LOCK:
        projects = _ensure_default_project_unlocked()
        ...
        with CANVAS_LOCK:
            for filename in os.listdir(CANVAS_DIR):
                ...
                try:
                    with open(path, 'r', encoding='utf-8') as f:
                        data = json.load(f)
                    if not isinstance(data, dict) or str(data.get("project") or "") != project_id:
                        continue
                    # Refuse malformed filename/record pairs rather than
                    # writing a document to an unrelated normalized path.
                    record_id = str(data.get("id") or "")
                    if record_id != filename[:-5]:
                        continue
                    data["project"] = DEFAULT_PROJECT_ID
                    _write_canvas_locked(data)
                    moved += 1
                except (OSError, UnicodeError, json.JSONDecodeError, TypeError, ValueError, KeyError):
                    # A single damaged canvas must not abort deletion of the
                    # project or corrupt the remaining records.
                    continue
            _save_projects_unlocked(projects)
...
@app.post("/api/canvases/batch-delete")
async def batch_delete_canvases(payload: CanvasBatchDeleteRequest):
    """批量移入回收站、恢复或永久删除画布。

    The endpoint intentionally uses one explicit action instead of inferring
    intent from the current page.  ``trash`` and ``restore`` are reversible;
    ``purge`` is only accepted for records already in the recycle bin.  Every
    missing/malformed/otherwise inapplicable ID is returned in ``skipped`` so
    the client can reconcile selection state without guessing.
    """
    ids = _strict_batch_ids(payload.ids, "画布 ID")
    action = str(payload.action or "").strip().lower()
    if action not in {"trash", "restore", "purge"}:
        raise HTTPException(status_code=400, detail="action 必须是 trash、restore 或 purge")

    changed = []
    skipped = []
    changed_versions = {}

    # Keep the complete read/modify/write phase under the canvas lock.  The
    # lock is a regular threading.Lock, so do not call list_canvases() or
    # cleanup_expired_canvas_trash() while holding it (both may acquire it).
    with CANVAS_LOCK:
        for canvas_id in ids:
            path = os.path.join(CANVAS_DIR, f"{canvas_id}.json")
            if not os.path.isfile(path):
                skipped.append(_batch_skip(canvas_id, "not_found", "画布不存在"))
                continue
            try:
                with open(path, "r", encoding="utf-8") as handle:
                    canvas = json.load(handle)
            except (OSError, UnicodeError, json.JSONDecodeError):
                skipped.append(_batch_skip(canvas_id, "malformed_record", "画布数据无法读取"))
                continue
            if not isinstance(canvas, dict) or str(canvas.get("id") or "") != canvas_id:
                skipped.append(_batch_skip(canvas_id, "id_mismatch", "画布记录 ID 与文件名不一致"))
                continue

            deleted_at = bool(canvas.get("deleted_at"))
            if action == "trash":
                if deleted_at:
                    skipped.append(_batch_skip(canvas_id, "already_trashed", "画布已在回收站"))
                    continue
                canvas["deleted_at"] = now_ms()
                try:
                    _write_canvas_locked(canvas)
                except (OSError, TypeError, ValueError) as exc:
                    skipped.append(_batch_skip(canvas_id, "write_failed", f"写入失败：{exc}"))
                    continue
                changed.append(canvas_id)
                changed_versions[canvas_id] = int(canvas.get("updated_at") or 0)
                continue

            if action == "restore":
                if not deleted_at:
                    skipped.append(_batch_skip(canvas_id, "not_trashed", "画布不在回收站"))
                    continue
            ...
            # Permanent deletion is deliberately gated by the recycle-bin
            # marker.  This makes an accidental double-click unable to bypass
            # the reversible trash step while preserving an explicit purge UI.
            if not deleted_at:
                skipped.append(_batch_skip(canvas_id, "purge_requires_trash", "请先移入回收站再永久删除"))
                continue
            try:
                os.remove(path)
            except OSError as exc:
                skipped.append(_batch_skip(canvas_id, "remove_failed", f"删除失败：{exc}"))
                continue
            changed.append(canvas_id)

    # Let open canvas tabs refresh without making the filesystem mutation wait
    # on a slow/disconnected websocket.  A purge uses version 0 as a tombstone.
    for canvas_id in changed:
        try:
            await manager.broadcast_canvas_updated(canvas_id, changed_versions.get(canvas_id, 0), "")
        except Exception:
            pass

    visible = list_deleted_canvases() if action == "purge" else list_canvases()
    return {
        "ok": True,
        "action": action,
        "requested": ids,
...
async def update_canvas_meta(canvas_id: str, payload: CanvasMetaUpdate):
    ...
    The complete read/modify/write transaction is protected by ``CANVAS_LOCK``.
    Metadata is stored in the same JSON document as nodes/logs, so reading the
    record before taking the lock could let a concurrent full editor save be
    followed by a stale metadata snapshot and silently revert that save.
    """
    with CANVAS_LOCK:
        canvas = load_canvas(canvas_id)
        ...
        # Do not call save_canvas/_write_canvas_locked here: metadata changes
        # intentionally retain the content timestamp and list ordering.  Use
        # the same atomic persistence primitive, though, so a list refresh
        # cannot observe a truncated JSON document while this patch is being
        # written.
        _atomic_json_write(canvas_path(canvas["id"]), canvas)
...
async def touch_canvas(canvas_id: str):
    # Opening a canvas touches its timestamp.  Keep the read and monotonic
    # write in one transaction; otherwise an open-tab touch can serialize a
    # stale snapshot over a simultaneous editor save.
    with CANVAS_LOCK:
        canvas = load_canvas(canvas_id)
        _write_canvas_locked(canvas)
...
async def delete_canvas_log(canvas_id: str, payload: DeleteCanvasLogRequest):
    """Delete one generation-log entry and optionally clean its media.

    The record mutation is atomic under ``CANVAS_LOCK``.  Media deletion is
    deliberately a second, locked phase so other canvases/logs can be
    inspected after the target entry is removed without holding the lock
    ...
    def remove_log_record():
        with CANVAS_LOCK:
            ...
            logs = [item for item in (canvas.get("logs") or []) if isinstance(item, dict)]
            ...
            canvas["logs"] = [item for item in (canvas.get("logs") or []) if not (
                isinstance(item, dict) and str(item.get("id") or "") == log_id
            )]
            _write_canvas_locked(canvas)
    ...
    def cleanup_unreferenced_media():
        ...
        # History is an index rather than an owner; remove stale cards before
        # deleting the files so it cannot retain dead output forever.
...
async def upload_asset_library_workflows(
    ...
    # Read request bodies first.  UploadFile.read() yields to the event loop;
    # holding ASSET_LIBRARY_LOCK across that await would serialize unrelated
    # edits and could deadlock a concurrent mutation test.
    staged = []
...
async def create_prompt_library(payload: PromptLibraryRequest):
    # Keep the complete load -> mutate -> save transaction under the same
    # process lock.  ``load_prompt_libraries``/``save_prompt_libraries`` each
    # lock their individual I/O, but locking only those calls still lets two
    # tabs read the same snapshot and silently drop one another's changes.
    with PROMPT_LIBRARY_LOCK:
        ...
        for library in data.get("libraries", []) or []:
            ...
            for index, item in enumerate(library.get("items", []) or []):
                if item.get("id") == item_id:
                    ...
                    data = save_prompt_libraries(data)
                    result = {"library": public_prompt_libraries(data), "item": next_item}
                    break
            else:
                continue
            break
...
async def batch_delete_prompt_library_items(payload: PromptLibraryBatchDeleteRequest):
    ids = _strict_batch_ids(payload.ids, "提示词 ID")
    removed_ids = []
    with PROMPT_LIBRARY_LOCK:
        data = load_prompt_libraries()
        for library in data.get("libraries", []) or []:
            ...
            library["items"] = keep
        data = save_prompt_libraries(data)
    seen_removed = set(removed_ids)
    skipped = [
        _batch_skip(item_id, "not_found", "提示词不存在")
        for item_id in ids
        if item_id not in seen_removed
    ]
    return {
...
async def delete_asset_library_category(category_id: str, library_id: str = ""):
    # Hold the index lock only for the load/mutate/save transaction.  File
    # cleanup below performs potentially slow reference scans and must not
    # block unrelated library edits while it is running.
    with ASSET_LIBRARY_LOCK:
        lib = load_asset_library()
        ...
        # 删除分组时先更新索引，再逐个清理文件。这样引用扫描不会把待删记录
        # 本身当成引用，也不会误删仍被画布/会话/历史使用的共享副本。
        removed_items = list(cat.get("items") or [])
        cat_dir = str(cat.get("dir") or "").strip("/").strip()
        category_dir_path = ""
        if cat_dir:
            try:
                target = os.path.abspath(os.path.join(ASSET_LIBRARY_DIR, cat_dir))
                library_root = os.path.abspath(ASSET_LIBRARY_DIR)
                if target.startswith(library_root + os.sep):
                    category_dir_path = target
            except Exception as exc:
                print(f"解析分组文件夹失败: {exc}")
        library["categories"] = [c for c in library.get("categories", []) if c.get("id") != category_id]
        lib = save_asset_library(lib)
    cleanup = {"deleted": 0, "skipped": 0}
    for item in removed_items:
        status = cleanup_asset_library_file_if_unreferenced(item)
        if status == "deleted":
            cleanup["deleted"] += 1
        elif status != "not_found":
            cleanup["skipped"] += 1
    # 文件清理完成后再尝试移除目录。 只使用 rmdir，目录中如果存在被
    # 引用的副本、未知文件或并发写入，就保留目录而不递归删除任何内容。
    if category_dir_path:
        try:
            if os.path.isdir(category_dir_path):
                os.rmdir(category_dir_path)
        except OSError:
            pass
...
async def add_asset_library_item(payload: AssetLibraryAddRequest):
    # Resolve the source/category before doing any potentially slow image
    # classification, but do not retain the library lock across that await.
    # The final append below reloads the latest index and is the authoritative
    # transaction, so a concurrent rename/delete cannot be overwritten by a
    # stale snapshot.
    lib = load_asset_library()
    ...
    with ASSET_LIBRARY_LOCK:
        ...
        latest_cat = find_asset_category_in_library(latest, payload.category_id, payload.library_id)
        if not latest_cat:
            # The destination category may have been deleted while the
            # classifier was running.  Remove only the newly copied file; it
            # is not part of any persisted document yet.
            cleanup_asset_library_file_if_unreferenced(item)
            raise HTTPException(status_code=404, detail="分类已被删除")
        if latest_cat.get("type") != "image":
            cleanup_asset_library_file_if_unreferenced(item)
            raise HTTPException(status_code=400, detail="该分类暂不支持添加媒体")
        latest_cat.setdefault("items", []).append(item)
        lib = save_asset_library(latest)
    return {"library": lib, "item": item}
...
async def batch_add_asset_library_items(payload: AssetLibraryBatchAddRequest):
    ...
    for entry in (payload.items or [])[:200]:
        ...
        if not src:
            continue
        _, item = make_asset_library_item(src, entry.name or os.path.basename(src), subdir=category_dir)
        if item.get("kind") == "image":
            classification = await classify_asset_image_best_effort(output_file_from_url(item.get("url") or "") or src)
            if classification:
                item["classification"] = classification
        added.append(item)
    # Classification above intentionally runs without the index lock.  Commit
    # every completed copy in one short transaction against the latest index;
    # this prevents a stale batch from resurrecting concurrent edits.
    with ASSET_LIBRARY_LOCK:
        latest = load_asset_library()
        latest_cat = find_asset_category_in_library(latest, payload.category_id, payload.library_id)
        if not latest_cat:
        ...
        if latest_cat.get("type") != "image":
            for item in added:
                cleanup_asset_library_file_if_unreferenced(item)
            raise HTTPException(status_code=400, detail="该分类暂不支持添加媒体")
        latest_cat.setdefault("items", []).extend(added)
        lib = save_asset_library(latest)
    return {"library": lib, "items": added}
...
async def import_shared_folder_files(payload: SharedFolderImport):
    ...
    for rel in (payload.paths or [])[:200]:
        ...
        if ext not in SHARED_MEDIA_EXTS:
            continue
        # Copy/classify outside the index lock; classification may await a
        # remote provider.  The staged records are committed against a fresh
        # category snapshot below.
        _, item = make_asset_library_item(abs_path, os.path.basename(abs_path), subdir=category_dir)
        if item.get("kind") == "image":
...
async def classify_asset_library_items(payload: AssetLibraryClassifyRequest):
    # Read the candidate snapshot once, then perform provider calls without
    # holding ASSET_LIBRARY_LOCK.  Results are merged into a fresh latest
    # snapshot below so concurrent renames/moves/deletes are preserved.
    lib = load_asset_library()
    results = []
    classifications = []
    for item_id in (payload.ids or [])[:80]:
        ...
        try:
            ...
            classifications.append((str(item_id), classification))
    ...
    if classifications:
        with ASSET_LIBRARY_LOCK:
            ...
            for item_id, classification in classifications:
                target_item = find_asset_item_in_library(latest, item_id, payload.library_id)
                if target_item is None:
                    # The item can legitimately disappear while an upstream
                    # classifier is running.  Keep the per-request result but
                    # do not recreate a deleted record.
                    continue
...
async def register_asset_library_avatar(item_id: str, payload: AssetAvatarRegisterRequest):
    ...
    # The upstream submission may take several seconds.  Merge its result
    # into the latest library snapshot instead of saving the pre-await copy.
    with ASSET_LIBRARY_LOCK:
        latest = load_asset_library()
        latest_item = find_asset_item_in_library(latest, item_id, payload.library_id)
        if not latest_item:
            raise HTTPException(status_code=404, detail="资产已被删除")
        regs = latest_item.get("registrations")
        if not isinstance(regs, dict):
            regs = {}
        regs[platform] = {
            "provider_id": provider["id"],
        ...
        latest_item["registrations"] = regs
        lib = save_asset_library(latest)
        target_item = latest_item
...
async def check_asset_library_avatar(item_id: str, payload: AssetAvatarRegisterRequest):
    ...
    if platform == "apimart":
        ...
        raise HTTPException(status_code=400, detail="该平台的认证后端尚未接入。")
    # Merge status into the latest item after the remote poll.  Do not hold a
    # filesystem lock while awaiting the provider response.
    with ASSET_LIBRARY_LOCK:
        latest = load_asset_library()
        latest_item = find_asset_item_in_library(latest, item_id, payload.library_id)
        if not latest_item:
            raise HTTPException(status_code=404, detail="资产已被删除")
        latest_regs = latest_item.get("registrations") if isinstance(latest_item.get("registrations"), dict) else {}
        latest_reg = latest_regs.get(platform) if isinstance(latest_regs.get(platform), dict) else {}
        latest_reg["status"] = result["status"]
        latest_reg["detail"] = result.get("detail") or ""
        if result["status"] == "Active" and result.get("asset_uri"):
            latest_reg["asset_uri"] = result["asset_uri"]
            latest_reg["asset_id"] = result["asset_uri"].replace("asset://", "")
        latest_regs[platform] = latest_reg
        latest_item["registrations"] = latest_regs
        lib = save_asset_library(latest)
        target_item = latest_item
    return {"library": lib, "item": target_item}
...
async def delete_asset_library_item(item_id: str):
    # Persist the index mutation atomically, then release the lock before the
    # reference scan/file deletion.  This closes the lost-update window while
    # keeping slow cleanup out of the critical section.
    with ASSET_LIBRARY_LOCK:
        lib = load_asset_library()
        removed = None
        for library in lib.get("libraries", []):
        ...
        if not removed:
            raise HTTPException(status_code=404, detail="资产不存在")
        # 先持久化索引，再检查画布/会话/历史中的引用；否则会把仍在使用的
        # 共享副本误删。批量删除接口采用同样的引用保护策略。
        lib = save_asset_library(lib)
    cleanup_status = cleanup_asset_library_file_if_unreferenced(removed)
    return {"library": lib, "file_cleanup": cleanup_status}

@app.post("/api/asset-library/items/delete")
async def batch_delete_asset_library_items(payload: AssetLibraryBatchDeleteRequest):
    ids = _strict_batch_ids(payload.ids, "资产 ID")
    removed_items = []
    found_ids = set()

    # Mutate the JSON index atomically.  File cleanup happens after the index
    # is persisted so reference scans see the post-delete state.
    with ASSET_LIBRARY_LOCK:
        lib = load_asset_library()
        for library in lib.get("libraries", []):
            if payload.library_id and library.get("id") != payload.library_id:
    ...
    skipped = [
        _batch_skip(item_id, "not_found", "资产不存在或不在当前资产库")
        for item_id in ids
        if item_id not in found_ids
    ]
    removed_files = []
    skipped_files = []
    # A single physical library file can be referenced by multiple records or
    # by a canvas/conversation.  Only remove it after all persisted references
    # are gone; remote URLs have no local file to clean.
    candidate_paths = []
    for item in removed_items:
        path = output_file_from_url(item.get("url") if isinstance(item, dict) else "")
        if path and path not in candidate_paths:
            candidate_paths.append(path)
    ...
    if os.path.isfile(HISTORY_FILE):
        try:
            with open(HISTORY_FILE, "r", encoding="utf-8-sig") as handle:
                remaining_history = json.load(handle)
        except (OSError, UnicodeError, json.JSONDecodeError):
            remaining_history = None
    for path in candidate_paths:
        if not os.path.isfile(path):
            skipped_files.append({"path": os.path.basename(path), "reason": "not_found"})
            continue
        referenced = persisted_json_references_media_path(path)
        if not referenced and remaining_history is not None:
            referenced = json_references_media_path(remaining_history, path)
        if referenced:
            skipped_files.append({"path": os.path.basename(path), "reason": "referenced"})
            continue
        try:
            delete_media_preview_cache(path)
            os.remove(path)
            ...
            skipped_files.append({"path": os.path.basename(path), "reason": "remove_failed", "detail": str(exc)})

    try:
        await manager.broadcast_asset_library_updated(int(lib.get("updated_at") or now_ms()))
    except Exception:
        pass
    return {
        "library": lib,
        ...
        "removed": len(removed_items),
        "removed_count": len(removed_items),
        "removed_ids": [str(item.get("id")) for item in removed_items],
        "deleted": [str(item.get("id")) for item in removed_items],
        "deleted_count": len(removed_items),
        "skipped": skipped,
        "skipped_count": len(skipped),
        "removed_files": removed_files,
...
async def batch_crop_asset_library_items(payload: AssetLibraryBatchCropRequest):
    ...
    if not ids:
        raise HTTPException(status_code=400, detail="没有选择资产")
    # Cropping is synchronous and can safely use the same short-lived lock as
    # the other index mutations.  Keeping it here also makes a concurrent
    # rename/delete deterministic instead of allowing a stale save to win.
    with ASSET_LIBRARY_LOCK:
        lib = load_asset_library()
        target_cat = None
...
async def update_canvas(canvas_id: str, payload: CanvasSaveRequest):
    # The canvas JSON is a single document containing metadata, nodes,
    # connections and generation logs.  Read and write it under one lock so a
    # concurrent metadata patch cannot interleave a stale read with this full
    # replacement.  Besides preventing lost updates, this also prevents a
    # reader from observing the transient empty/truncated file produced by a
    # direct JSON write.
    with CANVAS_LOCK:
        canvas = load_canvas(canvas_id)
        current_updated_at = int(canvas.get("updated_at") or 0)
        ...
        # Preserve settings when a legacy client omits the field.  An explicit
        # object (including ``{}``) still replaces the saved settings by design.
...
async def delete_canvas(canvas_id: str):
    # Soft-delete is a mutation of the same JSON document used by the editor.
    # Serialize the snapshot and write so a concurrent PUT cannot resurrect
    # an older document or lose nodes/logs.  Notify open tabs only after the
    # filesystem lock is released.
    changed = False
    with CANVAS_LOCK:
    ...
    if changed:
        try:
            await manager.broadcast_canvas_updated(
                canvas_id, int(canvas.get("updated_at") or now_ms()), ""
            )
        except Exception:
            pass
    return {"ok": True}
...
async def purge_canvas(canvas_id: str):
    """Permanently remove a canvas that is already in the recycle bin.

    The list UI only exposes purge for trashed records, but this route is also
    callable directly.  Keep the single-item route aligned with the guarded
    batch endpoint so a stale/accidental request cannot bypass the reversible
    trash step.  The filesystem check and marker check live under the canvas
    lock so a concurrent save/delete cannot race the decision.
    """
    removed = False
    with CANVAS_LOCK:
        path = canvas_path(canvas_id)
        if not os.path.exists(path):
            # Preserve the legacy idempotent response for a record that was
            # already purged by another tab/process.
            return {"ok": True, "removed": False}
        try:
            with open(path, "r", encoding="utf-8") as handle:
                canvas = json.load(handle)
        except (OSError, UnicodeError, json.JSONDecodeError) as exc:
            raise HTTPException(status_code=409, detail="画布数据无法读取，未执行永久删除") from exc
        if not isinstance(canvas, dict) or str(canvas.get("id") or "") != str(canvas_id):
            raise HTTPException(status_code=409, detail="画布记录无效，未执行永久删除")
        if not canvas.get("deleted_at"):
            raise HTTPException(status_code=400, detail="请先移入回收站再永久删除")
        try:
            os.remove(path)
        except OSError as exc:
    ...
    if removed:
        try:
            await manager.broadcast_canvas_updated(canvas_id, 0, "")
        except Exception:
            # A disconnected websocket must not turn a successful filesystem
            # deletion into a failed API response.
            pass
...
async def get_history_api(type: str = None):
    # Keep the complete read/filter/sort snapshot under HISTORY_LOCK so a
    # concurrent save or delete can never expose a half-written document.
    # ``utf-8-sig`` accepts both the current UTF-8 format and legacy exports
    # that include a BOM.  Malformed/non-list data is deliberately read-only:
    # callers receive an empty result while the original file remains intact.
    with HISTORY_LOCK:
        ...
        try:
            ...
            return []
        if not isinstance(loaded, list):
            print("读取历史文件失败（保留原文件）: history is not a list")
            return []

        # Ignore malformed scalar entries rather than allowing one legacy
        # record to make the entire endpoint fail with ``item.get``.
        ...
        data = [
            item
            for item in data
            if isinstance(item.get("images"), (list, tuple))
    ...
    # History may contain a legacy provider's raw response or a signed media
    # URL.  Sanitize at read time so old records cannot bypass the public
    # provider/runtime boundary.  Do this after releasing the lock because it
    # only works on the in-memory snapshot and can recurse through large raw
    # provider payloads.
    safe_data = sanitize_public_runtime_value(data)
    return safe_data if isinstance(safe_data, list) else []


@app.post("/api/history/batch-delete")
async def batch_delete_history(req: HistoryBatchDeleteRequest):
    """批量删除历史记录，并在安全前提下回收其生成文件。"""
    timestamps = _strict_batch_timestamps(req.timestamps)
    matched = []
    skipped = []
    target_records = []
    remaining = []

    def matches(item_ts, target):
        if isinstance(item_ts, (int, float)) and not isinstance(item_ts, bool):
            try:
                return abs(float(item_ts) - float(target)) < 0.001
            except (TypeError, ValueError):
                return False
        return str(item_ts) == str(target)
    ...
    with HISTORY_LOCK:
        ...
        if os.path.isfile(HISTORY_FILE):
            try:
                ...
                # A malformed history file is not safe to overwrite.  Report
                # all requested records as skipped and leave it untouched.
                skipped = [_batch_skip(ts, "malformed_history", "历史文件无法读取") for ts in timestamps]
                return {
                    "ok": True,
        ...
        if not history:
            skipped = [_batch_skip(ts, "not_found", "历史记录不存在") for ts in timestamps]
        else:
            requested_hits = {round(ts, 3): False for ts in timestamps}
            for item in history:
                item_ts = item.get("timestamp", 0) if isinstance(item, dict) else 0
                matched_target = next((ts for ts in timestamps if matches(item_ts, ts)), None)
                if matched_target is None:
                    remaining.append(item)
                    continue
                target_records.append(item)
                requested_hits[round(matched_target, 3)] = True
            for ts in timestamps:
                if requested_hits.get(round(ts, 3)):
                    matched.append(ts)
                else:
                    skipped.append(_batch_skip(ts, "not_found", "历史记录不存在"))

            if target_records:
                try:
                    # Atomic replacement in the same directory prevents a
                    # crash during a large batch from leaving a truncated
                    # history file.  Keep the failure response stable, just as
                    # the single-record endpoint does, instead of surfacing a
                    # filesystem traceback as a 500.
                    _atomic_json_write(HISTORY_FILE, remaining)
                except (OSError, TypeError, ValueError) as exc:
                    print(f"Batch delete history write failed (original preserved): {exc}")
                    skipped = [
                        _batch_skip(ts, "write_failed", "历史文件无法保存")
                        for ts in timestamps
                    ]
                    return {
                        "ok": True,
    ...
    skipped_files = []
    removed_previews = 0
    if req.remove_unreferenced_media and target_records:
        candidates = []
        for record in target_records:
            for url in collect_local_media_urls(record):
                path = generated_media_path_from_url(url)
                if path and path not in candidates:
                    candidates.append(path)
        # Re-read the history index while holding its lock.  A concurrent
        # generation may have added a reference after this batch rewrote the
        # file; checking the stale ``remaining`` list alone could then delete
        # media still visible in the newly-created history card.
        with HISTORY_LOCK:
            ...
            if os.path.isfile(HISTORY_FILE):
                try:
                    with open(HISTORY_FILE, "r", encoding="utf-8-sig") as handle:
                        loaded_current = json.load(handle)
                    current_history = loaded_current if isinstance(loaded_current, list) else None
                    history_unreadable = current_history is None
                except (OSError, UnicodeError, json.JSONDecodeError, TypeError, ValueError):
                    history_unreadable = True
            for path in candidates:
                # ``persisted_json_references_media_path`` scans canvases,
                # conversations and the asset library.  The fresh history
                # snapshot covers the fourth editable surface.
                if (
                    history_unreadable
                    or persisted_json_references_media_path(path)
                    or json_references_media_path(current_history, path)
                ):
                    skipped_files.append(os.path.basename(path))
                    continue
                try:
...
async def delete_history(req: DeleteHistoryRequest):
    # Keep the complete read/modify/write transaction under one lock.  The
    # previous route parsed and rewrote the file with ``open(..., 'w')`` and
    # then exposed the exception text to callers.  A malformed file could be
    # replaced by partial/empty JSON, while concurrent saves could be lost.
    # Returning stable messages also keeps the public API from leaking local
    ...
    with HISTORY_LOCK:
        ...
        try:
            ...
            print(f"Delete history read failed (original preserved): {exc}")
            return {"success": False, "message": "History file unreadable"}

        # A JSON object/string is not a valid history index.  Do not coerce it
        # to an empty list: doing so would silently destroy the user's data.
        if not isinstance(loaded, list):
            print("Delete history read failed (original preserved): history is not a list")
            return {"success": False, "message": "History file unreadable"}

        def matches_timestamp(item_timestamp: Any, target_timestamp: Any) -> bool:
            if (
                isinstance(target_timestamp, (int, float))
                and not isinstance(target_timestamp, bool)
                and isinstance(item_timestamp, (int, float))
                and not isinstance(item_timestamp, bool)
            ):
                try:
                    return abs(float(item_timestamp) - float(target_timestamp)) < 0.001
                except (TypeError, ValueError, OverflowError):
                    return False
            return str(item_timestamp) == str(target_timestamp)

        target_records = []
        new_history = []
        for item in loaded:
            # Keep legacy/scalar entries intact when deleting a valid record;
            # they should not make the whole operation fail.
            if not isinstance(item, dict):
                new_history.append(item)
                continue
            item_timestamp = item.get("timestamp", 0)
            if matches_timestamp(item_timestamp, req.timestamp):
                target_records.append(item)
            else:
                new_history.append(item)
        ...
        try:
            # Atomic replacement guarantees readers observe either the old
            # complete history or the new complete history, never a truncated
            # document.  The helper also cleans up its sibling temp file when
            # a write/replace fails, preserving the old index.
            _atomic_json_write(HISTORY_FILE, new_history)
        except Exception as exc:
            print(f"Delete history write failed (original preserved): {exc}")
            return {"success": False, "message": "History file could not be saved"}

        # Remove only generated-output media and only after checking every
        # persisted editable surface.  ``new_history`` covers the history
        # index itself; canvases, conversations and the asset library are
        # scanned by persisted_json_references_media_path().  Keep the history
        # lock while deleting so a concurrent save cannot add a reference
        # between the check and os.remove().
        removed_files = []
        skipped_files = []
        removed_previews = 0
        candidates = []
        ...
        if candidates:
            # Serialize physical deletion with the storage-files bulk route.
            # Lock order is HISTORY_LOCK -> STORAGE_DELETE_LOCK here; neither
            # lock is acquired in the opposite order by history persistence.
            with STORAGE_DELETE_LOCK:
                for path in candidates:
                    try:
                        referenced = persisted_json_references_media_path(path)
                    except Exception as exc:
                        # A scan failure is a conservative live reference.
                        print(f"History media reference scan failed for {path}: {exc}")
                        referenced = True
                    if not referenced and json_references_media_path(new_history, path):
                        referenced = True
                    if referenced:
                        skipped_files.append(os.path.basename(path))
                        continue
                    try:
                        ...
                        # History deletion itself succeeded; a failed cache or
                        # file cleanup is reported without turning the request
                        # into a misleading 500 response.
                        print(f"Failed to delete history media {path}: {exc}")
                        skipped_files.append(os.path.basename(path))
        ...
        async with httpx.AsyncClient(timeout=30) as client:
            ...
            for i in range(300):
                ...
                if status == "SUCCEED":
                    ...
                    if req.client_id:
                        await manager.send_personal_message({"type": "cloud_status", "status": "SUCCEED", "task_id": task_id}, req.client_id)
                    _schedule_background(manager.broadcast_new_image(record))
...
def save_runninghub_workflow_store(store):
    ...
    if not isinstance(store, dict):
        store = {}
    _atomic_json_write(RUNNINGHUB_WORKFLOW_STORE_FILE, store)
...
def runninghub_workflow_store_key(workflow_id: str) -> str:
    ...
    # A disabled legacy vendor must not be smuggled back in through an
    # imported workflow ID or a later store merge.
    if not value or len(value) > 128:
        return ""
    # Workflow IDs are opaque identifiers, not URLs or query strings.  Reject
    # control/URL/assignment forms so a signed object URL or an inline API key
    # cannot become a persistent store key and later reappear in list/detail
    # responses.  Keep the historical slash support for legacy IDs.
    if not re.fullmatch(r"[A-Za-z0-9][A-Za-z0-9._:/-]{0,127}", value):
        return ""
    if ".." in value or re.search(r"[\x00-\x20?#\\;%|=]", value) or "://" in value:
        return ""
    if _contains_open_source_blocked_text(value) or _redact_public_string(value) != value:
        return ""
    return value
...
@app.post("/api/workflows/batch-delete")
def batch_delete_workflows(payload: WorkflowBatchDeleteRequest):
    """批量删除用户工作流及其可选配置文件。

    Built-in workflows are never removed; they are returned as ``skipped`` so
    a settings UI can clear its selection without treating protection as an
    unexpected server error.
    """
    names = payload.names
    if not isinstance(names, list):
        raise HTTPException(status_code=400, detail="工作流名称列表格式不正确")
    if not names:
        raise HTTPException(status_code=400, detail="没有选择工作流")
    if len(names) > BATCH_DELETE_MAX_ITEMS:
        raise HTTPException(status_code=400, detail=f"单次最多选择 {BATCH_DELETE_MAX_ITEMS} 个工作流")
    invalid = []
    clean_names = []
    seen = set()
    for raw in names:
        if not isinstance(raw, str) or raw != raw.strip() or not WORKFLOW_NAME_RE.fullmatch(raw):
            invalid.append(str(raw))
            continue
        # Only names accepted by workflow_path_from_name may reach the disk;
        # explicitly reject dot segments even if a future regex is relaxed.
    ...
    if invalid:
        raise HTTPException(status_code=400, detail={"message": "包含非法工作流名称", "invalid": invalid[:50]})
    if not clean_names:
        raise HTTPException(status_code=400, detail="没有选择工作流")
    ...
    with WORKFLOW_FILE_LOCK:
        for name in clean_names:
            if is_builtin_workflow(name):
                skipped.append(_batch_skip(name, "builtin", "内置工作流不可删除"))
                continue
            try:
                workflow_path = workflow_path_from_name(name)
                cfg_path = workflow_config_path(name)
            except HTTPException:
                skipped.append(_batch_skip(name, "invalid_name", "工作流名称不合法"))
                continue
            if not os.path.isfile(workflow_path):
                skipped.append(_batch_skip(name, "not_found", "工作流不存在"))
                continue
            try:
                ...
                skipped.append(_batch_skip(name, "remove_failed", f"删除失败：{exc}"))
    return {
        "ok": True,
        ...
        "skipped": skipped,
        "skipped_count": len(skipped),
        "workflows": list_workflows().get("workflows", []),
    }
...
def get_workflow(name: str):
    ...
    try:
        ...
        # A damaged user workflow should be recoverable from the editor, not
        # surface as an opaque server traceback.  Keep the record untouched so
        # the user can replace/export it explicitly.
        raise HTTPException(status_code=409, detail="工作流 JSON 无法读取") from exc
    if not isinstance(workflow, dict):
        raise HTTPException(status_code=409, detail="工作流 JSON 格式无效")
    ...
    if os.path.exists(cfg_path):
        try:
            ...
            if isinstance(loaded_cfg, dict):
                cfg = loaded_cfg or cfg
        except (OSError, UnicodeError, json.JSONDecodeError, TypeError, ValueError):
            # Keep the valid workflow response even if its optional UI config
            # sidecar is damaged; the caller can overwrite the sidecar.
            pass
...
def upload_workflow(payload: WorkflowUploadRequest):
    ...
    with WORKFLOW_FILE_LOCK:
        ...
        _atomic_json_write(path, payload.workflow)
...
def save_workflow_config(name: str, payload: WorkflowConfig):
    ...
    with WORKFLOW_FILE_LOCK:
        _atomic_json_write(cfg_path, config)
