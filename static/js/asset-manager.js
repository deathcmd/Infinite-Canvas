...
let sharedFolders = [];
let sharedFoldersLoadGeneration = 0;
let sharedFoldersLoadController = null;
...
let localAssetsLoadGeneration = 0;
let localAssetsLoadController = null;
...
// Refreshes are cancellable and generation guarded.  A slow response from an
// earlier click must never replace the library/context loaded by the latest
// refresh (this is especially easy to trigger on a large local asset tree).
let loadAllGeneration = 0;
let loadAllController = null;
let canvasAssetsGeneration = 0;
...
let storageLoadController = null;
let storageSettingsGeneration = 0;
let storageSettingsController = null;
...
// Keep this in sync with the server-side batch endpoint limit.  Checking on
// the client avoids an opaque 400 after the user has spent time selecting a
// large set and makes it clear that the operation should be split.
...
function apiErrorMessage(payload, fallback='操作失败'){
...
function batchSelectionWithinLimit(ids, noun='项'){
    const values = [...new Set((ids || []).map(id => String(id || '')).filter(Boolean))];
    if(values.length > MAX_BATCH_ACTIONS){
        setStatus(`单次最多处理 ${MAX_BATCH_ACTIONS} 个${noun}，请分批操作`);
        return null;
    }
    return values;
}
function enhanceAssetA11y(scope=root){
    if(!scope?.querySelectorAll) return;
    // Dynamic templates contain many icon-only buttons.  Their title remains
    // useful as a tooltip; mirror it to aria-label so keyboard/screen-reader
    // users get the same action name without duplicating every template.
...
async function apiJson(url, options={}){
    ...
    try {
        ...
        const networkError = new Error('网络请求失败，请检查本地服务是否仍在运行');
        networkError.cause = err;
        throw networkError;
    }
    const data = await res.json().catch(() => ({}));
    if(!res.ok){
        const error = new Error(apiErrorMessage(data, `请求失败（${res.status || '未知状态'}）`));
...
function batchMutationCounts(data, requestedCount=0){
    const payload = data && typeof data === 'object' ? data : {};
    const removedArray = Array.isArray(payload.removed) ? payload.removed : null;
    const deletedArray = Array.isArray(payload.deleted) ? payload.deleted : null;
    const numericValue = payload.removed_count ?? payload.deleted_count
        ?? (typeof payload.removed === 'number' ? payload.removed : null)
        ?? (typeof payload.deleted === 'number' ? payload.deleted : null);
    const removed = Number(numericValue);
    const count = Number.isFinite(removed)
        ? Math.max(0, removed)
        : (removedArray ? removedArray.length : (deletedArray ? deletedArray.length : 0));
    const skipped = Array.isArray(payload.skipped) ? payload.skipped : [];
    return { removed: count, skipped, skippedCount: skipped.length };
}
function batchMutationStatus(data, requestedCount, noun='项'){
    const result = batchMutationCounts(data, requestedCount);
    if(result.skippedCount){
        return `已处理 ${result.removed} 个${noun}，${result.skippedCount} 个未删除（仍被引用或已不存在）`;
    }
    return `已删除 ${result.removed} 个${noun}`;
}
const STORAGE_KIND_LABELS = {upload:'上传素材', generated:'生成素材', local:'本地素材'};
async function openStorageSettings(){
    const generation = ++storageSettingsGeneration;
    if(storageSettingsController?.abort){
        try { storageSettingsController.abort(); } catch(_) {}
    }
    let controller = null;
    try { controller = typeof AbortController === 'function' ? new AbortController() : null; } catch(_) {}
    storageSettingsController = controller;
    storageSettingsState.open = true;
    storageSettingsState.selected = new Set();
    renderStorageSettingsModal();
    try {
        const requestOptions = controller?.signal ? {signal:controller.signal} : {};
        const [data, promptData] = await Promise.all([
            apiJson('/api/storage-settings', requestOptions),
            apiJson('/api/asset-classification-prompt', requestOptions)
        ]);
        if(generation !== storageSettingsGeneration || !storageSettingsState.open) return {stale:true};
        ...
        if(generation !== storageSettingsGeneration || isAbortError(err)) return {stale:true};
        setStatus(err.message || '加载存储设置失败');
        renderStorageSettingsModal();
    } finally {
        if(storageSettingsController === controller) storageSettingsController = null;
    }
}
function closeStorageSettings(){
    storageSettingsGeneration += 1;
    if(storageSettingsController?.abort){
        try { storageSettingsController.abort(); } catch(_) {}
    }
    storageLoadGeneration += 1;
    if(storageLoadController?.abort){
        try { storageLoadController.abort(); } catch(_) {}
    }
...
async function loadStorageFiles(kind, options={}){
    ...
    // A pagination request must not overlap another request.  A fresh kind
    // switch, however, supersedes an older fresh request (for example when a
    // user clicks generated → upload quickly), so abort that request and let
    // the generation guard discard any late response.
    if(append && (storageSettingsState.loading || storageSettingsState.loadingMore)) return {busy:true};
    if(append && !storageSettingsState.hasMore) return {done:true};
    if(!append && storageSettingsState.loading && storageSettingsState.kind === nextKind) return {busy:true};
    ...
    let controller = null;
    try { controller = typeof AbortController === 'function' ? new AbortController() : null; } catch(_) {}
    storageLoadController = controller;
    ...
    const previousState = {
        ...
        items:storageSettingsState.items,
        offset:storageSettingsState.offset,
        total:storageSettingsState.total,
        hasMore:storageSettingsState.hasMore,
        selected:storageSettingsState.selected
    };
    ...
    try {
        ...
        const data = await apiJson(`/api/storage-files?kind=${encodeURIComponent(nextKind)}&offset=${encodeURIComponent(offset)}&limit=${encodeURIComponent(limit)}`, controller?.signal ? {signal:controller.signal} : {});
        if(generation !== storageLoadGeneration) return {stale:true};
        ...
        if(isAbortError(err) || generation !== storageLoadGeneration) return {stale:true};
        // Keep the last good page visible when a refresh fails.  Clearing a
        // populated grid on a transient network error is both confusing and
        // makes it easy to delete the wrong page after retrying.
        if(!append && generation === storageLoadGeneration){
            // If this was a kind switch, restoring the previous kind keeps the
            // visible cards and tab semantically aligned.  Showing generated
            // cards while the upload tab is active is more dangerous than a
            // short-lived error message because the next delete click could
            // target the wrong directory.
            storageSettingsState.kind = previousState.kind || nextKind;
            storageSettingsState.items = previousState.items;
            storageSettingsState.offset = previousState.offset;
            storageSettingsState.total = previousState.total;
            storageSettingsState.hasMore = previousState.hasMore;
            storageSettingsState.selected = previousState.selected;
        }
        throw err;
    } finally {
        ...
        if(storageLoadController === controller) storageLoadController = null;
    }
}
...
function renderStorageSettingsModal(){
    ...
    const cards = storageSettingsState.loading
        ...
        ? storageSettingsState.items.map(item => `
            <label class="storage-file-card ${storageSettingsState.selected.has(item.id) ? 'selected' : ''}">
                <input type="checkbox" data-storage-file="${escapeAttr(item.id)}" data-storage-rel="${escapeAttr(item.rel)}" aria-label="选择文件：${escapeAttr(item.name || item.rel || '未命名文件')}" ${storageSettingsState.selected.has(item.id) ? 'checked' : ''}>
    ...
    const manageBody = `
        ...
        <div class="storage-file-grid" data-storage-file-grid aria-busy="${storageSettingsState.loading || storageSettingsState.loadingMore ? 'true' : 'false'}">${cards}</div>
    `;
    overlay.innerHTML = `
        <div class="storage-settings-modal" role="dialog" aria-modal="true" aria-labelledby="storageSettingsTitle">
            <div class="storage-settings-head">
                ...
                <button type="button" data-storage-close title="关闭偏好设置" aria-label="关闭偏好设置"><i data-lucide="x"></i></button>
            </div>
...
async function deleteSelectedStorageFiles(){
    ...
    const requestKind = storageSettingsState.kind;
    const selectedRels = selected.map(item => item.rel);
    try {
        const data = await apiJson('/api/storage-files/delete', {
            method:'POST',
            headers:{'Content-Type':'application/json'},
            body:JSON.stringify({kind:requestKind, items:selectedRels})
        });
        // loadStorageFiles intentionally clears selection for a fresh page.
        // Capture skipped paths before reloading, then restore only those
        // entries which the server explicitly kept (for example, referenced
        // files).  This prevents a failed/guarded delete from looking like a
        // successful destructive action.
        const skippedRels = new Set((Array.isArray(data?.skipped) ? data.skipped : [])
            .map(item => String(item?.id ?? item?.path ?? item ?? '')));
        // The user may have switched the storage tab while the delete was in
        // flight.  Do not kick off a reload for the old kind (which would
        // abort the newer tab's request or move the UI back unexpectedly).
        if(storageSettingsState.kind !== requestKind) return;
        const refreshResult = await loadStorageFiles(requestKind);
        if(storageSettingsState.kind !== requestKind || refreshResult?.stale || refreshResult?.busy) return;
        storageSettingsState.selected = new Set(storageSettingsState.items
            .filter(item => skippedRels.has(String(item.rel)))
            .map(item => item.id));
        renderStorageSettingsModal();
        setStatus(batchMutationStatus(data, selectedRels.length, '文件'));
    } catch(err) {
        if(isAbortError(err)) return {stale:true};
        setStatus(err.message || '删除文件失败');
...
function renderLocalCaptionTools(imageCount){
    ...
    return `
        <div class="local-caption-tools">
            ...
            <select id="localCaptionProvider" class="manage-select" title="平台" aria-label="反推平台" ${providers.length ? '' : 'disabled'}>
                ${providers.length ? providers.map(p => `<option value="${escapeAttr(p.id)}" ${p.id === localCaptionProvider ? 'selected' : ''}>${escapeHtml(p.name || p.id)}</option>`).join('') : '<option value="">暂无聊天平台</option>'}
            </select>
            <select id="localCaptionModel" class="manage-select" title="模型" aria-label="反推模型" ${models.length ? '' : 'disabled'}>
                ${models.length ? models.map(m => `<option value="${escapeAttr(m)}" ${m === localCaptionModel ? 'selected' : ''}>${escapeHtml(m)}</option>`).join('') : '<option value="">暂无模型</option>'}
            </select>
            <input id="localCaptionPrompt" class="local-caption-prompt-input" type="text" value="${escapeAttr(localCaptionPrompt || '描述图片')}" placeholder="描述图片" aria-label="图片提示词反推要求">
            ...
            <button class="asset-icon-btn" type="button" data-local-classify-toggle title="智能分类要求" aria-label="智能分类要求"><i data-lucide="${localClassifyPromptOpen ? 'chevron-up' : 'sliders-horizontal'}"></i></button>
            </div>
...
async function loadSharedFolders(){
    const generation = ++sharedFoldersLoadGeneration;
    if(sharedFoldersLoadController?.abort){
        try { sharedFoldersLoadController.abort(); } catch(_) {}
    }
    let controller = null;
    try { controller = typeof AbortController === 'function' ? new AbortController() : null; } catch(_) {}
    sharedFoldersLoadController = controller;
    try {
        const data = await apiJson('/api/shared-folders', controller?.signal ? {signal:controller.signal} : {});
        if(generation !== sharedFoldersLoadGeneration) return sharedFolders;
        if(!Array.isArray(data.folders)) throw new Error('共享文件夹返回数据格式不正确');
        sharedFolders = data.folders;
    } catch(err) {
        // A transient refresh failure should not make an already registered
        // folder disappear from the tree.  Aborted/stale requests are silent.
        if(isAbortError(err) || generation !== sharedFoldersLoadGeneration) return sharedFolders;
    } finally {
        if(sharedFoldersLoadController === controller) sharedFoldersLoadController = null;
    }
    return sharedFolders;
}
async function loadLocalAssets(){
    const generation = ++localAssetsLoadGeneration;
    if(localAssetsLoadController?.abort){
        try { localAssetsLoadController.abort(); } catch(_) {}
    }
    let controller = null;
    try { controller = typeof AbortController === 'function' ? new AbortController() : null; } catch(_) {}
    localAssetsLoadController = controller;
    try {
        const data = await apiJson('/api/local-assets', controller?.signal ? {signal:controller.signal} : {});
        if(generation !== localAssetsLoadGeneration) return localAssets;
        if(!Array.isArray(data.items)) throw new Error('本地素材返回数据格式不正确');
        ...
        // Keep the previous tree on a transient failure; an empty fallback can
        // look like data loss and invites accidental re-upload/delete actions.
        if(isAbortError(err) || generation !== localAssetsLoadGeneration) return localAssets;
    } finally {
        if(localAssetsLoadController === controller) localAssetsLoadController = null;
    }
    return localAssets;
}
...
async function refreshCanvasAssets(){
    const generation = ++canvasAssetsGeneration;
    try {
        setStatus('正在刷新画布资产...');
        const data = await apiJson('/api/canvas-assets');
        if(generation !== canvasAssetsGeneration) return {stale:true};
        canvasAssetsData = {
        ...
        if(generation !== canvasAssetsGeneration || isAbortError(err)) return {stale:true};
...
async function loadAll(){
    ...
    const canvasGeneration = ++canvasAssetsGeneration;
    const sharedGeneration = ++sharedFoldersLoadGeneration;
    const localGeneration = ++localAssetsLoadGeneration;
    if(sharedFoldersLoadController?.abort){
        try { sharedFoldersLoadController.abort(); } catch(_) {}
    }
    if(localAssetsLoadController?.abort){
    ...
    if(loadAllController?.abort){
        try { loadAllController.abort(); } catch(_) {}
    }
    let controller = null;
    try { controller = typeof AbortController === 'function' ? new AbortController() : null; } catch(_) {}
    loadAllController = controller;
    const request = url => apiJson(url, controller?.signal ? {signal:controller.signal} : {});
    const jobs = [
        ['资产库', request('/api/asset-library')],
        ['提示词库', request('/api/prompt-libraries')],
        ['API 平台', request('/api/providers')],
        ['画布资产', request('/api/canvas-assets')],
        ['共享文件夹', request('/api/shared-folders')],
        ['本地素材', request('/api/local-assets')]
    ];
    setStatus('加载中...');
    root?.setAttribute('aria-busy', 'true');
    try {
        // allSettled lets a temporarily unavailable optional endpoint (for
        // example a disconnected shared folder) leave the other libraries
        // usable.  Successful payloads are committed only for the latest
        // generation, so an older refresh can never overwrite a newer one.
        const results = await Promise.allSettled(jobs.map(([, promise]) => promise));
        if(generation !== loadAllGeneration) return {stale:true};
        const failures = [];
        const failed = (index, label, active=true) => {
            if(!active) return false;
            const result = results[index];
            if(result?.status === 'rejected'){
                if(!isAbortError(result.reason)) failures.push(`${label}：${result.reason?.message || '加载失败'}`);
                return true;
            }
            return false;
        };
        const [assetResult, promptResult, providerResult, canvasResult, sharedResult, localResult] = results;

        if(!failed(0, '资产库') && assetResult.value?.library && typeof assetResult.value.library === 'object'){
            assetLibrary = assetResult.value.library;
        } else if(assetResult?.status === 'fulfilled') {
            failures.push('资产库：返回数据格式不正确');
        }
        if(!failed(1, '提示词库') && promptResult.value?.library && typeof promptResult.value.library === 'object'){
            promptLibrary = promptResult.value.library;
        } else if(promptResult?.status === 'fulfilled') {
            failures.push('提示词库：返回数据格式不正确');
        }
        if(!failed(2, 'API 平台') && Array.isArray(providerResult.value?.providers)){
            apiProviders = providerResult.value.providers;
        } else if(providerResult?.status === 'fulfilled') {
            failures.push('API 平台：返回数据格式不正确');
        }
        if(!failed(3, '画布资产', canvasGeneration === canvasAssetsGeneration) && canvasGeneration === canvasAssetsGeneration && canvasResult.value
            && Array.isArray(canvasResult.value.categories)
            && Array.isArray(canvasResult.value.canvases)
            && Array.isArray(canvasResult.value.items)){
            canvasAssetsData = {
                categories:canvasResult.value.categories,
                canvases:canvasResult.value.canvases,
                items:canvasResult.value.items
            };
        } else if(canvasResult?.status === 'fulfilled' && canvasGeneration === canvasAssetsGeneration) {
            failures.push('画布资产：返回数据格式不正确');
        }
        if(!failed(4, '共享文件夹', sharedGeneration === sharedFoldersLoadGeneration) && sharedGeneration === sharedFoldersLoadGeneration && Array.isArray(sharedResult.value?.folders)){
            sharedFolders = sharedResult.value.folders;
        } else if(sharedResult?.status === 'fulfilled' && sharedGeneration === sharedFoldersLoadGeneration) {
            failures.push('共享文件夹：返回数据格式不正确');
        }
        if(!failed(5, '本地素材', localGeneration === localAssetsLoadGeneration) && localGeneration === localAssetsLoadGeneration && localResult.value
            && Array.isArray(localResult.value.items)){
            localAssets = localResult.value.items;
            localUploadTree = localResult.value.tree || {
                id:'__root__', path:'', name:'全部上传', count:localAssets.length, items:[], children:[]
            };
            if(activeLocalUploadFolder && !localUploadFolderExists(activeLocalUploadFolder)) activeLocalUploadFolder = '';
            if(activeLocalUploadClassFilter && !activeLocalUploadClassEntry()) activeLocalUploadClassFilter = '';
            selectedLocalUploadIds = new Set([...selectedLocalUploadIds].filter(id => localAssets.some(item => item.id === id)));
            localAssetsLoaded = true;
        } else if(localResult?.status === 'fulfilled' && localGeneration === localAssetsLoadGeneration) {
            failures.push('本地素材：返回数据格式不正确');
        }

        // Refreshing should not unexpectedly move the user to another library
        // or discard a bulk selection.  Keep the current context when it still
        // exists; normalize*State() prunes stale IDs and chooses a valid
        // fallback only when an entry was genuinely removed elsewhere.
        const libs = assetLibraries();
        const fallbackLibraryId = (libs.find(lib => lib.id === 'default') || libs[0])?.id || '';
        if(!libs.some(lib => lib.id === activeAssetLibraryId)) activeAssetLibraryId = fallbackLibraryId;
        if(!libs.some(lib => lib.id === activeWorkflowLibraryId)) activeWorkflowLibraryId = fallbackLibraryId;
        normalizeAssetState();
        ...
        normalizeLocalState();
        render();
        if(failures.length) setStatus(`已刷新，但${failures.join('、')}暂不可用`);
        else setStatus('准备就绪');
        return {stale:false, failures};
    } catch(err) {
        if(generation !== loadAllGeneration || isAbortError(err)) return {stale:true};
        setStatus(err.message || '加载失败');
        return {stale:false, failures:[err.message || '加载失败']};
    } finally {
        // A stale request must not clear the controller/flags owned by the
        // newer request that superseded it.
        if(generation === loadAllGeneration && loadAllController === controller){
...
function render(){
    ...
    document.querySelectorAll('[data-tab]').forEach(btn => {
        ...
        if(!btn.getAttribute('aria-controls')) btn.setAttribute('aria-controls', 'assetManagerRoot');
    });
        ...
        <section class="asset-panel asset-content ${canvasAssetManageMode ? 'manage-on' : ''}">
            <div class="content-toolbar">
                ...
                <div class="asset-tools">
                    ...
                    <label class="asset-search-wrap"><i data-lucide="search" aria-hidden="true"></i><input id="canvasAssetSearch" class="asset-search" type="search" aria-label="搜索画布资产" value="${escapeAttr(canvasAssetQuery)}" placeholder="搜索画布资产"></label>
                    ...
                    <button class="asset-btn ${canvasAssetManageMode ? 'primary' : ''}" type="button" data-canvas-asset-manage aria-pressed="${canvasAssetManageMode ? 'true' : 'false'}" ${total ? '' : 'disabled'}><i data-lucide="list-checks"></i><span>${canvasAssetManageMode ? '完成管理' : '批量管理'}</span></button>
                </div>
            </div>
...
function renderCanvasAssetCard(item){
    return `<article class="asset-card canvas-asset-card ${item.id === selectedCanvasAssetId ? 'active' : ''}" data-canvas-asset-card="${escapeAttr(item.id)}">
        <input class="asset-card-check" type="checkbox" data-canvas-asset-check="${escapeAttr(item.id)}" aria-label="选择画布资产：${escapeAttr(item.name || '未命名资产')}" ${selectedCanvasAssetIds.has(item.id) ? 'checked' : ''}>
    ...
    root.innerHTML = `
        ...
        <section class="asset-panel asset-content ${localUploadManageMode ? 'manage-on' : ''}">
            <div class="content-toolbar">
                ...
                <div class="asset-tools">
                    <label class="asset-search-wrap"><i data-lucide="search" aria-hidden="true"></i><input id="localUploadSearch" class="asset-search" type="search" aria-label="搜索本地上传" value="${escapeAttr(localUploadQuery)}" placeholder="搜索本地上传"></label>
                    <button class="asset-btn primary" type="button" data-localup-upload><i data-lucide="upload"></i><span>上传文件</span></button>
                    <button class="asset-btn ${localUploadManageMode ? 'primary' : ''}" type="button" data-localup-manage aria-pressed="${localUploadManageMode ? 'true' : 'false'}" ${total ? '' : 'disabled'}><i data-lucide="list-checks"></i><span>${localUploadManageMode ? '完成管理' : '批量管理'}</span></button>
                </div>
            </div>
            <div class="manage-tools local-manage-tools">
                <div class="manage-group manage-select-group">
                    <span class="manage-group-title">管理</span>
                    <span class="manage-summary" role="status" aria-live="polite">已选 ${selectedLocalUploadIds.size} 个，其中 ${imageCount} 张图片</span>
                    <div class="asset-tools local-manage-actions">
                        ...
                        <button class="asset-btn danger ${pendingBatchDelete === 'localup' ? 'detail-confirm' : ''}" type="button" data-localup-delete-selected ${selectedLocalUploadIds.size ? '' : 'disabled'} aria-label="${pendingBatchDelete === 'localup' ? '再次点击确认删除所选本地素材' : '删除所选本地素材'}"><i data-lucide="trash-2"></i><span>${pendingBatchDelete === 'localup' ? '确认删除' : '删除'}</span></button>
                    </div>
                </div>
...
function renderLocalUploadCard(item){
    ...
    return `<article class="asset-card ${item.id === selectedLocalUploadId ? 'active' : ''}" data-localup-card="${escapeAttr(item.id)}">
        <input class="asset-card-check" type="checkbox" data-localup-check="${escapeAttr(item.id)}" aria-label="选择本地素材：${escapeAttr(item.name || '未命名素材')}" ${selectedLocalUploadIds.has(item.id) ? 'checked' : ''}>
...
function renderLocalCard(item){
    ...
    return `<article class="asset-card ${item.id === selectedLocalId ? 'active' : ''}" data-local-card="${escapeAttr(item.id)}">
        <input class="asset-card-check" type="checkbox" data-local-check="${escapeAttr(item.id)}" aria-label="选择共享素材：${escapeAttr(item.name || '未命名素材')}" ${selectedLocalIds.has(item.id) ? 'checked' : ''}>
    ...
    root.innerHTML = `
        ...
        <section class="asset-panel asset-content ${assetManageMode ? 'manage-on' : ''}">
            <div class="content-toolbar">
                ...
                <div class="asset-tools">
                    <label class="asset-search-wrap"><i data-lucide="search" aria-hidden="true"></i><input id="assetSearch" class="asset-search" type="search" aria-label="搜索素材" value="${escapeAttr(assetQuery)}" placeholder="搜索素材"></label>
                    <button class="asset-btn ${assetManageMode ? 'primary' : ''}" type="button" data-asset-manage aria-pressed="${assetManageMode ? 'true' : 'false'}"><i data-lucide="list-checks"></i><span>${assetManageMode ? '完成管理' : '批量管理'}</span></button>
                </div>
            </div>
            ...
            <div class="manage-tools">
                <div class="manage-group manage-select-group">
                    <span class="manage-group-title">管理</span>
                    <span class="manage-summary" role="status" aria-live="polite">已选 ${selectedAssetIds.size} 个，其中 ${imageCount} 张图片</span>
                    <div class="asset-tools">
                        ...
                        <button class="asset-btn danger ${pendingBatchDelete === 'asset' ? 'detail-confirm' : ''}" type="button" data-asset-delete-selected ${selectedAssetIds.size ? '' : 'disabled'} aria-label="${pendingBatchDelete === 'asset' ? '再次点击确认删除所选素材' : '删除所选素材'}"><i data-lucide="trash-2"></i><span>${pendingBatchDelete === 'asset' ? '确认删除' : '删除所选'}</span></button>
                    </div>
                </div>
                <div class="manage-group manage-ai-group">
                    <span class="manage-group-title">AI 处理</span>
    ...
    root.innerHTML = `
        ...
        <section class="asset-panel asset-content ${workflowManageMode ? 'manage-on' : ''}">
            <div class="content-toolbar">
                ...
                <div class="asset-tools">
                    <label class="asset-search-wrap"><i data-lucide="search" aria-hidden="true"></i><input id="workflowSearch" class="asset-search" type="search" aria-label="搜索工作流" value="${escapeAttr(workflowQuery)}" placeholder="搜索工作流"></label>
                    <button class="asset-btn ${workflowManageMode ? 'primary' : ''}" type="button" data-workflow-manage aria-pressed="${workflowManageMode ? 'true' : 'false'}"><i data-lucide="list-checks"></i><span>${workflowManageMode ? '完成管理' : '批量管理'}</span></button>
                </div>
            </div>
            <div class="manage-tools">
                <span role="status" aria-live="polite">已选择 ${selectedWorkflowIds.size} 个工作流。</span>
                <div class="asset-tools">
                    ...
                    <button class="asset-btn danger ${pendingBatchDelete === 'workflow' ? 'detail-confirm' : ''}" type="button" data-workflow-delete-selected ${selectedWorkflowIds.size ? '' : 'disabled'} aria-label="${pendingBatchDelete === 'workflow' ? '再次点击确认删除所选工作流' : '删除所选工作流'}"><i data-lucide="trash-2"></i><span>${pendingBatchDelete === 'workflow' ? '确认删除' : '删除所选'}</span></button>
                </div>
            </div>
            <div class="content-scroll">
                <div class="asset-grid">
...
function renderWorkflowCard(item){
    return `<article class="asset-card workflow-card ${item.id === selectedWorkflowId ? 'active' : ''}" data-workflow-card="${escapeAttr(item.id)}">
        <input class="asset-card-check" type="checkbox" data-workflow-check="${escapeAttr(item.id)}" aria-label="选择工作流：${escapeAttr(item.name || '未命名工作流')}" ${selectedWorkflowIds.has(item.id) ? 'checked' : ''}>
...
function renderAssetCard(item){
    return `<article class="asset-card ${item.id === selectedAssetId ? 'active' : ''}" data-asset-card="${escapeAttr(item.id)}">
        <input class="asset-card-check" type="checkbox" data-asset-check="${escapeAttr(item.id)}" aria-label="选择素材：${escapeAttr(item.name || '未命名素材')}" ${selectedAssetIds.has(item.id) ? 'checked' : ''}>
        <div class="asset-thumb">${assetThumb(item)}</div>
        <div class="asset-card-body">
    ...
    root.innerHTML = `
        ...
        <section class="asset-panel asset-content ${promptManageMode ? 'manage-on' : ''}">
            <div class="content-toolbar">
                ...
                <div class="asset-tools">
                    <label class="asset-search-wrap"><i data-lucide="search" aria-hidden="true"></i><input id="promptSearch" class="asset-search" type="search" aria-label="搜索名称、说明或正文" value="${escapeAttr(promptQuery)}" placeholder="搜索名称、说明或正文"></label>
                    ...
                    <button class="asset-btn ${promptManageMode ? 'primary' : ''}" type="button" data-prompt-manage aria-pressed="${promptManageMode ? 'true' : 'false'}"><i data-lucide="list-checks"></i><span>${promptManageMode ? '完成管理' : '批量管理'}</span></button>
                </div>
            </div>
            <div class="manage-tools">
                <span role="status" aria-live="polite">已选择 ${selectedPromptIds.size} 条提示词，支持拖拽框选或逐个勾选。</span>
                <div class="asset-tools">
...
function renderPromptRow(item, readonly){
    return `<article class="prompt-row ${item.id === selectedPromptId ? 'active' : ''}" data-prompt-row="${escapeAttr(item.id)}">
        <input class="prompt-row-check" type="checkbox" data-prompt-check="${escapeAttr(item.id)}" aria-label="选择提示词：${escapeAttr(item.name || '未命名提示词')}" ${selectedPromptIds.has(item.id) ? 'checked' : ''} ${readonly ? 'disabled' : ''}>
...
async function deleteWorkflowItem(id){
    ...
    try {
        ...
        pendingDeleteAssetId = '';
        render();
        setStatus(err.message || '删除工作流失败');
    }
}
async function deleteSelectedWorkflows(){
    const ids = batchSelectionWithinLimit([...selectedWorkflowIds], '工作流');
    if(!ids){ pendingBatchDelete = ''; render(); return; }
    if(!ids.length) return;
    if(pendingBatchDelete !== 'workflow'){
        pendingBatchDelete = 'workflow';
        render();
        setStatus('再次点击确认删除所选工作流');
        return;
    }
    try {
        ...
        assetLibrary = data.library || assetLibrary;
        const result = batchMutationCounts(data, ids.length);
        const skippedIds = new Set((result.skipped || []).map(item => String(item?.id ?? item ?? '')));
        const removedIds = new Set((Array.isArray(data.removed_ids) ? data.removed_ids : (Array.isArray(data.deleted) ? data.deleted : []))
            .map(String));
        selectedWorkflowIds = new Set(ids.filter(id => skippedIds.has(String(id))));
        if(selectedWorkflowId && removedIds.has(String(selectedWorkflowId))) selectedWorkflowId = '';
        pendingBatchDelete = '';
        render();
        setStatus(batchMutationStatus(data, ids.length, '工作流'));
    } catch(err) {
        pendingBatchDelete = '';
        render();
        setStatus(err.message || '删除工作流失败');
...
async function deleteLocalAssets(ids, options={}){
    const selectedIds = batchSelectionWithinLimit(ids, '本地素材');
    if(!selectedIds){ pendingBatchDelete = ''; render(); return; }
    const selectedItems = selectedIds.map(id => findLocalUpload(id)).filter(Boolean);
    const names = selectedItems.map(item => item.file).filter(Boolean);
    if(!names.length) return;
    const requireConfirm = options.confirm !== false;
    if(requireConfirm && pendingBatchDelete !== 'localup'){
        pendingBatchDelete = 'localup';
        render();
        setStatus(`再次点击确认删除所选 ${names.length} 个本地素材`);
        return;
    }
    setStatus('正在删除...');
    try {
        ...
        await loadLocalAssets();
        if(activeLocalUploadClassFilter && !activeLocalUploadClassEntry()) activeLocalUploadClassFilter = '';
        const result = batchMutationCounts(data, names.length);
        const skippedNames = new Set((result.skipped || []).map(item => String(item?.id ?? item ?? '')));
        selectedLocalUploadIds = new Set(selectedItems
            .filter(item => skippedNames.has(String(item.file)))
            .map(item => item.id));
        pendingBatchDelete = '';
        if(selectedLocalUploadId && !findLocalUpload(selectedLocalUploadId)) selectedLocalUploadId = '';
        render();
        setStatus(batchMutationStatus(data, names.length, '素材'));
    } catch(err) {
        pendingBatchDelete = '';
        render();
...
async function handleClick(event){
    ...
    if(tabBtn){ activeTab = tabBtn.dataset.tab || 'assets'; selectedAssetIds.clear(); selectedWorkflowIds.clear(); selectedPromptIds.clear(); selectedLocalIds.clear(); selectedLocalUploadIds.clear(); selectedCanvasAssetIds.clear(); pendingBatchDelete = ''; pendingDeleteAssetId = ''; pendingDeletePromptId = ''; render(); return; }
    ...
    if(target.closest?.('[data-localup-select-all]')){ localUploadItems().forEach(item => selectedLocalUploadIds.add(item.id)); pendingBatchDelete = ''; render(); return; }
    if(target.closest?.('[data-localup-clear]')){ selectedLocalUploadIds.clear(); pendingBatchDelete = ''; render(); return; }
    ...
    if(localUpDeleteOne){ await deleteLocalAssets([localUpDeleteOne.dataset.localupDeleteOne || ''], {confirm:false}); return; }
    const localUpCheck = target.closest?.('[data-localup-check]');
    if(localUpCheck){
    ...
    if(target.closest?.('[data-workflow-select-all]')){ currentWorkflowItems().forEach(item => selectedWorkflowIds.add(item.id)); pendingBatchDelete = ''; render(); return; }
    if(target.closest?.('[data-workflow-clear-selection]')){ selectedWorkflowIds.clear(); pendingBatchDelete = ''; render(); return; }
    ...
    if(workflowLib){ activeWorkflowLibraryId = workflowLib.dataset.workflowLib || ''; activeWorkflowCategoryId = ''; selectedWorkflowId = ''; selectedWorkflowIds.clear(); pendingDeleteAssetId = ''; pendingBatchDelete = ''; render(); return; }
    const workflowCat = target.closest?.('[data-workflow-cat]');
    if(workflowCat){ activeWorkflowLibraryId = workflowCat.dataset.workflowCatLib || activeWorkflowLibraryId; activeWorkflowCategoryId = workflowCat.dataset.workflowCat || ''; selectedWorkflowId = ''; selectedWorkflowIds.clear(); pendingDeleteAssetId = ''; pendingBatchDelete = ''; render(); return; }
    const workflowCard = target.closest?.('[data-workflow-card]');
    if(workflowCard){
    ...
    if(assetLib){ activeAssetLibraryId = assetLib.dataset.assetLib || ''; assetTreeFocus = 'library'; activeAssetClassFilter = ''; activeAssetCategoryId = assetCategories()[0]?.id || ''; selectedAssetId = ''; selectedAssetIds.clear(); pendingDeleteAssetId = ''; pendingBatchDelete = ''; render(); return; }
    ...
    if(assetCat){ activeAssetLibraryId = assetCat.dataset.assetCatLib || activeAssetLibraryId; activeAssetCategoryId = assetCat.dataset.assetCat || ''; activeAssetClassFilter = ''; assetTreeFocus = 'category'; selectedAssetId = ''; selectedAssetIds.clear(); pendingDeleteAssetId = ''; pendingBatchDelete = ''; render(); return; }
    const assetCheck = target.closest?.('[data-asset-check]');
    if(assetCheck){
        event.preventDefault();
        event.stopPropagation();
    ...
    if(promptLib){ activePromptLibraryId = promptLib.dataset.promptLib || ''; activePromptCategory = 'all'; promptTreeFocus = 'library'; selectedPromptId = ''; promptCreateMode = false; promptEditMode = false; selectedPromptIds.clear(); pendingDeletePromptId = ''; pendingBatchDelete = ''; render(); return; }
    const promptCat = target.closest?.('[data-prompt-cat]');
    if(promptCat){ activePromptLibraryId = promptCat.dataset.promptCatLib || activePromptLibraryId; activePromptCategory = promptCat.dataset.promptCat || 'all'; promptTreeFocus = 'category'; selectedPromptId = ''; promptCreateMode = false; promptEditMode = false; selectedPromptIds.clear(); pendingDeletePromptId = ''; pendingBatchDelete = ''; render(); return; }
    const promptRow = target.closest?.('[data-prompt-row]');
    if(promptRow){
...
async function deleteAssetItem(id){
    ...
    try {
        ...
        pendingDeleteAssetId = '';
        render();
        setStatus(err.message || '删除素材失败');
    }
}
async function deleteSelectedAssets(){
    const ids = batchSelectionWithinLimit([...selectedAssetIds], '素材');
    if(!ids){ pendingBatchDelete = ''; render(); return; }
    if(!ids.length) return;
    if(pendingBatchDelete !== 'asset'){
        pendingBatchDelete = 'asset';
        render();
        setStatus('再次点击确认删除所选素材');
        return;
    }
    try {
        ...
        const removedIds = new Set((Array.isArray(data.removed_ids) ? data.removed_ids : (Array.isArray(data.deleted) ? data.deleted : []))
            .map(String));
        selectedAssetIds = new Set(ids.filter(id => skippedIds.has(String(id))));
        if(selectedAssetId && removedIds.has(String(selectedAssetId))) selectedAssetId = '';
        pendingBatchDelete = '';
        render();
        setStatus(batchMutationStatus(data, ids.length, '素材'));
    } catch(err) {
        pendingBatchDelete = '';
        render();
        setStatus(err.message || '删除素材失败');
...
async function deletePromptItem(id){
    ...
    try {
        ...
        pendingDeletePromptId = '';
        render();
        setStatus(err.message || '删除提示词失败');
    }
}
async function deleteSelectedPrompts(){
    const ids = batchSelectionWithinLimit([...selectedPromptIds], '提示词');
    if(!ids){ pendingBatchDelete = ''; render(); return; }
    if(!ids.length) return;
    ...
    try {
        const data = await apiJson('/api/prompt-libraries/items/delete', {method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({library_id:activePromptLibraryId, ids})});
        promptLibrary = data.library || promptLibrary;
        const result = batchMutationCounts(data, ids.length);
        const skippedIds = new Set((result.skipped || []).map(item => String(item?.id ?? item ?? '')));
        const removedIds = new Set((Array.isArray(data.removed_ids) ? data.removed_ids : (Array.isArray(data.deleted) ? data.deleted : []))
            .map(String));
        selectedPromptIds = new Set(ids.filter(id => skippedIds.has(String(id))));
        if(selectedPromptId && removedIds.has(String(selectedPromptId))) selectedPromptId = '';
        pendingBatchDelete = '';
        render();
        setStatus(batchMutationStatus(data, ids.length, '提示词'));
    } catch(err) {
        pendingBatchDelete = '';
        render();
        setStatus(err.message || '删除提示词失败');
