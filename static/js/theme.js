(function(){
    ...
    function autoScale(){
        ...
        // Automatic mode must preserve readable text.  The previous fitting
        // rule transformed the whole document to 0.68–0.99 on ordinary
        // laptops and phones, which made labels effectively disappear and
        // also defeated the page-level responsive breakpoints.  Let CSS
        // reflow the layout below the design reference size; only explicit
        // scale modes may intentionally choose a value below 100%.
        if(compactRatio < 1) return 1;
    ...
    function scheduleContentFit(mode){
        ...
        contentFitTimer = setTimeout(() => {
            ...
            // Content-fit is a safety net for accidental overflow, not a
            // second automatic text shrinker.  Keep auto mode at or above
            // 100%; responsive containers and scroll regions handle wide
            // content without making every glyph smaller.
            const fitted = Math.max(1, Math.min(current, viewportWidth / contentWidth));
