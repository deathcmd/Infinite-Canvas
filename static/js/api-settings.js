...
// The open-source build keeps the legacy template data and handlers for
// backwards-compatible imports, but does not surface provider promotions or
// account/credit upsells. Users configure an explicit custom/local provider.
const ENABLE_SERVICE_TEMPLATES = false;
// Keep legacy wallet fields readable for imported settings, but never let a
// hidden wallet value become the active credential or be edited accidentally.
const OPEN_SOURCE_NO_BILLING = true;
// Runtime defense-in-depth for stale API responses/imported settings. The
// backend applies the same policy, but this keeps a legacy page from rendering
// or selecting a disabled vendor or explicitly paid-tier entry if it is opened
// from an old cache.  Do not blanket-match `pro`/`plus`: those are common model
// names, not billing controls.
const OPEN_SOURCE_BLOCKED_PROVIDER_RE = /(?:grok|sub2api)/i;
const OPEN_SOURCE_BLOCKED_MODEL_RE = /grok/i;
const OPEN_SOURCE_BILLING_MODEL_RE = /(?:会员|付费|充值|订阅|(?<![a-z0-9])(?:vip|premium|paid|subscription|membership)(?![a-z0-9]))/i;
const OPEN_SOURCE_NO_BILLING_MESSAGE = '本地开源版本不提供积分/钱包功能';
const JIMENG_CREDIT_COMMAND_RE = /(?:credit|balance|quota|point|coin|wallet|积分|余额|额度|钱包)/i;
function isJimengCreditCommand(value){
    return JIMENG_CREDIT_COMMAND_RE.test(String(value || '').trim());
}
function isOpenSourceBlockedModel(value){
    const text = String(value || '').trim();
    return OPEN_SOURCE_BLOCKED_MODEL_RE.test(text) || OPEN_SOURCE_BILLING_MODEL_RE.test(text);
}
function isOpenSourceBlockedProvider(item){
    if(!item) return false;
    if(OPEN_SOURCE_BLOCKED_PROVIDER_RE.test(
        [item.id, item.name, item.base_url].map(value => String(value || '')).join(' ')
    )) return true;
    // Only provider id/name participate in the billing-label check.  A URL
    // such as `https://apistudio.vip` is not itself a billing selector.
    return OPEN_SOURCE_BILLING_MODEL_RE.test(
        [item.id, item.name].map(value => String(value || '')).join(' ')
    );
}
function sanitizeOpenSourceProvider(item){
    if(!item || typeof item !== 'object') return item;
    ['image_models', 'chat_models', 'video_models'].forEach(key => {
        if(Array.isArray(item[key])) item[key] = item[key].filter(model => !isOpenSourceBlockedModel(model));
    });
    if(item.model_names && typeof item.model_names === 'object'){
        Object.keys(item.model_names).forEach(key => {
            if(isOpenSourceBlockedModel(key) || isOpenSourceBlockedModel(item.model_names[key])) delete item.model_names[key];
        });
    }
    if(item.model_protocols && typeof item.model_protocols === 'object'){
        Object.keys(item.model_protocols).forEach(key => {
            if(isOpenSourceBlockedModel(key)) delete item.model_protocols[key];
        });
    }
    return item;
}
...
const RECOMMENDED_APIS = [
    ...
    },
    {
        ...
        advantages:['图像、视频和 LLM 接口', '支持自定义模型列表', '适合混合工作流'],
...
function isProviderTemporarilyHidden(item){
    return isOpenSourceBlockedProvider(item);
}
function visibleProviders(){
    return (providers || [])
        .filter(item => !isProviderTemporarilyHidden(item))
        .map(item => sanitizeOpenSourceProvider(item));
}
...
function rhFreeKeyHintText(item){
    return item?.has_key ? `当前服务 Key 已保存：${item.key_env || 'API/.env'} ${item.key_preview || ''}` : '尚未保存服务 Key。';
}
function rhWalletKeyHintText(item){
    return item?.has_wallet_key ? `兼容凭据已保存：${item.wallet_key_env || 'API/.env'} ${item.wallet_key_preview || ''}` : '暂无兼容凭据（可选）。';
}
...
function isNewUserProvider(item){
    ...
    if(item.id === 'runninghub') return !item.has_key && (OPEN_SOURCE_NO_BILLING || !item.has_wallet_key);
    return false;
}
                    ...
                    <div class="onboarding-rh-linear-row onboarding-ms-linear-row">
                        <div class="onboarding-rh-source-group">
                            ...
                            <div class="onboarding-key-actions onboarding-rh-key-actions">
                                <span class="onboarding-source-note"><i data-lucide="key-round" class="w-3.5 h-3.5"></i><span>${escapeHtml(tr('api.providerCredentialHint'))}</span></span>
                            </div>
                        </div>
                        ...
                        <label class="onboarding-key-field onboarding-rh-row-field">
                            <span>API Key</span>
                            <input type="password" aria-label="ModelScope API Key" value="${escapeAttr(keyInput?.value || '')}" placeholder="${escapeAttr(tr('api.msTokenPlaceholder'))}" oninput="syncOnboardingKeyInput('standard', this.value)">
                        </label>
                    </div>
                </div>
                ...
                <div class="onboarding-rh-linear-rows">
                    <div class="onboarding-rh-linear-row">
                        <div class="onboarding-rh-source-group">
                            ...
                            <div class="onboarding-key-actions onboarding-rh-key-actions">
                                <span class="onboarding-source-note"><i data-lucide="coins" class="w-3.5 h-3.5"></i><span>${escapeHtml(tr('api.rhCredentialHint'))}</span></span>
                            </div>
                        </div>
                        <div class="recommend-flow-arrow onboarding-flow-arrow onboarding-rh-row-arrow" aria-hidden="true"><span></span><b></b></div>
                        <label class="onboarding-key-field onboarding-rh-row-field">
                            <span>${escapeHtml(tr('api.rhCoinApiKeyRequired'))}</span>
                            <input type="password" aria-label="RunningHub 服务 API Key" value="${escapeAttr(rhFreeKeyInput?.value || '')}" placeholder="${escapeAttr(tr('api.rhCoinPlaceholder'))}" oninput="syncOnboardingKeyInput('free', this.value)">
                        </label>
                    </div>
                </div>
                <div class="onboarding-rh-save-line">
...
function syncOnboardingKeyInput(kind, value){
    if(kind === 'wallet'){
        if(!OPEN_SOURCE_NO_BILLING && rhWalletKeyInput) rhWalletKeyInput.value = value || '';
        return;
    }
...
function syncEditor(){
    ...
    if(item.id === 'runninghub'){
        const freeKey = rhFreeKeyInput?.value.trim() || '';
        const walletKey = OPEN_SOURCE_NO_BILLING ? '' : (rhWalletKeyInput?.value.trim() || '');
...
function pickRhThumbnail(kind, index){
    ...
    input.type = 'file';
    input.setAttribute('aria-label', '上传 RunningHub 缩略图');
    input.accept = 'image/*';
...
function renderRhMappedPreviewHtml(config){
    ...
    const workflowOptionsHtml = rhEditorMode === 'workflow' ? `
        <div class="rh-workflow-run-mode">
            <label>
                <span>空可选图</span>
                <select aria-label="可选图片为空时的处理方式" onchange="setRhWorkflowOptionalImageMode(this.value)">
...
function renderRhPreviewControl(field){
    ...
    if(['IMAGE','VIDEO','AUDIO'].includes(kind)){
        ...
        return `<div class="rh-preview-field"><div class="rh-preview-label">${label}${slot}</div><button class="rh-preview-media ${previewState.url ? 'has-media' : ''}" type="button" aria-label="上传${escapeAttr(field.label || field.fieldName || '媒体')}" onclick="pickRhPreviewMedia('${escapeAttr(key)}','${kind}')">${media}</button></div>`;
    }
    if(kind === 'BOOLEAN'){
    ...
    if(kind === 'SLIDER'){
        ...
        return `<div class="rh-preview-field"><div class="rh-preview-label"><span>${label}</span><span class="rh-preview-slider-val">${escapeHtml(numericValue)}</span></div><input class="rh-preview-slider" aria-label="${escapeAttr(field.label || field.fieldName || '滑块')}" type="range" min="${escapeAttr(min)}" max="${escapeAttr(max)}" step="${escapeAttr(step)}" value="${escapeAttr(numericValue)}" oninput="updateRhPreviewValue('${escapeAttr(key)}', this.value); const val=this.closest('.rh-preview-field')?.querySelector('.rh-preview-slider-val'); if(val) val.textContent=this.value;"></div>`;
    }
    if(options.length || kind === 'SELECT'){
        return `<div class="rh-preview-field"><div class="rh-preview-label">${label}</div><select aria-label="${escapeAttr(field.label || field.fieldName || '选项')}" disabled>${(options.length ? options : [value || '选项']).map(option => `<option>${escapeHtml(option)}</option>`).join('')}</select></div>`;
    }
    const randomButton = kind === 'NUMBER' && field.random_enabled
        ? `<button class="random-btn rh-preview-random-btn ${randomActive ? 'active' : ''}" type="button" aria-label="${randomActive ? '使用随机数' : '使用固定数'}" onclick="toggleRhPreviewRandom('${escapeAttr(key)}')" title="${randomActive ? '使用随机数' : '使用固定数'}"><i data-lucide="dice-5" class="w-4 h-4"></i></button>`
        : '';
    const readonly = randomActive ? 'disabled' : '';
    return `<div class="rh-preview-field"><div class="rh-preview-label">${label}</div><div class="rh-preview-random-row" style="${randomButton ? '' : 'grid-template-columns:1fr'}"><input ${readonly} aria-label="${escapeAttr(field.label || field.fieldName || '文本')}" type="${kind === 'NUMBER' ? 'number' : 'text'}" value="${escapeAttr(value)}" placeholder="${kind === 'NUMBER' && randomActive ? '随机数' : ''}" oninput="updateRhPreviewValue('${escapeAttr(key)}', this.value)">${randomButton}</div></div>`;
}
function renderRhPreviewMedia(url, kind, name=''){
    const safe = escapeAttr(url || '');
    if(kind === 'VIDEO') return `<video src="${safe}" muted preload="metadata" playsinline controls aria-label="${escapeHtml(name || '视频预览')}"></video>`;
    if(kind === 'AUDIO') return `<span class="rh-preview-audio"><i data-lucide="file-audio" class="w-5 h-5"></i>${escapeHtml(name || '音频')}</span><audio src="${safe}" controls preload="metadata" aria-label="${escapeHtml(name || '音频预览')}"></audio>`;
    return `<img src="${safe}" alt="图片预览">`;
}
...
async function pickRhPreviewMedia(key, kind){
    const input = document.createElement('input');
    input.type = 'file';
    input.setAttribute('aria-label', `上传${kind === 'VIDEO' ? '视频' : kind === 'AUDIO' ? '音频' : '图片'}`);
...
function renderRhWorkflowEditorField(field){
    ...
    const imageSlotControls = rhEditorMode === 'workflow' && type === 'IMAGE' ? `
        <div class="rh-image-slot-row">
            <label><span>排序</span><input type="number" aria-label="${escapeAttr(field.label || field.fieldName || '图片')} 排序" min="1" step="1" value="${escapeAttr(field.imageOrder || '')}" oninput="updateRhWorkflowEditorField('${escapeAttr(key)}','imageOrder',this.value)"></label>
        ...
        <div class="rh-editor-field-row ${checked ? 'active' : ''}">
            ...
            <div class="rh-editor-field-main">
                ...
                <div class="rh-editor-field-controls">
                    <input type="text" aria-label="字段显示名称" value="${escapeAttr(field.label || '')}" placeholder="显示名称" oninput="updateRhWorkflowEditorField('${escapeAttr(key)}','label',this.value)">
                    <select aria-label="字段类型" onchange="updateRhWorkflowEditorField('${escapeAttr(key)}','fieldType',this.value)">
                ...
                <div class="rh-editor-field-controls rh-editor-wide-controls">
                    <textarea aria-label="下拉选项" placeholder="下拉选项：每行一个，例如 1024x1024" oninput="updateRhWorkflowEditorField('${escapeAttr(key)}','options',this.value)">${escapeHtml(optionsText)}</textarea>
                </div>
                <div class="rh-editor-random-row">
                    ...
                    <input type="number" aria-label="字段最小值" value="${escapeAttr(field.min ?? '')}" placeholder="最小" oninput="updateRhWorkflowEditorField('${escapeAttr(key)}','min',this.value)">
                    <input type="number" aria-label="字段最大值" value="${escapeAttr(field.max ?? '')}" placeholder="最大" oninput="updateRhWorkflowEditorField('${escapeAttr(key)}','max',this.value)">
                    <input type="number" aria-label="字段步长" value="${escapeAttr(field.step ?? '')}" placeholder="步长" oninput="updateRhWorkflowEditorField('${escapeAttr(key)}','step',this.value)">
                </div>
            </div>
        </div>
    `;
        ...
        <div class="rh-config-card">
            ...
            <div class="rh-card-main">
                <label class="rh-card-title-field">
                    <span>名称</span>
                    <input type="text" aria-label="${kind === 'app' ? 'AI 应用名称' : '工作流名称'}" value="${escapeAttr(entry.title || '')}" oninput="updateRhEntry('${kind}', ${entry._rhIndex ?? index}, 'title', this.value)" placeholder="${kind === 'app' ? 'AI 应用名称' : '工作流名称'}">
                </label>
                ...
                <textarea aria-label="备注、用途、参数说明" oninput="updateRhEntry('${kind}', ${entry._rhIndex ?? index}, 'note', this.value)" placeholder="备注、用途、参数说明">${escapeHtml(entry.note || '')}</textarea>
            </div>
...
function syncRecommendView(){
    if(!ENABLE_SERVICE_TEMPLATES) recommendInlineOpen = false;
    ...
    if(recommendContent) recommendContent.setAttribute('aria-hidden', recommendInlineOpen ? 'false' : 'true');
    if(recommendApiOverlay){
        recommendApiOverlay.hidden = true;
        recommendApiOverlay.style.display = 'none';
    }
...
function renderRecommendApi(){
            ...
            <div class="recommend-platform-setup">
                ...
                <div class="recommend-quick-stack recommend-setup-flow">
                    <div class="recommend-guide-source onboarding-rh-source-group">
                        <div class="onboarding-rh-source-label">${escapeHtml(tr('api.customProviderLabel'))}</div>
                    ...
                    <div class="recommend-guide-save">
                        <label class="onboarding-key-field onboarding-rh-row-field">
                            ...
                            <input type="password" aria-label="${escapeAttr(api.name)} API Key" data-recommend-key="${index}" placeholder="${escapeAttr(trf('api.recommendKeyPlaceholder', {name:api.name}))}">
                        </label>
    ...
    recommendPanel.innerHTML = `
        ...
        <div class="recommend-note recommend-seedance-private-note">
            <span class="recommend-seedance-private-icon"><i data-lucide="shield-check" class="w-3.5 h-3.5"></i></span>
            <span class="recommend-seedance-private-text">模板只用于填写本地连接配置，不会代替你创建账号或跳转外部页面。</span>
        </div>
    `;
    refreshIcons();
}
function recommendedProviderForApi(api){
    if(!api || isOpenSourceBlockedProvider(api) || (api.image_models || []).some(isOpenSourceBlockedModel) || (api.chat_models || []).some(isOpenSourceBlockedModel) || (api.video_models || []).some(isOpenSourceBlockedModel)) return null;
...
async function saveRecommendedApi(index){
    ...
    if(!api || isOpenSourceBlockedProvider(api)) return;
    const input = recommendPanel?.querySelector(`[data-recommend-key="${index}"]`);
...
function renderProviderList(){
    providerList.innerHTML = sortedProviders().map(item => {
        ...
        // A legacy wallet credential is intentionally not considered an
        // active configuration signal in the open-source build; only the
        // regular service key (or a local CLI protocol) marks a provider ready.
        const stateClass = item.enabled === false ? 'is-disabled' : (item.has_key || CLI_PROTOCOLS.has(itemProtocol) ? 'has-key' : 'missing-key');
        const protocolLabel = item.id === 'runninghub' ? 'RH' : String(item.protocol || 'openai').toUpperCase();
        if(item.id === 'modelscope'){
            return `
                <button class="provider-card provider-card-banner ${active} ${stateClass}" type="button" aria-label="ModelScope（OpenAI 兼容）" title="选择 ModelScope" onclick="selectProvider('${escapeHtml(item.id)}')">
        ...
        if(item.id === 'runninghub'){
            return `
                <button class="provider-card provider-card-banner ${active} ${stateClass}" type="button" aria-label="RunningHub（RH）" title="选择 RunningHub" onclick="selectProvider('${escapeHtml(item.id)}')">
                    <span class="provider-banner-inner">
                        <span class="provider-logo-wrap">
        ...
        if(item.id === 'volcengine'){
            return `
                <button class="provider-card provider-card-banner ${active} ${stateClass}" type="button" aria-label="火山引擎（Ark）" title="选择火山引擎" onclick="selectProvider('${escapeHtml(item.id)}')">
                    <span class="provider-banner-inner">
                        <span class="provider-logo-wrap">
...
function renderEditor(){
    ...
    if(isRunningHub){
        ...
        if(rhFreeKeyInput){
            ...
            rhFreeKeyInput.placeholder = item.has_key ? `保持当前服务 Key ${item.key_preview || ''}` : '粘贴服务 API Key';
        }
        if(rhWalletKeyInput){
            rhWalletKeyInput.value = '';
            rhWalletKeyInput.placeholder = item.has_wallet_key ? `保持兼容凭据 ${item.wallet_key_preview || ''}` : '兼容凭据（可选）';
        }
...
function jimengCreditText(raw){
    // Kept as a no-op compatibility helper for old inline handlers.  Account
    // credit data is intentionally never rendered in the open-source build.
    return raw ? OPEN_SOURCE_NO_BILLING_MESSAGE : '';
}
...
async function refreshJimengStatus(showCredit=true){
    ...
    try {
        ...
        const statusText = data.logged_in === true ? '已登录' : (data.logged_in === null && data.installed ? '待执行验证' : (data.installed ? '未登录' : '未安装'));
        setJimengStatus(statusText, data.logged_in === true);
...
async function refreshJimengCredit(){
    // Legacy inline handler: account/credit queries are disabled and never
    // reach the backend in the open-source build.
    setJimengStatus('不可用', false);
    if(jimengCredit) jimengCredit.textContent = OPEN_SOURCE_NO_BILLING_MESSAGE;
}
...
async function loadJimengHelp(){
    ...
    try {
        ...
        if(isJimengCreditCommand(command)){
            jimengHelpOutput.textContent = OPEN_SOURCE_NO_BILLING_MESSAGE;
            return;
        }
...
function currentProviderApiKey(item){
    if(item?.id === 'runninghub'){
        // Wallet billing is intentionally disabled in this build. Prefer the
        // service key even if a password manager populated the hidden legacy
        // wallet input.
        return rhFreeKeyInput?.value.trim() || keyInput?.value.trim() || '';
    }
...
function setFetchedModelState(data){
    const safeModels = values => Array.isArray(values)
        ? values.map(value => String(value || '').trim()).filter(value => value && !isOpenSourceBlockedModel(value))
        : [];
    lastFetchedAll = safeModels(data?.all);
    lastFetchedSuggestion = {
        image: new Set(safeModels(data?.image_models)),
        chat: new Set(safeModels(data?.chat_models)),
        video: new Set(safeModels(data?.video_models)),
    };
    lastFetchedModelNames = (data?.model_names && typeof data.model_names === 'object')
        ? Object.fromEntries(Object.entries(data.model_names).filter(([key, value]) => !isOpenSourceBlockedModel(key) && !isOpenSourceBlockedModel(value)))
        : {};
}
...
function openModelPicker(){
    ...
    const safe = values => (Array.isArray(values) ? values : []).filter(id => !isOpenSourceBlockedModel(id));
    const existing = { image: new Set(safe(item.image_models)), chat: new Set(safe(item.chat_models)), video: new Set(safe(item.video_models)) };
    const allIds = new Set([...safe(lastFetchedAll), ...safe(item.image_models), ...safe(item.chat_models), ...safe(item.video_models)]);
...
function renderModelPicker(){
    ...
    const ids = Object.keys(pickerState.category).filter(id => !isOpenSourceBlockedModel(id)).sort();
    // 各分类总数 / 已选数
...
function applyModelPicker(){
    ...
    Object.entries(pickerState.selected).forEach(([id, sel]) => {
        if(!sel || isOpenSourceBlockedModel(id)) return;
        const cat = pickerState.category[id];
...
function modelProtocolSelectHtml(kind, index, model, item){
    ...
    return `<select class="model-protocol-select" aria-label="${escapeAttr(model || '模型')} 请求协议" title="该模型使用的协议，默认跟随平台全局协议" onchange="updateModelProtocol('${kind}', ${index}, this.value)">
...
function renderModels(kind){
    ...
    const models = (item?.[key] || []).filter(model => !isOpenSourceBlockedModel(model));
    if(!models.length){
        ...
        return `
            <div class="model-row${showProtocol ? ' has-protocol' : ''}">
                <div class="model-id-field">
                    ...
                    <input type="text" aria-label="${escapeAttr(kind === 'image' ? '生图' : kind === 'video' ? '视频' : 'LLM')} 模型名称" value="${escapeAttr(model)}" oninput="updateModel('${kind}', ${index}, this.value)">
                </div>
...
function msLoraTargetOptions(selected){
    ...
    return models.filter(model => model && !isOpenSourceBlockedModel(model)).map(model => `<option value="${escapeAttr(model)}" ${model === selected ? 'selected' : ''}>${escapeHtml(model)}</option>`).join('');
}
        ...
        return `
            <div class="lora-row">
                <label class="lora-field">
                    ...
                    <input type="text" aria-label="LoRA ID" value="${escapeAttr(lora.id || '')}" placeholder="${escapeAttr(tr('api.loraIdPlaceholder'))}" oninput="updateMsLora(${index}, 'id', this.value)">
                </label>
                <label class="lora-field">
                    <span>${tr('api.loraTargetModel')}</span>
                    <select aria-label="LoRA 目标模型" onchange="updateMsLora(${index}, 'target_model', this.value)">${msLoraTargetOptions(target)}</select>
                </label>
                <label class="lora-field">
                    <span>${tr('api.loraDefaultStrength')}</span>
                    <input type="number" aria-label="LoRA 默认强度" min="0" max="2" step="0.05" value="${strength}" oninput="updateMsLora(${index}, 'strength', this.value)">
                </label>
...
async function saveRhKeyOnly(kind){
    const item = provider();
    if(!item || item.id !== 'runninghub') return;
    if(kind === 'wallet' && OPEN_SOURCE_NO_BILLING) return;
...
function updateModel(kind, index, value){
    ...
    if(isOpenSourceBlockedModel(newName)){
        item[key][index] = '';
        setStatus('该模型在开源版本中不可用。');
        renderModels(kind);
        return;
    }
...
async function loadProviders(){
    ...
    try {
        ...
        providers = (Array.isArray(data.providers) ? data.providers : [])
            .filter(item => !isOpenSourceBlockedProvider(item))
            .map(item => sanitizeOpenSourceProvider(item));
        selectedId = sortedProviders()[0]?.id || '';
        renderEditor();
        // Keep the first screen on explicit provider configuration. Legacy
        // recommendation data/handlers stay in place for imports, while the
        // user-facing template view remains disabled in this build.
        syncRecommendView();
        renderRecommendApi();
        setStatus('');
    } catch(err) {
...
async function saveProviders(){
    syncEditor();
    // Drop stale legacy entries before serializing. This is intentionally a
    // client-side guard only; the API validates the same policy server-side.
    providers = (providers || [])
        .filter(item => !isOpenSourceBlockedProvider(item))
        .map(item => sanitizeOpenSourceProvider(item));
