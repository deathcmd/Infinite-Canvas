...
/*
 * The director desk is a wide authoring surface, not a portrait video card.
 * Keep the defaults in one place so rendering, first-viewport framing and the
 * resize affordance all agree.  A stage-host node may still be resized by the
 * user; the default applies when saved dimensions are absent, while the small
 * desktop floor keeps the four-column editor usable during a resize.
 */
const CANVAS_DIRECTOR_STAGE_SIZE = Object.freeze({w:1280, h:720});
const CANVAS_DIRECTOR_STAGE_MIN_SIZE = Object.freeze({w:960, h:520});
const CANVAS_LTX_DIRECTOR_SIZE = Object.freeze({w:1280, h:760});
...
window.addEventListener('message', event => {
    ...
    if(event.data?.type === 'providers-changed' || event.data?.type === 'workflows-changed' || event.data?.type === 'comfy-instances-changed'){
        refreshCanvasConfigFromSettings().catch(err => {
            console.error('canvas config refresh failed', err);
            setStatus(langIsEn() ? 'Settings refresh failed' : '设置刷新失败');
        });
    }
    if(event.data?.type === 'canvas-focus'){
        ...
        if(canvas) syncRemoteCanvasNow().catch(err => {
            console.error('canvas sync failed', err);
            setStatus(langIsEn() ? 'Sync failed' : '同步失败');
        });
    }
});
...
const canvasDeleteSelectedBtn = document.getElementById('canvasDeleteSelectedBtn');
...
// Connection motion is intentionally injected here instead of extending the
// shared canvas stylesheet. The editor can be opened by older cached shells,
// so keeping the rules beside the SVG renderer makes the animation self
// contained and lets us de-duplicate the style element on hot reloads.
const CANVAS_LINK_MOTION_STYLE_ID = 'canvas-link-motion-styles';
function ensureCanvasLinkMotionStyles(){
    if(typeof document === 'undefined' || !document.head) return;
    if(document.getElementById(CANVAS_LINK_MOTION_STYLE_ID)) return;
    const style = document.createElement('style');
    style.id = CANVAS_LINK_MOTION_STYLE_ID;
    style.textContent = `
@keyframes canvas-link-flow-run { from { stroke-dashoffset: 0; } to { stroke-dashoffset: -48; } }
@keyframes canvas-link-core-run { from { stroke-dashoffset: 0; } to { stroke-dashoffset: -96; } }
@keyframes canvas-link-fluid-glow {
    0%,100% { opacity:.09; stroke-width:8px; }
    50% { opacity:.2; stroke-width:10px; }
}
@keyframes canvas-link-fluid-body {
    0%,100% { opacity:.72; }
    45% { opacity:.98; }
    58% { opacity:.84; }
}
@keyframes canvas-link-fluid-core {
    0%,100% { opacity:.26; }
    48% { opacity:.68; }
    62% { opacity:.4; }
}
@keyframes canvas-link-spark-halo {
    0%,100% { opacity:.16; transform:scale(.68); }
    42% { opacity:.54; transform:scale(1.12); }
    58% { opacity:.28; transform:scale(.86); }
}
@keyframes canvas-link-spark-core {
    0%,100% { opacity:.54; transform:scale(.76); }
    44% { opacity:1; transform:scale(1.16); }
    62% { opacity:.7; transform:scale(.9); }
}
@keyframes canvas-link-spark-flare {
    0%,100% { opacity:.3; transform:scale(.52); }
    38% { opacity:1; transform:scale(1.34); }
    56% { opacity:.5; transform:scale(.8); }
}
@keyframes canvas-link-spark-streak {
    0%,100% { opacity:.22; transform:scaleX(.56); }
    36% { opacity:.9; transform:scaleX(1.12); }
    58% { opacity:.42; transform:scaleX(.82); }
}
@keyframes canvas-link-spark-sheen {
    0%,100% { opacity:.16; transform:scaleX(.72); }
    50% { opacity:.58; transform:scaleX(1.08); }
}
@keyframes canvas-link-spark-star {
    0%,100% { opacity:.18; transform:scale(.5); }
    38% { opacity:.95; transform:scale(1.18); }
    56% { opacity:.36; transform:scale(.78); }
}
@keyframes canvas-link-flow-breathe {
    0%,100% { opacity:var(--canvas-link-glow-min,.09); }
    50% { opacity:var(--canvas-link-glow-max,.2); }
}
@keyframes canvas-link-temp-run { from { stroke-dashoffset: 0; } to { stroke-dashoffset: -30; } }
@keyframes canvas-link-knife-run { from { stroke-dashoffset: 0; } to { stroke-dashoffset: -26; } }
#links .link-base { transition: stroke .16s ease, opacity .16s ease, filter .16s ease; }
#links .link-base.link-hovered { stroke:var(--accent,#0f766e); opacity:1; }
#links .link-base:not(.link-active) { stroke:rgba(15,118,110,.52); }
#links .link-motion-glow,
#links .link-motion-flow,
#links .link-motion-core { fill:none; pointer-events:none; vector-effect:non-scaling-stroke; }
#links .link-motion-glow {
    stroke:url(#canvas-link-gradient);
    stroke-width:9;
    ...
    opacity:.14;
    filter:drop-shadow(0 0 5px rgba(20,184,166,.52)) blur(.35px);
    animation:canvas-link-fluid-glow var(--canvas-link-breathe-duration,2.6s) ease-in-out infinite;
    animation-delay:var(--canvas-link-breathe-delay,0s);
}
#links .link-motion-flow {
    stroke:url(#canvas-link-gradient);
    stroke-width:3.8;
    ...
    stroke-dashoffset:0 !important;
    opacity:.96;
    filter:drop-shadow(0 0 2.5px rgba(20,184,166,.7));
    animation:canvas-link-fluid-body var(--canvas-link-duration,5.2s) ease-in-out infinite;
    animation-delay:var(--canvas-link-delay,0s);
}
#links .link-motion-core {
    stroke:#e6fffb;
    stroke-width:1.15;
    stroke-linecap:round;
    stroke-linejoin:round;
    ...
    stroke-dashoffset:0 !important;
    opacity:.46;
    filter:drop-shadow(0 0 2.5px rgba(20,184,166,.72));
    animation:canvas-link-fluid-core var(--canvas-link-duration,5.2s) ease-in-out infinite;
    animation-delay:calc(var(--canvas-link-delay,0s) - .12s);
}
#links .link-motion-spark {
    pointer-events:none;
    transform-box:fill-box;
    transform-origin:center;
    mix-blend-mode:screen;
}
#links .link-motion-spark-sheen {
    pointer-events:none;
    fill:var(--canvas-link-spark-color,#2dd4bf);
    opacity:.42;
    filter:drop-shadow(0 0 6px var(--canvas-link-spark-shadow,rgba(20,184,166,.76))) blur(.8px);
    transform-box:fill-box;
    transform-origin:center;
    animation:canvas-link-spark-sheen 2.4s ease-in-out infinite;
}
#links .link-motion-spark-streak {
    fill:var(--canvas-link-spark-color,#f8ffff);
    filter:drop-shadow(0 0 4px var(--canvas-link-spark-shadow,rgba(20,184,166,.9)));
    transform-box:fill-box;
    transform-origin:center;
    animation:canvas-link-spark-streak 2s ease-in-out infinite;
}
/* Defensive guard for tabs that still hold pre-fluid9 round/asterisk spark
   markup (including the older pre-fluid8 variant). Current links use only
   elongated ellipse highlights; endpoint and delete affordance circles are
   outside this group. */
#links .link-motion-spark > circle,
#links .link-motion-spark > path { display:none !important; }
#links .link-motion-spark-halo {
    fill:var(--canvas-link-spark-color,#d9fffb);
    filter:drop-shadow(0 0 5px var(--canvas-link-spark-shadow,rgba(20,184,166,.84))) blur(.3px);
    animation:canvas-link-spark-halo 1.1s ease-in-out infinite;
}
#links .link-motion-spark-core {
    fill:var(--canvas-link-spark-color,#d9fffb);
    filter:drop-shadow(0 0 4px var(--canvas-link-spark-shadow,rgba(20,184,166,.9)));
    animation:canvas-link-spark-core 1.1s ease-in-out infinite;
}
#links .link-motion-spark-flare {
    fill:#fff;
    filter:drop-shadow(0 0 3px #fff);
    animation:canvas-link-spark-flare .72s ease-in-out infinite;
}
#links .link-motion-spark-star {
    pointer-events:none;
    stroke:#fff;
    stroke-linecap:round;
    ...
    transform-box:fill-box;
    transform-origin:center;
    animation:canvas-link-spark-star 1.05s ease-in-out infinite;
}
#links .link-motion-glow.link-active {
    --canvas-link-glow-min:.14;
    --canvas-link-glow-max:.32;
    opacity:.22;
    stroke:url(#canvas-link-gradient);
}
#links .link-motion-flow.link-active { opacity:1; stroke:url(#canvas-link-gradient); stroke-width:4.1; }
#links .link-motion-glow.link-hovered {
    --canvas-link-glow-min:.18;
    --canvas-link-glow-max:.38;
    opacity:.27;
    stroke:url(#canvas-link-gradient);
}
#links .link-motion-flow.link-hovered { opacity:1; stroke:url(#canvas-link-gradient); stroke-width:4.1; }
#links .link-motion-flow.link-active.link-hovered { stroke:var(--accent,#14b8a6); stroke-width:3.35; }
#links .link-motion-core.link-active,
#links .link-motion-core.link-hovered { stroke:#d9fffb; opacity:1; }
#links .link-motion-temp {
    stroke:var(--strong,#111827);
    stroke-width:2.6;
    stroke-dasharray:8 8;
    opacity:.9;
    animation:canvas-link-temp-run var(--canvas-link-duration,.9s) linear infinite;
    animation-delay:var(--canvas-link-delay,0s);
}
#links .link-motion-glow.link-motion-temp {
    --canvas-link-glow-min:.12;
    --canvas-link-glow-max:.24;
    stroke:var(--accent,#14b8a6);
    opacity:.15;
    stroke-width:11;
}
#links .link-motion-core.link-motion-temp {
    stroke:var(--strong,#111827);
    stroke-dasharray:8 18;
    opacity:.9;
    animation-duration:var(--canvas-link-duration,.9s);
}
#links .link-motion-spark.link-motion-temp { display:none; }
#links .link.knife-trail.link-motion-knife {
    animation:canvas-link-knife-run var(--canvas-link-duration,.72s) linear infinite;
    animation-delay:var(--canvas-link-delay,0s);
}
@media (prefers-reduced-motion: reduce) {
    ...
    #links .link-motion-spark-sheen,
    #links .link-motion-temp,
    #links .link.knife-trail.link-motion-knife { animation:none !important; }
    #links .link-motion-flow { stroke-dasharray:none; }
    #links .link-motion-core { stroke-dasharray:none; }
    #links .link-motion-glow { stroke-dasharray:none; opacity:.08; }
    #links .link-motion-flow.link-active,
    #links .link-motion-flow.link-hovered { stroke-dasharray:none; }
    #links .link-motion-spark { display:none !important; }
    #links linearGradient animateTransform { display:none !important; }
}
`;
...
// A freshly opened canvas can legitimately have a zeroed viewport from the
// server.  On a phone that leaves the first node (especially the 3D director
// desk, which is intentionally wide on desktop) outside the visible area.
// Keep this one-shot flag separate from the persisted viewport so an explicit
// user pan/zoom is never overwritten on reload.
let initialCanvasViewportPending = true;
let dragNode = null;
let dragBoard = null;
...
let undoStack = [];
// Redo belongs to the currently opened canvas only.  Keeping a separate stack
// prevents an edit made after undo from resurrecting an obsolete branch, and
// the history is reset whenever a different document is loaded below.
let redoStack = [];
const UNDO_MAX = 30;
...
// Video models are intentionally not seeded with a vendor catalog.  A clean
// checkout opens in local/mock mode; models become available only when the
// user configures a provider (or types a model for a local OpenAI-compatible
// endpoint).  This keeps the workflow model-agnostic and avoids implying a
// paid service is part of the open-source default.
...
function isOpenSourceBlockedModel(value){
    const text = String(value || '').trim();
    return /grok/i.test(text) || OPEN_SOURCE_BILLING_MODEL_RE.test(text);
}
function uniqueModels(list){
    const seen = new Set();
    return list.map(item => String(item || '').trim()).filter(item => {
        if(!item || isOpenSourceBlockedModel(item) || seen.has(item)) return false;
        seen.add(item);
        return true;
    });
}
function hasLocalViewport(canvasId){
    const item = loadLocalViewportMap()[canvasId || ''];
    return Boolean(item && typeof item === 'object'
        && Number.isFinite(Number(item.x))
        && Number.isFinite(Number(item.y))
        && Number.isFinite(Number(item.scale)));
}
function isDefaultCanvasViewport(value){
    const item = value && typeof value === 'object' ? value : {};
    const x = Number(item.x || 0);
    const y = Number(item.y || 0);
    const scale = Number(item.scale == null ? 1 : item.scale);
    return Number.isFinite(x) && Number.isFinite(y) && Number.isFinite(scale)
        && Math.abs(x) < 0.5 && Math.abs(y) < 0.5 && Math.abs(scale - 1) < 0.01;
}
function focusInitialCanvasViewport(){
    if(!initialCanvasViewportPending || !canvas || !nodes.length || !board) return;
    if(document.body.classList.contains('vwf-desk-on')) return;
    if(hasLocalViewport(canvas.id) || !isDefaultCanvasViewport(canvas.viewport)){
        initialCanvasViewportPending = false;
        return;
    }

    /*
     * A freshly-created classic canvas used to open at world (0, 0) on the
     * desktop.  That is technically a valid viewport, but most workflows are
     * arranged around x=100..1800 and the first screen consequently showed
     * clipped assets (or a single oversized video node).  Focus the connected
     * working set once, as a presentation-only operation.  We deliberately
     * calculate a new viewport rather than changing any node coordinates or
     * persisting a different canvas payload.  A user/local viewport always
     * wins through the guard above.
     */
    const desktop = (board.clientWidth || window.innerWidth) >= 900;
    if(desktop){
        initialCanvasViewportPending = false;
        // Node dimensions settle after render (the stage and image previews
        // can mount asynchronously), so defer measurement by two frames.
        requestAnimationFrame(() => requestAnimationFrame(() => {
            if(!canvas || !board || !nodes.length) return;
            const boardRect = board.getBoundingClientRect();
            if(!boardRect.width || !boardRect.height) return;
            const measured = nodes.map(node => ({node, rect:estimatedNodeRect(node)}))
                .filter(item => {
                    const r = item.rect || {};
                    return Number.isFinite(Number(r.x)) && Number.isFinite(Number(r.y))
                        && Number.isFinite(Number(r.w)) && Number.isFinite(Number(r.h))
                        && Number(r.w) > 0 && Number(r.h) > 0;
                });
            if(!measured.length) return;

            // Prefer nodes that participate in the graph and live in the
            // normal working area.  Legacy asset imports can be hundreds of
            // pixels to the left, while long video/desk nodes can extend far
            // below the fold; neither should make the opening composition
            // unreadably small.  Fall back to all measured nodes for unusual
            // or disconnected canvases.
            const connectedIds = new Set();
            (connections || []).forEach(link => {
                if(link?.from != null) connectedIds.add(String(link.from));
                if(link?.to != null) connectedIds.add(String(link.to));
            });
            const inWorkingArea = item => {
                const r = item.rect;
                return Number(r.x) >= -180 && Number(r.x) <= 1900 && Number(r.y) <= 980;
            };
            const connected = measured.filter(item =>
                inWorkingArea(item)
                && (item.node?.stageHost || connectedIds.has(String(item.node?.id)))
            );
            const working = measured.filter(inWorkingArea);
            const primary = connected.length >= 2 ? connected : (working.length ? working : measured);
            // Include one nearby left-hand context card when it is just
            // outside the connected flow.  This removes the distracting
            // half-card crop that older boards often produced at x≈-250,
            // without allowing a historical asset hundreds of pixels farther
            // left to shrink the entire opening view.
            const primaryMinX = Math.min(...primary.map(item => Number(item.rect.x)));
            const primaryTopY = Math.min(...primary.map(item => Number(item.rect.y)));
            const leftContext = measured.filter(item => {
                const r = item.rect;
                return Number(r.x) < primaryMinX
                    && Number(r.x) >= primaryMinX - 360
                    && Number(r.y) <= 980
                    && Number(r.y) <= primaryTopY + 620;
            });
            const framed = [...new Map([...primary, ...leftContext].map(item => [item.node?.id || item, item])).values()];

            const minX = Math.min(...framed.map(item => Number(item.rect.x)));
            const maxX = Math.max(...framed.map(item => Number(item.rect.x) + Number(item.rect.w)));
            const topY = Math.min(...framed.map(item => Number(item.rect.y)));
            // Keep a large media/desk card from forcing the entire workflow
            // into a thumbnail.  The full node remains reachable by panning.
            const maxVisibleBottom = Math.max(...framed.map(item => {
                const r = item.rect;
                return Number(r.y) + Math.min(Number(r.h), 620);
            }));
            const contentWidth = Math.max(1, maxX - minX);
            const contentHeight = Math.max(360, Math.min(700, maxVisibleBottom - topY));
            const horizontalPad = 96;
            const verticalPad = 72;
            const topbarBottom = document.querySelector('.topbar')?.getBoundingClientRect?.().bottom || 70;
            const safeTop = Math.max(92, Math.min(boardRect.height * .34, topbarBottom + 18));
            const safeBottom = Math.max(safeTop + 220, boardRect.height - 22);
            const availableWidth = Math.max(640, boardRect.width - horizontalPad);
            const availableHeight = Math.max(300, safeBottom - safeTop);
            const scale = Math.max(.52, Math.min(.82,
                availableWidth / (contentWidth + horizontalPad),
                availableHeight / (contentHeight + verticalPad)
            ));
    ...
    // On a narrow viewport focus the first stage/working node so the
    // responsive desk remains useful.  This is intentionally kept separate
    // from the desktop framing above.
    const target = nodes.find(item => item?.stageHost) || nodes.find(Boolean);
    if(!target) return;
    initialCanvasViewportPending = false;
    // Node dimensions settle after the workflow panel (and Three.js stage)
    // mounts.  Two frames avoid measuring the pre-mount 0px height.
    requestAnimationFrame(() => requestAnimationFrame(() => {
        if(!canvas || !target || !nodes.includes(target) || !board) return;
        const boardRect = board.getBoundingClientRect();
        const nodeEl = nodesEl.querySelector(`.node[data-id="${CSS.escape(target.id)}"]`);
        const fallback = defaultNodeSize(target.type, target);
        const width = Math.max(220, nodeEl?.offsetWidth || target.w || fallback.w || 260);
        const height = Math.max(96, nodeEl?.offsetHeight || target.h || fallback.h || 180);
        const topbarBottom = document.querySelector('.topbar')?.getBoundingClientRect?.().bottom || 96;
        const safeTop = Math.min(boardRect.height - 140, Math.max(96, topbarBottom + 16));
        const safeBottom = Math.max(safeTop + 160, boardRect.height - 18);
        const availableWidth = Math.max(120, boardRect.width - 28);
        const availableHeight = Math.max(180, safeBottom - safeTop);
        // Keep labels at a useful size while fitting the first node into the
        // unobscured part of a phone.  Responsive stage CSS normally makes
        // the desk fit at scale 1; the floor is a guard for legacy wide nodes
        // that were saved before that rule existed.
        const floor = target.stageHost ? 0.62 : 0.8;
        const scale = Math.max(floor, Math.min(1, availableWidth / width, availableHeight / height));
        const targetCenterY = safeTop + (safeBottom - safeTop) / 2;
        const cx = Number(target.x || 0) + width / 2;
        const cy = Number(target.y || 0) + height / 2;
        viewport.scale = scale;
        viewport.x = boardRect.width / 2 - cx * scale;
        viewport.y = targetCenterY - cy * scale;
        applyViewport();
        renderLinks();
        renderSelectionHub();
        scheduleViewportSave();
...
const OPEN_SOURCE_BLOCKED_PROVIDER_RE = /(?:grok|sub2api)/i;
function isOpenSourceBlockedProvider(provider){
    if(!provider) return false;
    if(OPEN_SOURCE_BLOCKED_PROVIDER_RE.test([provider.id, provider.name, provider.base_url].map(value => String(value || '')).join(' '))) return true;
    return OPEN_SOURCE_BILLING_MODEL_RE.test([provider.id, provider.name].map(value => String(value || '')).join(' '));
}
function sanitizeOpenSourceProviders(list){
    return (Array.isArray(list) ? list : [])
        .filter(provider => !isOpenSourceBlockedProvider(provider))
        .map(provider => {
            const item = {...provider};
            ['image_models', 'chat_models', 'video_models'].forEach(key => { item[key] = uniqueModels(item[key] || []); });
            if(item.model_names && typeof item.model_names === 'object'){
                item.model_names = Object.fromEntries(Object.entries(item.model_names).filter(([key, value]) => !isOpenSourceBlockedModel(key) && !isOpenSourceBlockedModel(value)));
            }
            if(item.model_protocols && typeof item.model_protocols === 'object'){
                item.model_protocols = Object.fromEntries(Object.entries(item.model_protocols).filter(([key]) => !isOpenSourceBlockedModel(key)));
            }
            return item;
        });
}
...
function imageApiProviders(){
    const providers = (apiProviders.length ? apiProviders : defaultApiProviders())
        .filter(p => !isOpenSourceBlockedProvider(p) && p.id !== 'modelscope' && p.enabled !== false && (p.image_models || []).some(model => !isOpenSourceBlockedModel(model)));
    return providers;
}
function midjourneyApiProviders(){
    return (apiProviders.length ? apiProviders : [])
        .filter(provider => !isOpenSourceBlockedProvider(provider) && provider.enabled !== false && (
...
function providerById(id){
    return (apiProviders.length ? apiProviders : defaultApiProviders()).find(p => !isOpenSourceBlockedProvider(p) && p.id === id) || imageApiProviders()[0] || defaultApiProviders()[0];
}
...
function chatApiProviders(){
    const providers = (apiProviders.length ? apiProviders : defaultApiProviders())
        .filter(p => !isOpenSourceBlockedProvider(p) && p.enabled !== false && (p.chat_models || []).some(model => !isOpenSourceBlockedModel(model)));
...
function providerChatModels(providerId){
    const provider = apiProviders.find(p => !isOpenSourceBlockedProvider(p) && p.id === providerId);
    return uniqueModels(provider?.chat_models || []);
}
function resolveImageProviderId(id){
    const raw = String(id || '').trim();
    if(window.VideoWorkflowSchema?.isLocalEngine?.(raw)) return raw;
    const providers = imageApiProviders();
    // Keep an explicitly unknown provider empty instead of silently routing
    // an operation to the first configured (possibly remote) provider.
    return providers.find(p => p.id === raw)?.id || (raw ? '' : providers[0]?.id || '');
}
...
function videoApiProviders(){
    const providers = (apiProviders.length ? apiProviders : [])
        // A provider is selectable even before it advertises a model list;
        // OpenAI-compatible/local endpoints commonly accept a user-entered
        // model id. Hiding those providers made the workflow vendor-bound.
        .filter(p => !isOpenSourceBlockedProvider(p) && p.id !== 'modelscope' && p.enabled !== false);
    return providers;
}
function localVideoProviders(){
    return [
        {id:'comfyui', name: tr('videoWf.engineComfy')},
        {id:'openai_local', name: tr('videoWf.engineOpenAILocal')}
    ];
}
function videoProviderCatalog(){
    const api = videoApiProviders();
    const seen = new Set(api.map(p => String(p.id || '')));
    return localVideoProviders().filter(p => !seen.has(p.id)).concat(api);
}
function resolveVideoProviderId(id){
    const raw = String(id || '').trim();
    if(!raw) return '';
    if(window.VideoWorkflowSchema?.isLocalEngine?.(raw)) return raw;
    const hit = videoProviderCatalog().find(p => p.id === raw);
    return hit ? hit.id : '';
}
function videoProviderOptions(selectedId){
    const selected = resolveVideoProviderId(selectedId);
    const unset = `<option value="" ${selected ? '' : 'selected'}>${escapeHtml(tr('videoWf.engineUnset'))}</option>`;
    return unset + videoProviderCatalog().map(provider => `<option value="${escapeHtml(provider.id)}" ${provider.id === selected ? 'selected' : ''}>${escapeHtml(provider.name || provider.id)}</option>`).join('');
}
function providerVideoModels(providerId){
    // 不走 providerById（会 fallback 到第一个 provider，造成串台），直接查精确匹配
    if(window.VideoWorkflowSchema?.isLocalEngine?.(providerId)) return [];
    const provider = apiProviders.find(p => !isOpenSourceBlockedProvider(p) && p.id === providerId);
...
function sanitizeVideoNodeProviderModel(node){
    ...
    // Selecting an endpoint is not consent to use its first catalog entry.
    // Keep a saved custom id, otherwise leave the model empty until the user
    // explicitly chooses one (or enters a local/OpenAI-compatible id).
    if(!node.apiProvider) node.model = '';
    else if(node.model) node.model = isOpenSourceBlockedModel(node.model) ? '' : String(node.model).trim();
}
function videoModelOptions(selectedModel, providerId){
    const models = providerVideoModels(providerId);
    const manualOption = `<option value="__manual__">手动输入模型…</option>`;
    if(!models.length){
        const selected = String(selectedModel || '');
        if(window.VideoWorkflowSchema?.isLocalEngine?.(providerId) || !providerId){
            return `<option value="${escapeHtml(selected)}" selected>${escapeHtml(selected || tr('videoWf.engineModel'))}</option>${manualOption}`;
        }
        return `<option value="" disabled selected>${tr('canvas.noModelsHint') || '暂无模型，可手动输入'}</option>${manualOption}`;
    }
    const selected = isOpenSourceBlockedModel(selectedModel) ? '' : String(selectedModel || '').trim();
    const options = uniqueModels([selected, ...models]).filter(Boolean).map(model => `<option value="${escapeHtml(model)}" ${model === selected ? 'selected' : ''}>${escapeHtml(model)}</option>`).join('');
    const placeholder = `<option value="" ${selected ? '' : 'selected'}>模型</option>`;
    return placeholder + options + manualOption;
}
...
function modelscopeImageModels(selected = ''){
    const provider = (apiProviders.length ? apiProviders : []).find(p => !isOpenSourceBlockedProvider(p) && p.id === 'modelscope');
    return uniqueModels([
        selected,
...
function allChatModels(){
    const providerModels = chatApiProviders().flatMap(p => (p.chat_models || []).filter(model => !isOpenSourceBlockedModel(model)));
...
function estimatedNodeRect(n){
    ...
    const size = defaultNodeSize(n.type, n);
...
function renderMinimap(){
    ...
    // Keep a visible, mouse/touch-friendly affordance for the same multi-node
    // deletion operation that is available through Delete/Backspace.  The
    // button lives beside the minimap so it remains reachable when node cards
    // are zoomed or partially off-screen.
    canvasDeleteSelectedBtn?.toggleAttribute('hidden', selected.size === 0);
...
// Presentation bridge for the shared LibTV view dock.  Keep the existing
// fit implementation and persistence semantics; the skin only dispatches a
// namespaced event because editor helpers are lexical in some builds.
window.addEventListener('libtv-fit-view', () => {
    try { fitAllNodesViewport(); } catch (_) {}
});
...
async function saveCanvas(){
    // A timeout handle is no longer pending once this callback starts.  Clear
    // it before the early guards so remote-update arbitration does not mistake
    // a completed timer for a still-dirty local save.
    saveTimer = null;
    if(!canvas || applyingRemoteCanvas) return;
    if(savingCanvasNow){
        saveCanvasAgain = true;
    ...
    try {
        const res = await fetch(`/api/canvases/${canvas.id}`, {
            ...
            body:JSON.stringify({
                ...
                viewport,
                logs:canvas.logs || [],
                // Keep canvas-level settings in the save payload.  The
                // backend also preserves settings for older clients that do
                // not send this field, but current saves should round-trip
...
async function loadConfig(){
    ...
    try {
        ...
        const safeConfigModels = values => uniqueModels(Array.isArray(values) ? values : []);
        const safeImageModels = safeConfigModels(cfg.image_models);
        const safeChatModels = safeConfigModels(cfg.chat_models);
        const safeMsChatModels = safeConfigModels(cfg.ms_chat_models);
        imageModels = safeImageModels.length ? safeImageModels : uniqueModels(imageModels);
        chatModels = safeChatModels.length ? safeChatModels : uniqueModels(chatModels);
        msChatModels = safeMsChatModels.length ? safeMsChatModels : uniqueModels(msChatModels);
        comfyBackendCount = Math.max(1, (cfg.comfy_instances || []).length || 1);
        apiProviders = sanitizeOpenSourceProviders(
            Array.isArray(cfg.api_providers) && cfg.api_providers.length ? cfg.api_providers : defaultApiProviders()
        );
        // Ignore the legacy aggregate video list.  Only models explicitly
        // attached to a configured provider should appear in the workflow;
        // a clean checkout therefore remains local/mock by default.
        videoModels = uniqueModels(apiProviders.flatMap(provider => provider?.video_models || []));
        models.nano = imageModels.find(m => m.toLowerCase().includes('nano')) || 'nano-banana-pro';
        const configuredImageModel = String(cfg.image_model || '').trim();
        models.gpt = imageModels.find(m => !m.toLowerCase().includes('nano'))
            || (!isOpenSourceBlockedModel(configuredImageModel) ? configuredImageModel : '')
            || 'gpt-image-2';
        try {
        ...
        // RunningHub workflow definitions are fetched lazily when a user
        // opens/uses a RunningHub node. A canvas load must remain local and
        // deterministic; prefetching every configured remote workflow caused
        // unsolicited network calls (and noisy 404s) even when RH was never
        // selected.
    } catch(e) {
        apiProviders = sanitizeOpenSourceProviders(defaultApiProviders());
        videoModels = [];
    }
}
...
async function createCanvas(){
    ...
    try {
        ...
        initialCanvasViewportPending = true;
        resetTransientRunState(nodes);
        sanitizeConnections();
        selected.clear();
        setCanvasMode(true);
        render();
        focusInitialCanvasViewport();
        setStatus('Saved');
        setCreateMode(false);
        await loadCanvasList(false);
        renderCanvasList();
...
function openSmartCanvasPage(id){
    if(!id) return;
    window.location.href = `/static/smart-canvas.html?id=${encodeURIComponent(id)}&v=2026.09.01.fluid13`;
}
...
async function setCanvasIcon(id, icon, event){
    event?.preventDefault();
    event?.stopPropagation();
    const nextIcon = icon || 'layers';
    closeCanvasMetaPopover();
    // Metadata edits must never use the full canvas PUT.  That endpoint
    // replaces nodes/connections/logs; a manager-only icon change otherwise
    // sends the legacy default `logs: []` and silently erases generation
    // history (and can overwrite a newer editor save).  The dedicated
    // metadata route performs a narrow, atomic patch instead.
    await patchCanvasMeta(id, {icon: nextIcon});
}
...
async function setCanvasTitle(id, title){
    ...
    if(!nextTitle) return;
    // Use the narrow metadata endpoint for the same reason as icon edits:
    // renaming from the list is independent of the editor document and must
    // not replace its logs, settings, nodes, or connections.
    await patchCanvasMeta(id, {title: nextTitle});
...
function applyRemoteCanvasData(remote){
    ...
    try {
        resetCascadeRuntimeState();
        // A remote snapshot is a new document baseline.  Keeping local undo
        // entries here could restore stale nodes after another tab's edit.
        resetCanvasHistory();
...
function handleCanvasUpdatedMessage(data){
    ...
    // Do not clear the local dirty bit when another tab announces an update.
    // The current PUT carries `base_updated_at`; if it is stale the server
    // returns 409 and saveCanvas retries the local document against the newer
    // base.  Clearing the bit here used to make that 409 look like a clean
    // canvas and apply the remote snapshot, silently dropping the user's edit.
    const hasPendingLocalSave = Boolean(localCanvasDirty || saveTimer || savingCanvasNow || saveCanvasAgain);
    clearTimeout(remoteSyncTimer);
    remoteSyncTimer = setTimeout(syncRemoteCanvasNow, hasPendingLocalSave ? (savingCanvasNow ? 700 : 1000) : 120);
    setStatus(hasPendingLocalSave ? 'Saving...' : 'Syncing...');
}
...
function addPromptNode(point, opts={}){
    const p = point || defaultPoint(0, 0);
    return addNode({id:uid('prompt'), type:'prompt', x:p.x, y:p.y, text:opts.text || ''});
}
...
function addVideoNode(point, opts={}){
    const p = point || defaultPoint(160, 0);
    // New nodes start unconfigured so the first run is always a local/mock
    // preview. Users can choose any configured provider/model explicitly.
    const providerId = '';
    const models = [];
    return addNode({
        id:uid('vid'),
        ...
        multimodal:false,
        tempShLinks:[],
        videoWorkflow: window.VideoWorkflowSchema ? window.VideoWorkflowSchema.emptyWorkflow() : null,
        inputs:[],
        running:false,
        // Set stage flags before addNode() renders.  A stage node is rendered
        // synchronously, so assigning these flags after addVideoNode() would
        // leave the first paint looking like a normal video card and would
        // miss the automatic desk-open handoff.  Keep normal video nodes
        // backward-compatible by omitting false-valued fields.
        ...(opts.stageHost ? {
            stageHost:true,
            // Persist the wide desk dimensions for newly-created nodes.  The
            // render helper also supplies these values for legacy nodes that
            // predate the explicit size fields.
            w:Number(opts.w) > 0 ? Number(opts.w) : CANVAS_DIRECTOR_STAGE_SIZE.w,
            h:Number(opts.h) > 0 ? Number(opts.h) : CANVAS_DIRECTOR_STAGE_SIZE.h
        } : {}),
        ...(opts.openDirectorDesk ? {openDirectorDesk:true} : {})
    });
}

// Stage controls can open a second page immediately after an edit while the
// canvas save is still in flight.  Wait for the current request to settle,
// then flush one final dirty pass, with a bounded wait so a stalled provider
// cannot trap the user on the canvas page.
async function flushCanvasSaveForStage(){
    clearTimeout(saveTimer);
    saveTimer = 0;
    // A stage opener needs an explicit outcome.  Returning `true` for an
    // empty canvas keeps the local/fixture mounts navigable, while every
    // connected dirty/saving state is checked below so a rejected PUT cannot
    // silently hand the standalone page an older server snapshot.
    if(!canvas || applyingRemoteCanvas) return true;
    const waitForIdle = async (limitMs=5000) => {
        const started = Date.now();
        while(savingCanvasNow && Date.now() - started < limitMs){
            await new Promise(resolve => setTimeout(resolve, 25));
        }
    };
    await waitForIdle();
    if(savingCanvasNow) return false;
    if(localCanvasDirty || saveCanvasAgain) await saveCanvas();
    await waitForIdle();
    if(savingCanvasNow) return false;
    if(saveCanvasAgain && !savingCanvasNow){
        await saveCanvas();
        await waitForIdle();
    }
    return Boolean(canvas && !applyingRemoteCanvas && !savingCanvasNow && !localCanvasDirty && !saveCanvasAgain);
}
function addAssetCardNode(point, opts={}){
    const p = point || defaultPoint(80, 40);
    const kind = opts.assetKind || 'character';
    return addNode({
        id:uid('asset'),
        type:'assetCard',
        x:p.x,
        ...
        notes:opts.notes || '',
        url:'',
        panorama:Boolean(opts.panorama) || kind === 'panorama',
        variants:{},
        inputs:[]
    });
}
function addCharacterCardNode(point, opts={}){
    return addAssetCardNode(point, {assetKind:'character', ...opts});
}
function addSceneCardNode(point, opts={}){
    return addAssetCardNode(point, {assetKind:'scene', ...opts});
}
function addDirectorStageNode(point){
    // Pass the flags into addVideoNode so they are present during its
    // synchronous render/mount cycle (see mountCanvasVideoWorkflowPanel).
    return addVideoNode(point || defaultPoint(0, 40), {
        stageHost: true,
        openDirectorDesk: true
    });
}
...
function addLTXDirectorNode(point){
    ...
    return addNode({
        ...
        x:p.x,
        y:p.y,
        w:CANVAS_LTX_DIRECTOR_SIZE.w,
        h:CANVAS_LTX_DIRECTOR_SIZE.h,
        globalPrompt:'',
        durationFrames:120,
        durationSeconds:5,
        frameRate:24,
        ...
        ltxSegments:[],
        ltxSelectedSegId:'',
        ltxStage: window.VideoWorkflowSchema ? window.VideoWorkflowSchema.defaultStage() : {actors:[], camera:{x:0.5,y:0.82,facing:0}, layoutUrl:''},
        inputs:[],
        running:false
    });
}
...
function linkCreateOptions(state){
    ...
    if(state.originKind === 'out'){
        if(node.type === 'video'){
            return [
                {type:'video', label:tr('canvas.kit.nextShot'), icon:'clapperboard'},
                {type:'output', label:'Output', icon:'circle-dot'}
            ];
        }
        if(['image','prompt','assetCard','loop','group','promptGroup','llm','output'].includes(node.type)){
            return [
                ...
                {type:'stage3d', label:tr('canvas.kit.stage3d'), icon:'box'},
    ...
    if(CANVAS_GENERATOR_TYPES.includes(node.type) || node.type === 'llm'){
        return [
            {type:'image', label:tr('canvas.kit.image'), icon:'image'},
            {type:'character', label:tr('canvas.kit.character'), icon:'user'},
            {type:'scene', label:tr('canvas.kit.scene'), icon:'map'},
            {type:'stage3d', label:tr('canvas.kit.stage3d'), icon:'box'},
            {type:'audio', label:tr('canvas.kit.audio'), icon:'music'},
            {type:'prompt', label:tr('canvas.kit.text'), icon:'text-cursor-input'},
...
function createLinkedNode(type){
    ...
    if(canConnect(fromId, toId) && !connections.some(c => c.from === fromId && c.to === toId)){
        ...
        if(fromNode?.type === 'video' && toNode?.type === 'video' && window.VideoWorkflowSchema){
            const wf = window.VideoWorkflowSchema.normalize(toNode.videoWorkflow || window.VideoWorkflowSchema.emptyWorkflow());
            wf.continuePrev.enabled = true;
            wf.continuePrev.useLastFrame = true;
            window.VideoWorkflowSchema.writeHost(toNode, wf);
        }
...
function createNodeByType(type, point){
    ...
    if(type === 'character' || type === 'assetCard') return addCharacterCardNode(point);
    if(type === 'scene') return addSceneCardNode(point);
    if(type === 'stage3d') return addDirectorStageNode(point);
...
function menuAdd(type){
    closeCreateMenu();
    createNodeByType(type, menuPoint);
}
function parseStarterSeedKinds(raw){
    return String(raw || '')
        .split(/[,\s]+/)
...
function starterKindWidth(kind){
    if(kind === 'stage3d') return CANVAS_DIRECTOR_STAGE_SIZE.w + 40;
    if(kind === 'video') return 740;
    if(kind === 'prompt') return 330;
    return 310;
}
function seedStarterNodes(kinds, originPoint){
    const unique = parseStarterSeedKinds(Array.isArray(kinds) ? kinds.join(',') : kinds);
    if(!unique.length) return [];
    const origin = originPoint || defaultPoint(0, 0);
    const hasStage = unique.includes('stage3d');
    const others = unique.filter(kind => kind !== 'stage3d');
    const created = [];
    addNodeSilent = true;
    try {
        const gap = 28;
        const widths = others.map(starterKindWidth);
        const total = widths.reduce((sum, w) => sum + w, 0) + Math.max(0, others.length - 1) * gap;
        let cursor = origin.x - total / 2;
        others.forEach((kind, i) => {
            created.push(createNodeByType(kind, {
                x:cursor,
                y:origin.y + (hasStage ? -260 : 0)
            }));
            cursor += (widths[i] || 310) + gap;
        });
        if(hasStage){
            created.push(createNodeByType('stage3d', {x:origin.x - 360, y:origin.y + (others.length ? 80 : 0)}));
        }
    } finally {
        addNodeSilent = false;
    }
    if(created.filter(Boolean).length){
        render();
        scheduleSave();
    }
    return created.filter(Boolean);
}
function consumeStarterSeed(){
    try {
        consumeSeedPlan();
        ...
        seedStarterNodes(seed, defaultPoint(0, 0));
    } catch(e) {
        console.error(e);
    }
}
function consumeSeedPlan(){
    let raw = '';
    try { raw = sessionStorage.getItem('canvasLab.seedPlan') || ''; } catch(e) { raw = ''; }
    if(!raw) return;
    try { sessionStorage.removeItem('canvasLab.seedPlan'); } catch(e) {}
    if((nodes || []).length) return;
    let plan = null;
    try { plan = JSON.parse(raw); } catch(e) { return; }
    applySeedPlan(plan);
}
function applySeedPlan(plan){
    if(!plan || typeof plan !== 'object') return;
    const origin = defaultPoint(0, 0);
    const characters = Array.isArray(plan.characters) ? plan.characters.slice(0, 8) : [];
    const scenes = Array.isArray(plan.scenes) ? plan.scenes.slice(0, 6) : [];
    const shots = Array.isArray(plan.shots) ? plan.shots.slice(0, 8) : [];
    const aspect = String(plan.aspect || '16:9');
    addNodeSilent = true;
    const created = [];
    try {
        characters.forEach((item, i) => {
            created.push(addCharacterCardNode({ x: origin.x - 520 + i * 280, y: origin.y - 320 }, { name:item.name || '', notes:item.notes || '' }));
        });
        scenes.forEach((item, i) => {
            created.push(addSceneCardNode({ x: origin.x - 520 + i * 280, y: origin.y - 40 }, { name:item.name || '', notes:item.notes || '' }));
        });
        shots.forEach((shot, i) => {
            const text = [shot.title || `分镜 ${i + 1}`, shot.text || ''].filter(Boolean).join('\n');
            const promptNode = addPromptNode({ x: origin.x - 420, y: origin.y + 260 + i * 240 }, { text });
            const videoNode = addVideoNode({ x: origin.x - 40, y: origin.y + 250 + i * 240 });
            if(videoNode) videoNode.aspectRatio = aspect;
            if(promptNode && videoNode) connections.push({ id:uid('c'), from:promptNode.id, to:videoNode.id });
            created.push(promptNode, videoNode);
        });
        if(plan.includeStage !== false){
            created.push(addDirectorStageNode({ x: origin.x + 420, y: origin.y - 40 }));
        }
    } finally {
        addNodeSilent = false;
    }
...
function closeToolbarMore(){
    document.getElementById('toolbarMore')?.classList.remove('open');
}
...
function nodeHasLiveMedia(node){
    return Boolean(node?.url) && ['video','audio'].includes(mediaKindForNode(node)) && (node.type === 'image' || node.type === 'audio');
}
...
// An expanded XYQ workflow desk is temporarily reparented from its canvas
// node into <body> so it can cover the full viewport. The normal canvas
// renderer only owns children under #nodes, so a full render (for example
// after adding another node) would otherwise leave the old desk in <body>
// and create a second, stale stage. Bring the transient desk state back to
// its node before the renderer rebuilds the canvas.
function cleanupDetachedVideoWorkflowDesks(){
    // Only desks registered by a canvas workflow host belong to this
    // renderer. Standalone LTX stage mounts use the same CSS class but have
    // their own lifecycle; touching unregistered nodes would remove them on
    // every canvas refresh.
    const stages = [...document.querySelectorAll('body > .vwf-stage.is-desk')]
        .filter(stage => stage._vwfCanvasHost || stage.dataset?.vwfNodeId);
    if(!stages.length) return;
    let removed = false;
    for(const stage of stages){
        const host = stage._vwfCanvasHost || null;
        const nodeId = stage.dataset?.vwfNodeId || host?._vwfCanvasNodeId || '';
        const node = nodeId ? nodes.find(item => item.id === nodeId) : null;
        if(node && node.type === 'video' && node.stageHost){
            // mountCanvasVideoWorkflowPanel consumes this one-shot flag and
            // reopens the desk on the replacement stage during render().
            node.openDirectorDesk = true;
        }
        if(node?.type === 'ltxDirector') node._ltxStageDeskOpen = true;
        if(host?._vwfDeskEsc){
            document.removeEventListener('keydown', host._vwfDeskEsc);
            host._vwfDeskEsc = null;
        }
        if(host){
            host._vwfDesk = false;
    ...
    if(removed) document.body.classList.remove('vwf-desk-on');
}

function render(){
    cleanupDetachedVideoWorkflowDesks();
...
function isNodeControl(target){
    return !!target.closest('textarea, input, select, option, button, audio, video, canvas, [contenteditable="true"], .seg, .gen-btn, .comfy-run, .input-item, .blank-image, .mode-tabs, .ms-model-tabs, .llm-provider, .llm-output, .llm-chat-log, .llm-bubble, .llm-pane-resizer, .loop-preview, .ltx-director-timeline-host, .ltx-stage-host, .minimax-canvas-workbench, .pr-wrapper, .pr-toolbar, .pr-viewport, .pr-canvas, .pr-player-controls, .pr-prompt-area, .video-workflow-panel, .video-workflow-host, .asset-card-body, .audio-node-body');
}
...
function renderNode(node){
    ...
    const el = document.createElement('div');
    const size = defaultNodeSize(node.type, node);
    const hasFixedSize = Boolean(node.h || size.h);
    el.className = `node ${node.type}-node ${node.stageHost ? 'stage-host-node' : ''} ${node.url ? 'has-image' : ''} ${hasFixedSize ? 'sized' : ''} ${selected.has(node.id) ? 'selected' : ''}`;
    ...
    const title = node.type === 'image' ? (nodeTitleForMedia(node) === 'Image' ? tr('canvas.kit.image') : nodeTitleForMedia(node))
        : node.type === 'prompt' ? tr('canvas.kit.text')
        : node.type === 'audio' ? tr('canvas.kit.audio')
        : node.type === 'assetCard' ? (node.assetKind === 'scene' ? tr('canvas.kit.scene') : node.assetKind === 'character' ? tr('canvas.kit.character') : tr('canvas.assetCard'))
        ...
        : node.type === 'video' ? (node.stageHost ? tr('canvas.kit.stage3d') : tr('canvas.videoGenerateNode'))
    ...
    const statusHtml = showStatus ? (() => {
        const label = { queued:'排队中', running:'运行中', done:'完成', failed:'失败', preview:'本地预览' }[node.runStatus] || '';
    ...
    // Keep the node affordance keyboard/AT discoverable.  This header is
    // generated for every node (including imported legacy nodes), so do not
    // rely on the icon glyph alone for its accessible name.  Bind the action
    // below instead of interpolating an id into an inline JavaScript handler;
    // imported canvas data is not guaranteed to have been generated by this
    // client.
    const deleteNodeLabel = `${tr('common.delete') || '删除'} ${displayTitle}`;
    const safeNodeId = escapeAttr(String(node.id || ''));
    el.innerHTML = `<div class="node-head"><span class="node-title">${displayTitle}</span><div style="display:flex;align-items:center;gap:8px">${statusHtml}<button type="button" data-node-delete="${safeNodeId}" class="text-gray-300 hover:text-red-500" title="${escapeAttr(deleteNodeLabel)}" aria-label="${escapeAttr(deleteNodeLabel)}"><i data-lucide="x" class="w-4 h-4" aria-hidden="true"></i></button></div></div>`;
    const headerDeleteButton = el.querySelector('[data-node-delete]');
    if(headerDeleteButton) headerDeleteButton.addEventListener('click', e => deleteNodeFromButton(node.id, e));
    ...
    if(node.type === 'prompt') {
        ...
        textarea.oninput = e => {
            ...
            showCanvasAssetMentions(textarea, node);
        };
        textarea.addEventListener('blur', () => setTimeout(hideCanvasAssetMentions, 180));
    }
    if(node.type === 'assetCard') body.appendChild(renderAssetCardBody(node));
    if(node.type === 'audio') body.appendChild(renderAudioNodeBody(node));
    ...
    const canOutput = ['image','prompt','assetCard','audio','loop','group','promptGroup','generator','midjourney','comfy','ltxDirector','llm','msgen','video','rh','minimax','output'].includes(node.type);
...
function defaultNodeSize(type, node=null){
    ...
    if(type === 'video') return node?.stageHost ? CANVAS_DIRECTOR_STAGE_SIZE : {w:720, h:0};
    if(type === 'audio') return {w:280, h:0};
    if(type === 'assetCard') return {w:300, h:0};
    if(type === 'minimax') return {w:980, h:720};
    if(type === 'rh') return {w:430, h:0};
    if(type === 'comfy') return {w:420, h:460};
    if(type === 'ltxDirector') return CANVAS_LTX_DIRECTOR_SIZE;
        ...
        card.querySelectorAll('.canvas-asset-action').forEach(btn => {
         ...
         card.querySelector('[data-canvas-asset-rename]')?.addEventListener('click', async event => {
             ...
             try {
                 ...
                 console.error('canvas asset rename failed', err);
                 setStatus(err?.message || '重命名资产失败');
             }
         });
         card.querySelector('[data-canvas-asset-delete]')?.addEventListener('click', async event => {
             ...
             try {
                 ...
                 console.error('canvas asset delete failed', err);
                 setStatus(err?.message || '删除资产失败');
             }
         });
    });
    refreshIcons();
...
function toggleCanvasAssetLibrary(open=!canvasAssetLibraryOpen){
    ...
    if(canvasAssetLibraryOpen) {
        loadCanvasAssetLibrary().catch(err => {
            console.error('canvas asset library refresh failed', err);
            setStatus(err?.message || '素材库刷新失败');
        });
    }
}
...
function openAssetManager(){
    ...
    // Opening the manager is a user action; observe both network requests so
    // an offline/local-server failure cannot become an unhandled rejection.
    Promise.all([loadCanvasAssetLibrary({renderPanel:false}), loadCanvasPromptTemplates()])
        .then(renderAssetManager)
        .catch(err => {
            console.error('asset manager bootstrap failed', err);
            setStatus(err?.message || '素材管理器加载失败');
            renderAssetManager();
        });
}
...
function applyCanvasManagerBatchDeleteResult(data, selected, noun='项'){
    const requested = [...selected].map(String);
    const skippedIds = new Set((Array.isArray(data?.skipped) ? data.skipped : [])
        .map(item => String(item?.id || item?.item_id || ''))
        .filter(Boolean));
    const deletedIds = new Set((Array.isArray(data?.deleted) ? data.deleted : (Array.isArray(data?.removed_ids) ? data.removed_ids : []))
        .map(String));
    if(!deletedIds.size){
        requested.filter(id => !skippedIds.has(id)).forEach(id => deletedIds.add(id));
    }
    deletedIds.forEach(id => selected.delete(id));
    const skippedCount = skippedIds.size;
    const retainedFileCount = (Array.isArray(data?.skipped_files) ? data.skipped_files : [])
        .filter(item => item?.reason === 'referenced').length;
    const removedCount = deletedIds.size;
    if(skippedCount){
        setStatus(`已删除 ${removedCount} ${noun}，跳过 ${skippedCount} ${noun}（仍保留选择）`);
    } else if(retainedFileCount){
        setStatus(`已删除 ${removedCount} ${noun}，${retainedFileCount} 个关联文件已保留`);
    } else {
        setStatus(`已删除 ${removedCount} ${noun}`);
    }
}
...
function renderImageAssetManager(){
    ...
    upload?.addEventListener('change', async () => {
        ...
        try {
            ...
            setStatus(err?.message || '素材上传失败');
        } finally {
            // Allow selecting the same file again after a failed or cancelled
            // upload; Chromium otherwise suppresses the next change event.
            upload.value = '';
        }
    });
}
...
function renderWorkflowAssetManager(){
    ...
    upload?.addEventListener('change', async () => {
        ...
        try {
            ...
            const response = await fetch('/api/asset-library/workflows/upload', {method:'POST', body:form});
            const data = await response.json().catch(() => ({}));
            if(!response.ok) throw new Error(data.detail || '工作流上传失败');
            canvasAssetLibrary = data.library || canvasAssetLibrary;
            managerSelectedWorkflowIds.clear();
            renderAssetManager();
            renderCanvasAssetLibrary();
        } catch(err) {
            console.error('canvas workflow upload failed', err);
            setStatus(err?.message || '工作流上传失败');
        } finally {
            upload.value = '';
        }
    });
...
async function extractVideoFrames(url, stamps){
    return new Promise((resolve, reject) => {
        ...
        video.src = url;
        video.onerror = () => reject(new Error('抽帧失败'));
        video.onloadedmetadata = async () => {
            ...
            try {
                for (const stamp of stamps) {
                    ...
                    video.currentTime = Math.min(Math.max(0, stamp * dur), Math.max(0, dur - 0.05));
...
function lastVideoUrl(node){
    const items = [...(node.images || [])].reverse();
    const hit = items.find(item => {
        const url = outputUrlValue(item);
        return url && (mediaKindForOutputItem(item) === 'video' || /\.(mp4|webm|mov)(\?|$)/i.test(url));
    });
    return hit ? outputUrlValue(hit) : '';
}
function canvasWorkflowSlotReady(node, slot){
    const schema = window.VideoWorkflowSchema;
    if(!schema) return false;
    const wf = schema.normalize(node?.videoWorkflow || schema.emptyWorkflow());
    if(schema.engineReady(wf, slot)) return true;
    if(slot === 'video') return Boolean(resolveVideoProviderId(node?.apiProvider));
    if(slot === 'image') return Boolean(canvasImageEngine().provider);
    return false;
}
function canvasToolPreviewStatus(node, slot, readyText){
    if(canvasWorkflowSlotReady(node, slot)) return readyText;
    return tr('videoWf.toolMock');
}
async function refreshNodeWorkflowPreview(node){
    const host = document.querySelector(`.node[data-id="${CSS.escape(node.id)}"] .video-workflow-host`);
    if(!host || !window.VideoWorkflowPanel) return;
    const sources = orderedSources(node, generatorSources(node));
    const prompt = sources.map(s => s.prompt).filter(Boolean).join('\n\n');
    const refs = sources.flatMap(s => s.refs || []);
    const applied = await applyCanvasVideoWorkflow(node, prompt, refs, stageWorkflowsFromSources(sources));
    const preview = window.VideoWorkflowAdapter.previewBody({
        prompt: applied.prompt,
        images: applied.images,
        videos: applied.payload?.videos || applied.videos,
        audios: applied.payload?.audios || applied.audios,
        return_last_frame: applied.returnLastFrame
    }, applied.leftover);
    window.VideoWorkflowPanel.setPreview(host, JSON.stringify(preview, null, 2));
}
async function runVideoTool(node, tool, hostEl){
    const wf = node.videoWorkflow = window.VideoWorkflowSchema
        ? window.VideoWorkflowSchema.normalize(node.videoWorkflow || window.VideoWorkflowSchema.emptyWorkflow())
        : (node.videoWorkflow || {});
    const url = lastVideoUrl(node);
    if (tool === 'frames') {
        if (!url) { setStatus(tr('videoWf.needVideoForFrames')); return; }
        setStatus(tr('videoWf.extractingFrames'));
        const frames = await extractVideoFrames(url, [0, 0.25, 0.5, 0.75, 1]);
        frames.forEach((dataUrl, i) => {
            const img = addImageNode({ x: node.x + 220, y: node.y + i * 90 });
            img.url = dataUrl;
            img.name = tr('videoWf.frameName') + ' ' + (i + 1);
        });
        refreshNodes();
        scheduleSave();
        setStatus(tr('videoWf.extractedFrames') + ' ' + frames.length);
        return;
    }
    if (tool === 'upscale') {
        node.enableUpsample = true;
        wf.engines = wf.engines || {};
        setStatus(canvasToolPreviewStatus(node, 'upscale',
            (wf.engines.upscale && wf.engines.upscale.provider)
                ? (tr('videoWf.upscaleUsing') + ' ' + wf.engines.upscale.provider)
                : tr('videoWf.upscaleOpened')));
        if(window.VideoWorkflowSchema) window.VideoWorkflowSchema.writeHost(node, wf);
        scheduleSave();
        refreshNodes([node.id]);
        refreshNodeWorkflowPreview(node);
        return;
    }
    if (tool === 'clip') {
        const pop = hostEl?.querySelector?.('.video-clip-pop');
        if (pop) {
            pop.classList.toggle('hidden');
            const a = pop.querySelector('[data-clip-in]');
            const b = pop.querySelector('[data-clip-out]');
            if (a) a.value = String(node.clipIn || 0);
            if (b) b.value = String(node.clipOut || node.duration || 5);
            return;
        }
        node.clipIn = Number(node.clipIn || 0);
        node.clipOut = Number(node.clipOut || node.duration || 5);
        setStatus(tr('videoWf.clipRange') + ' ' + node.clipIn + 's – ' + node.clipOut + 's');
        scheduleSave();
        refreshNodeWorkflowPreview(node);
        return;
    }
    if (tool === 'split-audio') {
        wf.audioSplit = true;
        setStatus(canvasToolPreviewStatus(node, 'tts', tr('videoWf.audioSplitNoted')));
        if(window.VideoWorkflowSchema) window.VideoWorkflowSchema.writeHost(node, wf);
        scheduleSave();
        refreshNodeWorkflowPreview(node);
        return;
    ...
    if (tool === 'crop') {
        wf.redo = wf.redo || {};
        wf.redo.enabled = true;
        setStatus(canvasToolPreviewStatus(node, 'redo', tr('videoWf.redoOpened')));
        if(window.VideoWorkflowSchema) window.VideoWorkflowSchema.writeHost(node, wf);
        scheduleSave();
        refreshNodes([node.id]);
        refreshNodeWorkflowPreview(node);
    ...
    if (tool === 'parse' || tool === 'breakdown') {
        wf.analyze = tool;
        setStatus(canvasToolPreviewStatus(node, 'llm', tool === 'breakdown' ? tr('videoWf.breakdownOpened') : tr('videoWf.parseOpened')));
        if(window.VideoWorkflowSchema) window.VideoWorkflowSchema.writeHost(node, wf);
        scheduleSave();
        refreshNodeWorkflowPreview(node);
        return;
    ...
    if (tool === 'matte') {
        wf.greenscreen = wf.greenscreen || {};
        wf.greenscreen.enabled = true;
        setStatus(canvasToolPreviewStatus(node, 'matting', tr('videoWf.matteOpened')));
        if(window.VideoWorkflowSchema) window.VideoWorkflowSchema.writeHost(node, wf);
        scheduleSave();
        refreshNodes([node.id]);
        refreshNodeWorkflowPreview(node);
...
function renderVideoBody(node){
    ...
    // A 3D director desk is a first-class canvas source, not a video
    // generator with a hidden execution path.  Keep the node compact and
    // expose only the stage editor; rendering the regular video toolbar here
    // made the node look executable even though runVideoNode intentionally
    // treats stageHost nodes as data-only sources.
    if(node?.stageHost){
        wrap.classList.add('stage-host-body');
        wrap.innerHTML = `
            <div class="stage-host-intro">
                <span class="stage-host-kicker">3D DIRECTOR DESK</span>
                <span class="stage-host-note">片场数据源 · 连接到视频或输出节点后参与生成</span>
            </div>
            <div class="video-workflow-host stage-host-workflow"></div>
        `;
        const host = wrap.querySelector('.stage-host-workflow');
        mountCanvasVideoWorkflowPanel(host, node);
        return wrap;
    }
    const inputSources = generatorSources(node);
    const ordered = orderedSources(node, inputSources);
    ...
    const configuredVideoModels = providerVideoModels(node.apiProvider);
    const customVideoModel = Boolean(node.model && (!configuredVideoModels.length || !configuredVideoModels.includes(node.model)));
    const videoUrl = lastVideoUrl(node);
    const prevShot = (connections || []).some(c => {
        if(c.to !== node.id) return false;
        const from = nodes.find(n => n.id === c.from);
        return from && (from.type === 'video' || Boolean(lastVideoUrl(from)));
    });
    const contOn = Boolean(node.videoWorkflow?.continuePrev?.enabled);
    wrap.innerHTML = `
        <div class="video-float-tools">
            <button type="button" class="vfloat-btn" data-vtool="frames"><i data-lucide="layers"></i><span>${escapeHtml(tr('videoWf.tool.frames'))}</span></button>
            <button type="button" class="vfloat-btn" data-vtool="upscale"><i data-lucide="sparkles"></i><span>${escapeHtml(tr('videoWf.tool.upscale'))}</span></button>
            <button type="button" class="vfloat-btn" data-vtool="clip"><i data-lucide="scissors"></i><span>${escapeHtml(tr('videoWf.tool.clip'))}</span></button>
            <button type="button" class="vfloat-btn" data-vtool="split-audio"><i data-lucide="audio-lines"></i><span>${escapeHtml(tr('videoWf.tool.splitAudio'))}</span></button>
            <button type="button" class="vfloat-btn" data-vtool="crop"><i data-lucide="crop"></i><span>${escapeHtml(tr('videoWf.tool.crop'))}</span></button>
            <button type="button" class="vfloat-btn" data-vtool="parse"><i data-lucide="scan-search"></i><span>${escapeHtml(tr('videoWf.tool.parse'))}</span></button>
            <button type="button" class="vfloat-btn" data-vtool="breakdown"><i data-lucide="clapperboard"></i><span>${escapeHtml(tr('videoWf.tool.breakdown'))}</span></button>
            <button type="button" class="vfloat-btn" data-vtool="matte"><i data-lucide="user-round"></i><span>${escapeHtml(tr('videoWf.tool.matte'))}</span></button>
        </div>
        <div class="video-clip-pop hidden">
            <label>${escapeHtml(tr('videoWf.inPoint'))} <input type="number" min="0" step="0.1" data-clip-in value="${Number(node.clipIn || 0)}"></label>
            <label>${escapeHtml(tr('videoWf.outPoint'))} <input type="number" min="0" step="0.1" data-clip-out value="${Number(node.clipOut || node.duration || 5)}"></label>
            <button type="button" data-clip-ok>${escapeHtml(tr('videoWf.tool.clip'))}</button>
        </div>
        ${prevShot ? `<button type="button" class="video-prev-chip ${contOn ? 'is-on' : ''}" data-prev-shot>${escapeHtml(tr('videoWf.prevShotHint'))}</button>` : ''}
        <div class="video-hero">${videoUrl
            ? `<video src="${escapeAttr(videoUrl)}" muted playsinline preload="metadata"></video>`
            : `<span>${escapeHtml(tr('videoWf.videoHeroEmpty'))}</span>`}</div>
        <div class="prompt-list mb-3"></div>
        ...
        <div class="gen-settings">
            <div class="gen-settings-row">
                <select class="select-lite video-provider" style="flex:1" aria-label="${escapeHtml(tr('canvas.apiProvider'))}" title="${escapeHtml(tr('canvas.apiProvider'))}">${videoProviderOptions(node.apiProvider)}</select>
                <select class="select-lite video-model" style="flex:2" aria-label="${escapeHtml(tr('canvas.videoModel'))}" title="${escapeHtml(tr('canvas.videoModel'))}">${videoModelOptions(node.model, node.apiProvider)}</select>
            </div>
            <label class="video-model-manual-field ${customVideoModel ? '' : 'is-hidden'}" data-video-manual-model-wrap>
                <span class="video-model-manual-label">自定义模型标识</span>
                <input class="video-model-manual" data-video-model-manual type="text" value="${escapeAttr(node.model || '')}" placeholder="输入模型标识" autocomplete="off" spellcheck="false">
                <small>平台没有模型目录时，直接填写它接受的模型 ID。</small>
            </label>
            <div class="gen-settings-row">
                <label class="field" style="flex:1">
    ...
    const manualModelWrap = wrap.querySelector('[data-video-manual-model-wrap]');
    const manualModelInput = wrap.querySelector('[data-video-model-manual]');
    ...
    providerSelect.onchange = e => {
        ...
        node.apiProvider = e.target.value;
        const models = providerVideoModels(node.apiProvider);
        if(!models.includes(node.model)) node.model = '';
        modelSelect.innerHTML = videoModelOptions(node.model, node.apiProvider);
        if(manualModelInput) manualModelInput.value = node.model || '';
        syncVideoManualModelField();
        syncNodeEngineToWorkflow(node, 'video');
        scheduleSave();
    };
    function syncVideoManualModelField(focus = false){
        if(!manualModelWrap) return;
        const models = providerVideoModels(node.apiProvider);
        const selectedIsManual = modelSelect?.value === '__manual__';
        const visible = Boolean(node.apiProvider && (selectedIsManual || !models.length || (node.model && !models.includes(node.model))));
        manualModelWrap.classList.toggle('is-hidden', !visible);
        manualModelWrap.hidden = !visible;
        if(visible && manualModelInput && manualModelInput.value !== (node.model || '')) manualModelInput.value = node.model || '';
        if(focus && visible) {
    ...
    modelSelect.onchange = e => {
        ...
        if(e.target.value === '__manual__'){
            syncVideoManualModelField(true);
        } else {
            node.model = e.target.value;
            if(manualModelInput) manualModelInput.value = node.model || '';
            syncVideoManualModelField();
    ...
    if(manualModelInput) {
        manualModelInput.onmousedown = e => e.stopPropagation();
        manualModelInput.onclick = e => e.stopPropagation();
        manualModelInput.oninput = e => {
            e.stopPropagation();
            node.model = String(e.target.value || '').trim();
            syncNodeEngineToWorkflow(node, 'video');
            scheduleSave();
        };
        manualModelInput.onchange = e => {
            e.stopPropagation();
            node.model = String(e.target.value || '').trim();
            syncNodeEngineToWorkflow(node, 'video');
            scheduleSave();
    ...
    wrap.querySelectorAll('[data-vtool]').forEach(btn => {
        btn.onmousedown = e => e.stopPropagation();
        btn.onclick = async e => {
            e.stopPropagation();
            try { await runVideoTool(node, btn.getAttribute('data-vtool'), wrap); }
            catch (err) { setStatus(err.message || '工具失败'); }
        };
    });
    wrap.querySelector('[data-clip-ok]')?.addEventListener('click', e => {
        e.stopPropagation();
        const a = Number(wrap.querySelector('[data-clip-in]')?.value || 0);
        const b = Number(wrap.querySelector('[data-clip-out]')?.value || 0);
        node.clipIn = Math.max(0, a);
        node.clipOut = Math.max(node.clipIn, b);
        wrap.querySelector('.video-clip-pop')?.classList.add('hidden');
        setStatus(tr('videoWf.clipRange') + ' ' + node.clipIn + 's – ' + node.clipOut + 's');
        scheduleSave();
        refreshNodeWorkflowPreview(node);
    });
    wrap.querySelector('[data-prev-shot]')?.addEventListener('click', e => {
        e.stopPropagation();
        if(!window.VideoWorkflowSchema) return;
        const wf = window.VideoWorkflowSchema.normalize(node.videoWorkflow || window.VideoWorkflowSchema.emptyWorkflow());
        wf.continuePrev.enabled = !wf.continuePrev.enabled;
        wf.continuePrev.useLastFrame = true;
        window.VideoWorkflowSchema.writeHost(node, wf);
        scheduleSave();
        refreshNodes([node.id]);
        refreshNodeWorkflowPreview(node);
    });
    ...
    wfHost.className = 'video-workflow-host';
    const runRow = wrap.querySelector('.gen-run-row');
    if(runRow) runRow.before(wfHost);
    else wrap.appendChild(wfHost);
    mountCanvasVideoWorkflowPanel(wfHost, node);
    return wrap;
}
function canvasVideoWorkflowAssets(node){
    const fromCards = window.VideoWorkflowAdapter ? window.VideoWorkflowAdapter.collectCanvasAssets(nodes) : [];
    const fromPanel = node?.videoWorkflow?.assets || [];
    return [...fromCards, ...fromPanel];
}
function applyWorkflowEngineToNode(node, slot){
    const eng = node?.videoWorkflow?.engines?.[slot || 'video'];
    if(!eng || !eng.provider) return;
    const known = (apiProviders || []).some(p => p && !isOpenSourceBlockedProvider(p) && p.id === eng.provider)
        || window.VideoWorkflowSchema?.isLocalEngine?.(eng.provider);
    if(known){
        node.apiProvider = eng.provider;
        if(eng.model) node.model = eng.model;
        sanitizeVideoNodeProviderModel(node);
    }
}
function syncNodeEngineToWorkflow(node, slot){
    if(!window.VideoWorkflowSchema || !node) return;
    node.videoWorkflow = window.VideoWorkflowSchema.normalize(node.videoWorkflow || window.VideoWorkflowSchema.emptyWorkflow());
    const key = slot || 'video';
    const prev = node.videoWorkflow.engines[key] || { provider: '', model: '', baseUrl: '' };
    node.videoWorkflow.engines[key] = {
        provider: node.apiProvider || prev.provider || '',
        model: node.model || prev.model || '',
        baseUrl: prev.baseUrl || ''
    };
}
async function applyCanvasVideoWorkflow(node, prompt, refs, stageWorkflows=[]){
    if(!window.VideoWorkflowAdapter || !window.VideoWorkflowSchema) return {prompt, refs, leftover:{}, images:imageRefsOnly(refs || []), payload:{videos:videoRefsOnly(refs || []).map(r => r.url), audios:audioRefsOnly(refs || []).map(r => r.url)}};
    const schema = window.VideoWorkflowSchema;
    node.videoWorkflow = schema.fromHost(node);
    applyWorkflowEngineToNode(node, 'video');
    // Inherit stage data from an upstream director desk without persisting a
    // copy into the target node. This keeps the connection live while making
    // actors/cameras/layout/keyframes available to the generic adapter.
    const workflow = schema.normalize(node.videoWorkflow);
    (stageWorkflows || []).forEach(sourceWorkflow => {
        const source = schema.normalize(sourceWorkflow);
        const sourceStage = source.stage;
        if(!sourceStage || !(schema.stageHasContent?.(sourceStage))) return;
        const targetStage = workflow.stage || schema.defaultStage();
        if(!targetStage.layoutUrl && sourceStage.layoutUrl) targetStage.layoutUrl = sourceStage.layoutUrl;
        if(!(targetStage.actors || []).length && (sourceStage.actors || []).length) targetStage.actors = sourceStage.actors.map(actor => ({...actor}));
        const cameraSame = (a, b) => a && b && String(a.kind || '') === String(b.kind || '') &&
            ['x', 'y', 'facing', 'alt'].every(key => Math.abs(Number(a[key] || 0) - Number(b[key] || 0)) < 0.0001);
        const baselineCamera = schema.defaultStage?.()?.cameras?.[0];
        const targetCameraIsDefault = cameraSame(targetStage.cameras?.[0], baselineCamera);
        const sourceCameraChanged = sourceStage.cameras?.[0] && !cameraSame(sourceStage.cameras[0], baselineCamera);
        if((targetStage.cameras || []).length <= 1 && ((sourceStage.cameras || []).length > 1 || (targetCameraIsDefault && sourceCameraChanged))){
            targetStage.cameras = sourceStage.cameras.map(camera => ({...camera}));
            targetStage.camera = targetStage.cameras[0];
        }
        if(!targetStage.cameraMove && sourceStage.cameraMove) targetStage.cameraMove = sourceStage.cameraMove;
        if(!(targetStage.keyframes || []).length && (sourceStage.keyframes || []).length) targetStage.keyframes = sourceStage.keyframes.map(frame => ({...frame}));
        const defaultScene = schema.defaultScene?.();
        if(defaultScene && JSON.stringify(targetStage.scene) === JSON.stringify(defaultScene) && sourceStage.scene && JSON.stringify(sourceStage.scene) !== JSON.stringify(defaultScene)){
            targetStage.scene = {...sourceStage.scene};
        }
        const defaultStage = schema.defaultStage?.();
        if(defaultStage){
            if(targetStage.aspect === defaultStage.aspect && sourceStage.aspect !== defaultStage.aspect) targetStage.aspect = sourceStage.aspect;
            if(targetStage.duration === defaultStage.duration && sourceStage.duration !== defaultStage.duration) targetStage.duration = sourceStage.duration;
            if(targetStage.fps === defaultStage.fps && sourceStage.fps !== defaultStage.fps) targetStage.fps = sourceStage.fps;
        }
        workflow.stage = targetStage;
        const existing = new Set((workflow.extraRefs || []).map(ref => `${ref.kind || ''}:${ref.url || ''}:${ref.purpose || ''}`));
        (source.extraRefs || []).forEach(ref => {
            const key = `${ref.kind || ''}:${ref.url || ''}:${ref.purpose || ''}`;
            if(ref.url && !existing.has(key)){
                workflow.extraRefs.push({...ref});
                existing.add(key);
            }
        });
    });
    const previousVideos = window.VideoWorkflowAdapter.collectCanvasPreviousVideos(node, nodes, connections, {
        mediaKindForNode,
        generatedImageRefs,
        outputUrlValue,
        lastVideoUrl
    });
    return window.VideoWorkflowAdapter.apply({
        prompt,
        refs,
        workflow,
        previousVideos,
        assets: canvasVideoWorkflowAssets(node),
        node
    });
}
/* Keep video-workflow uploads on the same response contract as the generic
 * media uploader.  The old inline callbacks parsed JSON without checking
 * `response.ok`, so a 413/500 response that happened to contain `{files: []}`
 * looked like a successful upload and the panel silently discarded the file.
 * Throwing here lets VideoWorkflowPanel report the failure through its
 * onUploadError hook while preserving the editor state. */
async function uploadCanvasVideoWorkflowFiles(files){
    const form = new FormData();
    ...
    if(!response.ok){
        ...
        throw new Error(String(detail || `${tr('studio.uploadFailed') || '上传失败'}（${response.status}）`));
    }
    return Array.isArray(data?.files) ? data.files : [];
}
function mountCanvasVideoWorkflowPanel(host, node){
    if(!window.VideoWorkflowPanel || !host) return;
    // Keep a small transient link from the reparented desk back to its canvas
    // node. It is deliberately kept off the persisted workflow schema and is
    // only used to clean up the desk before a canvas-wide render.
    host._vwfCanvasNodeId = node?.id || '';
    window.VideoWorkflowSchema.writeHost(node, window.VideoWorkflowSchema.fromHost(node));
    if(node.openDirectorDesk){
        host._vwfDesk = true;
        node.openDirectorDesk = false;
    }

    // Director-stage nodes use the stage-only mount.  This keeps their
    // persisted workflow compatible with video nodes while preventing the
    // generic prompt/media/generate controls from being rendered on a
    // data-source node.
    if(node.stageHost && typeof window.VideoWorkflowPanel.mountStage === 'function'){
        window.VideoWorkflowPanel.mountStage(
            host,
            () => window.VideoWorkflowSchema.fromHost(node).stage,
            stage => {
                const wf = window.VideoWorkflowSchema.fromHost(node);
                wf.stage = stage;
                window.VideoWorkflowSchema.writeHost(node, wf);
                scheduleSave();
            },
            ...
            mode:'canvas',
            assets: canvasVideoWorkflowAssets(node),
                upload: uploadCanvasVideoWorkflowFiles,
                getWorkflow: () => window.VideoWorkflowSchema.fromHost(node),
                setWorkflow: wf => {
                    window.VideoWorkflowSchema.writeHost(node, wf);
                    scheduleSave();
                },
                flushSave: flushCanvasSaveForStage,
                onOpenPageError: message => setStatus(message),
                onRecordError: message => setStatus(message),
                onUploadError: message => setStatus(message),
                onPlacementError: message => setStatus(message),
                onExportError: message => setStatus(message),
                ownerNodeId: node.id,
                openPageUrl: canvas?.id
                    ? '/static/director-desk.html?id=' + encodeURIComponent(canvas.id) + '&node=' + encodeURIComponent(node.id)
                    : '',
                deskOpen: Boolean(host._vwfDesk),
                onDeskChange: on => {
                    host._vwfDesk = Boolean(on);
                    node._stageDeskOpen = Boolean(on);
                    scheduleSave();
                },
                onExport: async (dataUrl, stage) => {
                    const wf = window.VideoWorkflowSchema.fromHost(node);
                    wf.stage = stage;
                    if(dataUrl) wf.stage.layoutUrl = dataUrl;
                    window.VideoWorkflowSchema.writeHost(node, wf);
                    scheduleSave();
                    setStatus(tr('videoWf.layoutReady'));
                }
            }
        );
        const stage = host._vwfDeskEl || host.querySelector('.vwf-stage');
        if(stage){
            stage.dataset.vwfNodeId = node?.id || '';
            stage._vwfCanvasHost = host;
        }
        return;
    }
    window.VideoWorkflowPanel.mount(host, {
        mode:'canvas',
        providers: (apiProviders || []).filter(p => p && !isOpenSourceBlockedProvider(p) && p.enabled !== false),
        assets: canvasVideoWorkflowAssets(node),
        getState: () => {
            const wf = window.VideoWorkflowSchema.fromHost(node);
            if(host._vwfDesk) wf._deskOpen = true;
            return wf;
        },
        setState: wf => {
            window.VideoWorkflowSchema.writeHost(node, wf);
        ...
        onPlacementError: message => setStatus(message),
        onExportError: message => setStatus(message),
        onPreviewError: message => setStatus(message),
        upload: uploadCanvasVideoWorkflowFiles,
...
function canvasStageTargets(){
    return (nodes || []).filter(n => n.type === 'video' || n.type === 'ltxDirector' || n.type === 'minimax');
}
function pickCanvasStageTarget(assetNode){
    const targets = canvasStageTargets();
    if(!targets.length) return null;
    const connected = (connections || [])
        .filter(c => c.from === assetNode.id)
        .map(c => nodes.find(n => n.id === c.to))
        .filter(n => n && targets.some(t => t.id === n.id));
    if(connected.length) return connected[0];
    const selectedTargets = targets.filter(n => selected.has(n.id) && n.id !== assetNode.id);
    if(selectedTargets.length) return selectedTargets[0];
    return targets[0];
}
function placeAssetCardOnStage(assetNode){
    const schema = window.VideoWorkflowSchema;
    if(!schema || !assetNode) return;
    const target = pickCanvasStageTarget(assetNode);
    if(!target){
        setStatus(tr('videoWf.placeNeedTarget'));
        return;
    }
    const name = String(assetNode.name || '').trim() || tr('videoWf.asset.character');
    const rawKind = String(assetNode.assetKind || assetNode.kind || 'character').trim().toLowerCase();
    /* Scene cards are backgrounds in the director workflow.  Treat the
       explicit panorama flag and the dedicated panorama kind as scene
       backgrounds too; previously only panorama cards took this path, so a
       normal scene card was incorrectly converted into a mannequin actor. */
    const isScene = rawKind === 'scene' || rawKind === 'panorama' || Boolean(assetNode.panorama);
    const isStyle = rawKind === 'style';
    if(isStyle){
        setStatus(tr('videoWf.styleNotStageObject'));
        return;
    }
    if(isScene && !String(assetNode.url || '').trim()){
        setStatus(tr('videoWf.sceneNeedsImage'));
        return;
    }
    const isPanorama = Boolean(assetNode.panorama) || rawKind === 'panorama';
    const asset = {
        id: assetNode.id,
        kind: isPanorama ? 'panorama' : rawKind,
        name,
        notes: assetNode.notes || '',
        url: assetNode.url || '',
        panorama: isPanorama,
        ...(assetNode.primitive ? { primitive: assetNode.primitive } : {}),
        ...(assetNode.material ? { material: assetNode.material } : {})
    };
    if(target.type === 'ltxDirector'){
        const stage = schema.normalize({stage: target.ltxStage}).stage;
        if(isScene){
            stage.scene = stage.scene || schema.defaultScene();
            stage.scene.bgMode = 'image';
            stage.scene.bgUrl = asset.url;
            target.ltxStage = stage;
        } else {
            const existing = stage.actors.find(actor => actor.assetId === assetNode.id);
            if(existing){
                setStatus(tr('videoWf.alreadyOnStage'));
                return;
            }
            stage.actors.push(schema.actorFromAsset(asset, stage.actors));
            target.ltxStage = stage;
        }
    } else {
        const wf = schema.fromHost(target);
        if(!isScene){
            const existing = wf.stage.actors.find(actor => actor.assetId === assetNode.id);
            if(existing){
                setStatus(tr('videoWf.alreadyOnStage'));
                return;
            }
        }
        if(!wf.assets.some(item => item.id === assetNode.id || item.name === name)){
            wf.assets.push(asset);
        }
        if(isScene){
            wf.stage.scene = wf.stage.scene || schema.defaultScene();
            wf.stage.scene.bgMode = 'image';
            wf.stage.scene.bgUrl = asset.url;
        } else {
            wf.stage.actors.push(schema.actorFromAsset(asset, wf.stage.actors));
        }
        schema.writeHost(target, wf);
    }
    render();
    scheduleSave();
    setStatus(`${name} ${tr('videoWf.placedOnStage')}`);
}
function renderAudioNodeBody(node){
    const wrap = document.createElement('div');
    wrap.className = 'audio-node-body';
    wrap.innerHTML = `
        <input class="setting-input audio-name" type="text" value="${escapeHtml(node.name || '')}" placeholder="${escapeHtml(tr('canvas.kit.audio'))}">
        ${node.url
            ? `<div class="media-card audio-card"><i data-lucide="file-audio" class="w-8 h-8"></i><div class="audio-title">${escapeHtml(node.name || tr('canvas.kit.audio'))}</div><audio src="${escapeAttr(node.url)}" data-url="${escapeAttr(node.url)}" controls preload="metadata"></audio></div>`
            : `<button type="button" class="tool-btn audio-pick">${escapeHtml(tr('videoWf.pick'))}</button>`}
    `;
    wrap.querySelector('.audio-name').oninput = e => {
        node.name = e.target.value;
        scheduleSave();
    };
    wrap.querySelector('.audio-pick')?.addEventListener('click', e => {
        e.stopPropagation();
        pickAudioForNode(node.id);
    });
    return wrap;
}
function pickAudioForNode(nodeId){
    ...
    input.onchange = async () => {
        ...
        if(file?.url && node){
            ...
            if(!node.name) node.name = String(file.name || '').replace(/\.[^.]+$/, '');
            render();
            scheduleSave();
        }
    };
...
function renderAssetCardBody(node){
    const wrap = document.createElement('div');
    wrap.className = 'asset-card-body';
    const kinds = (window.VideoWorkflowSchema?.ASSET_KINDS || ['character','scene','prop','style','panorama']);
    const showPano = node.assetKind === 'scene' || node.assetKind === 'panorama' || node.panorama;
    wrap.innerHTML = `
        <select class="select-lite asset-kind" aria-label="资产类型" title="资产类型">${kinds.map(kind => `<option value="${kind}" ${node.assetKind === kind ? 'selected' : ''}>${escapeHtml(tr('videoWf.asset.' + kind))}</option>`).join('')}</select>
        <input class="setting-input asset-name" type="text" value="${escapeHtml(node.name || '')}" placeholder="${escapeHtml(tr('videoWf.assetName'))}">
        <input class="setting-input asset-notes" type="text" value="${escapeHtml(node.notes || '')}" placeholder="${escapeHtml(tr('videoWf.assetNotes'))}">
        ${showPano ? `<label class="vwf-check asset-pano"><input type="checkbox" ${node.panorama || node.assetKind === 'panorama' ? 'checked' : ''}> ${escapeHtml(tr('videoWf.asset.panorama'))}</label>` : ''}
        <div class="asset-card-tools">
            <button type="button" class="tool-btn" data-asset-tool="threeView"><i data-lucide="gallery-horizontal"></i><span>${escapeHtml(tr('videoWf.assetTool.threeView'))}</span></button>
            <button type="button" class="tool-btn" data-asset-tool="closeup"><i data-lucide="scan-face"></i><span>${escapeHtml(tr('videoWf.assetTool.closeup'))}</span></button>
            <button type="button" class="tool-btn" data-asset-tool="exprGrid"><i data-lucide="grid-3x3"></i><span>${escapeHtml(tr('videoWf.assetTool.exprGrid'))}</span></button>
            <button type="button" class="tool-btn" data-asset-tool="beauty"><i data-lucide="sparkles"></i><span>${escapeHtml(tr('videoWf.assetTool.beauty'))}</span></button>
            <button type="button" class="tool-btn" data-asset-tool="expr"><i data-lucide="smile"></i><span>${escapeHtml(tr('videoWf.assetTool.expr'))}</span></button>
            <button type="button" class="tool-btn" data-asset-tool="portrait"><i data-lucide="user-round"></i><span>${escapeHtml(tr('videoWf.assetTool.portrait'))}</span></button>
            <button type="button" class="tool-btn" data-asset-tool="upscale"><i data-lucide="maximize"></i><span>${escapeHtml(tr('videoWf.assetTool.upscale'))}</span></button>
            <button type="button" class="tool-btn" data-asset-tool="layers"><i data-lucide="layers"></i><span>${escapeHtml(tr('videoWf.assetTool.layers'))}</span></button>
            <button type="button" class="tool-btn" data-asset-tool="crop"><i data-lucide="crop"></i><span>${escapeHtml(tr('videoWf.assetTool.crop'))}</span></button>
            <button type="button" class="tool-btn" data-asset-tool="rotate"><i data-lucide="rotate-cw"></i><span>${escapeHtml(tr('videoWf.assetTool.rotate'))}</span></button>
        </div>
        ${node.url ? `<img class="asset-card-thumb" src="${escapeAttr(canvasMediaPreviewUrl(node.url, 256))}" alt="">` : `<button type="button" class="tool-btn asset-pick">${escapeHtml(tr('videoWf.pick'))}</button>`}
        ${Object.keys(node.variants || {}).length ? `<div class="asset-card-variants">${Object.entries(node.variants).map(([key, url]) => `<button type="button" class="asset-variant" data-variant="${escapeAttr(key)}"><img src="${escapeAttr(canvasMediaPreviewUrl(url, 96))}" alt=""><span>${escapeHtml(tr('videoWf.assetTool.' + key) || key)}</span></button>`).join('')}</div>` : ''}
        <button type="button" class="tool-btn asset-to-stage">${escapeHtml(tr('videoWf.toStage'))}</button>
    `;
    wrap.querySelector('.asset-kind').onchange = e => {
        node.assetKind = e.target.value;
        if(node.assetKind === 'panorama') node.panorama = true;
        render();
        scheduleSave();
    };
    wrap.querySelector('.asset-name').oninput = e => { node.name = e.target.value; scheduleSave(); };
    wrap.querySelector('.asset-notes').oninput = e => { node.notes = e.target.value; scheduleSave(); };
    wrap.querySelector('.asset-pano input')?.addEventListener('change', e => {
        node.panorama = e.target.checked;
        scheduleSave();
    });
    wrap.querySelector('.asset-to-stage')?.addEventListener('click', e => {
        e.stopPropagation();
        placeAssetCardOnStage(node);
    });
    wrap.querySelectorAll('[data-asset-tool]').forEach(btn => {
        btn.onmousedown = e => e.stopPropagation();
        btn.onclick = e => {
            e.stopPropagation();
            runAssetCardTool(node, btn.getAttribute('data-asset-tool'));
        };
    });
    ...
    wrap.querySelector('.asset-pick')?.addEventListener('click', async e => {
        e.stopPropagation();
        const input = document.createElement('input');
        input.type = 'file';
         input.accept = 'image/*';
         input.onchange = async () => {
             ...
             try {
                 ...
                 const response = await fetch('/api/ai/upload', {method:'POST', body:form});
                 const data = await response.json().catch(() => ({}));
                 if(!response.ok) throw new Error(data.detail || '图片上传失败');
                 const file = data.files?.[0];
                 if(file?.url){
                     node.url = file.url;
                     if(!node.name) node.name = String(file.name || '').replace(/\.[^.]+$/, '');
                 ...
                 console.error('canvas node asset upload failed', err);
                 showErrorModal(err?.message || '图片上传失败', '图片上传失败');
             } finally {
                 input.value = '';
             }
         };
...
const ASSET_TOOL_PROMPTS = {
    threeView: '同一角色三视图：正面、侧面、背面，白底全身，保持脸和服装一致。',
    closeup: '同一角色面部特写，五官清晰，保持身份一致。',
    exprGrid: '同一角色表情九宫格，3x3 不同情绪，身份一致。',
    beauty: '同一角色妆容精修，皮肤质感更好，不改变身份。',
    expr: '同一角色表情特写，情绪明确，身份一致。',
    portrait: '同一角色提升人像质感、皮肤和服装细节，不改变五官。',
    texture: '同一角色提升人像质感和材质细节，不改变五官。',
    upscale: '提高分辨率和清晰度，保持构图和身份不变。',
    layers: '把角色从背景分离，透明或纯色底，图层分离。'
};
function canvasImageEngine(){
    const schema = window.VideoWorkflowSchema;
    const selectedVid = (nodes || []).find(n => selected.has(n.id) && n.videoWorkflow?.engines?.image?.provider);
    const anyVid = selectedVid || (nodes || []).find(n => n.videoWorkflow?.engines?.image?.provider);
    const eng = schema?.engineOf?.(anyVid?.videoWorkflow, 'image') || anyVid?.videoWorkflow?.engines?.image;
    if(eng?.provider) return { provider: eng.provider, model: eng.model || '', baseUrl: eng.baseUrl || '' };
    return { provider: '', model: '', baseUrl: '' };
}
async function uploadCanvasBlob(blob, name){
    const form = new FormData();
    form.append('files', blob, name || 'asset.png');
    const data = await fetch('/api/ai/upload', {method:'POST', body:form}).then(r => r.json());
    return data.files?.[0] || null;
}
async function transformAssetCardImage(node, mode){
    if(!node.url){
        setStatus(tr('videoWf.needAssetImage'));
        return;
    }
    ...
    if(mode === 'rotate'){
        ...
        const cutX = Math.round(img.naturalWidth * 0.08);
        const cutY = Math.round(img.naturalHeight * 0.08);
        const w = Math.max(8, img.naturalWidth - cutX * 2);
        const h = Math.max(8, img.naturalHeight - cutY * 2);
        canvas.width = w;
        canvas.height = h;
        ctx.drawImage(img, cutX, cutY, w, h, 0, 0, w, h);
    }
    const blob = await new Promise(resolve => canvas.toBlob(resolve, 'image/png'));
    if(!blob) return;
    const file = await uploadCanvasBlob(blob, `${node.name || 'asset'}-${mode}.png`);
    if(file?.url){
        node.url = file.url;
        node.variants = node.variants || {};
        node.variants[mode] = file.url;
        render();
        scheduleSave();
        setStatus(tr('videoWf.assetTool.' + mode));
    }
}
async function runAssetCardTool(node, tool){
    if(tool === 'rotate' || tool === 'crop'){
        try { await transformAssetCardImage(node, tool); }
        catch(err){ showErrorModal(err.message || String(err), tr('videoWf.assetTool.' + tool)); }
        return;
    }
    if(!node.url){
        setStatus(tr('videoWf.needAssetImage'));
    ...
    const promptNode = addPromptNode({x: node.x + 280, y: node.y});
    promptNode.text = ASSET_TOOL_PROMPTS[tool] || ASSET_TOOL_PROMPTS.closeup;
    const gen = addGeneratorNode({x: node.x + 540, y: node.y});
    const eng = canvasImageEngine();
    if(eng.provider) gen.apiProvider = eng.provider;
    else gen.apiProvider = '';
    if(eng.model) gen.model = eng.model;
    connections.push({id: uid('c'), from: node.id, to: gen.id});
    connections.push({id: uid('c'), from: promptNode.id, to: gen.id});
    render();
    scheduleSave();
    if(!eng.provider || window.VideoWorkflowSchema?.isLocalEngine?.(eng.provider)){
        node.variants = node.variants || {};
        node.variants[tool] = node.url;
        gen.runStatus = 'preview';
        gen.runError = tr('videoWf.toolMock');
        setStatus(tr('videoWf.toolMock'));
        refreshNodes([gen.id, node.id]);
        return;
    }
    try {
        await runGenerator(gen.id);
        const out = (gen.generatedOutputs || []).map(item => outputUrlValue(item)).filter(Boolean).pop()
            || lastVideoUrl(gen);
        if(out){
            node.variants = node.variants || {};
            node.variants[tool] = out;
            refreshNodes([node.id]);
        }
    } catch(err){ showErrorModal(err.message || String(err), tr('videoWf.assetTool.' + tool)); }
}
async function runCanvasSmartExecute(){
    const types = typeof canvasRunTypes === 'function' ? canvasRunTypes() : CANVAS_GENERATOR_TYPES;
    const gens = (nodes || []).filter(n => types.includes(n.type) && !n.stageHost);
    if(!gens.length){
        setStatus(tr('canvas.smartExecuteEmpty'));
        return;
    }
    const terminals = gens.filter(n => isTerminalGenerator(n.id));
    const starts = terminals.length ? terminals : gens;
    const seen = new Set();
    const order = [];
    starts.forEach(t => {
        const chain = typeof computeCascadeOrder === 'function' ? computeCascadeOrder(t.id) : [t.id];
        chain.forEach(id => {
            if(!seen.has(id)){
                seen.add(id);
                order.push(id);
    ...
    setStatus(tr('canvas.smartExecute') + ` ${order.length}`);
    for(const id of order){
        ...
        try {
            ...
            if(node.runStatus === 'failed'){
                showErrorModal(node.runError || tr('canvas.generationFailed'), tr('canvas.smartExecute'));
                refreshNodes([id]);
                break;
            }
            if(node.runStatus !== 'preview') node.runStatus = 'done';
        } catch(err) {
            node.runStatus = 'failed';
            node.runError = err.message || String(err);
            refreshNodes([id]);
            showErrorModal(node.runError, tr('canvas.smartExecute'));
            break;
        }
        refreshNodes([id]);
        if(node.runStatus === 'failed') break;
    }
    scheduleSave();
}
function canvasAssetMentionItems(){
    const items = [];
    (nodes || []).forEach(n => {
        if(n.type === 'assetCard' && (n.name || n.url)) items.push({name:n.name || 'asset', kind:n.assetKind || 'character', notes:n.notes || '', url:n.url || ''});
        (n.videoWorkflow?.assets || []).forEach(asset => {
            if(asset.name) items.push(asset);
        });
    });
    return items;
}
function hideCanvasAssetMentions(){
    document.getElementById('vwfMention')?.remove();
}
function showCanvasAssetMentions(textarea, node){
    hideCanvasAssetMentions();
    const caret = textarea.selectionStart || 0;
    const prefix = String(textarea.value || '').slice(0, caret);
    const match = prefix.match(/@([^\s@]*)$/);
    if(!match) return;
    const q = match[1].toLowerCase();
    const hits = canvasAssetMentionItems().filter(item => String(item.name || '').toLowerCase().includes(q)).slice(0, 8);
    if(!hits.length) return;
    const box = document.createElement('div');
    box.id = 'vwfMention';
    box.className = 'vwf-mention';
    box.innerHTML = hits.map(item => `<button type="button" data-name="${escapeAttr(item.name)}">@${escapeHtml(item.name)} <small>${escapeHtml(tr('videoWf.asset.' + (item.kind || 'character')))}</small></button>`).join('');
    ...
    box.querySelectorAll('button').forEach(btn => {
        ...
        btn.onclick = () => {
            ...
            textarea.value = prefix.replace(/@([^\s@]*)$/, `@${name} `) + String(textarea.value || '').slice(caret);
            node.text = textarea.value;
            hideCanvasAssetMentions();
            refreshPromptCounter(textarea.closest('.node-body') || textarea.parentElement, node.text);
            scheduleSave();
            scheduleGeneratorInputSync();
        };
    });
...
function renderMiniMaxBody(node){
            ...
            <div class="minimax-wb-main" style="--minimax-preview-h:${previewH}px;--minimax-video-h:${videoTrackH}px;--minimax-ref-lane-h:${refLaneH}px;--minimax-ref-h:${Math.max(78, refLanes * refLaneH)}px">
                ...
                <div class="minimax-current-panel">
                    ...
                    <div class="minimax-clip-parameters"><div class="minimax-section-label"><i data-lucide="sliders-horizontal"></i><span>Clip settings</span></div><div class="minimax-settings minimax-segment-fields">
                        ...
                        <!-- rhPayment is retained on the node for legacy workflow payloads,
                             but payment/account selection is intentionally not exposed. -->
    ...
    shell.className = 'minimax-with-workflow';
    shell.appendChild(wrap);
    const wfHost = document.createElement('div');
    wfHost.className = 'video-workflow-host minimax-vwf-host';
    shell.appendChild(wfHost);
    mountCanvasVideoWorkflowPanel(wfHost, node);
    return shell;
}
function bindMiniMaxWorkbench(wrap, node){
    ...
    wrap.querySelectorAll('[data-minimax-payment]').forEach(select => {
        select.onchange = e => { e.stopPropagation(); node.rhPayment = 'free'; scheduleSave(); };
    });
...
function runningHubProvider(){
    const provider = (apiProviders || []).find(p => !isOpenSourceBlockedProvider(p) && p.id === 'runninghub');
...
function rhPaymentOptions(node){
    const provider = runningHubProvider();
    // Retain this helper for imported legacy templates, but expose only the
    // normal service-key path in the open-source build.  Wallet billing is not
    // a selectable feature and rhUseWallet() always returns false.
    const selected = 'free';
    return `
        <option value="free" ${selected === 'free' ? 'selected' : ''}>服务 API Key${provider?.has_key ? '' : '（未配置）'}</option>
    `;
}
function rhUseWallet(node){
    // Billing/account routing is intentionally disabled in the open-source
    // build. Keep the legacy field serialized for old workflow files, but
    // never let an imported node select a wallet key or paid account path.
    if(node && node.rhPayment !== 'free') node.rhPayment = 'free';
    return false;
}
...
function renderRhBody(node){
    ...
    wrap.innerHTML = `
        <div class="rh-top">
            ...
            <!-- Keep rhPayment/useWallet in serialized nodes for backend compatibility;
                 this open-source UI has no payment or wallet selector. -->
...
function hasDownstreamGenerator(nodeId){
    return connections.some(c => {
        ...
        const to = nodes.find(n => n.id === c.to);
        if(!to) return false;
        if(CANVAS_GENERATOR_TYPES.includes(to.type) && !to.stageHost) return true;
        if(to.type !== 'output') return false;
        return connections.some(cc => {
            if(cc.from !== to.id) return false;
            const next = nodes.find(n => n.id === cc.to);
            return next && CANVAS_GENERATOR_TYPES.includes(next.type) && !next.stageHost;
        });
    });
}
function shouldCreateOutputForNode(node){
    if(!node) return false;
    if(node.stageHost) return false;
...
function syncLatestGeneratedOutputToConnection(fromId, toId){
    ...
    if(source.stageHost){
        const workflow = stageWorkflowFromNode(source);
        if(workflow){
            out.stageWorkflow = workflow;
            return true;
        }
    }
...
function mediaRefsFromNode(node){
    if(!node) return [];
    if((node.type === 'image' || node.type === 'audio') && node.url){
        const kind = mediaKindForNode(node);
...
function stageWorkflowFromNode(node){
    if(!node?.stageHost || !window.VideoWorkflowSchema) return null;
    try { return window.VideoWorkflowSchema.fromHost(node); }
    catch(err) { return null; }
}
function stageSourceForNode(node){
    const workflow = stageWorkflowFromNode(node);
    if(!workflow) return null;
    const refs = [];
    const push = ref => {
        if(!ref?.url || refs.some(item => item.url === ref.url)) return;
        refs.push({...ref, _stageSource:true});
    };
    const stage = workflow.stage || {};
    if(stage.layoutUrl) push({url:stage.layoutUrl, name:'片场布局', kind:'image', purpose:'layout'});
    (workflow.extraRefs || []).forEach(push);
    // A stage is a data source even before an export image exists. Keeping the
    // stageWorkflow on the source lets downstream video nodes inherit actors,
    // cameras, keyframes and scene settings without pretending the desk is a
    // generator that should submit a remote job.
    return {
        id:`${node.id}:stage-workflow`,
        type:'stageWorkflow',
        label:tr('canvas.kit.stage3d'),
        preview:stage.layoutUrl || '',
        refs,
        stageWorkflow:workflow,
        prompt:''
    };
}
function stageWorkflowsFromSources(sources){
    const seen = new Set();
    return (sources || []).map(source => source?.stageWorkflow).filter(workflow => {
        if(!workflow) return false;
        const key = JSON.stringify(workflow.stage || workflow);
        if(seen.has(key)) return false;
        seen.add(key);
        return true;
    });
...
function generatorSources(gen){
    return connections.filter(c => c.to === gen.id).map(c => nodes.find(n => n.id === c.from)).filter(Boolean).map(n => {
        if(n.type === 'output'){
            if(!n.stageWorkflow){
                const stageLink = connections.find(link => link.to === n.id && nodes.find(item => item.id === link.from)?.stageHost);
                const stageNode = stageLink ? nodes.find(item => item.id === stageLink.from) : null;
                if(stageNode) n.stageWorkflow = stageWorkflowFromNode(stageNode);
            }
        }
        if(n.type === 'output' && n.stageWorkflow){
            const relay = stageSourceForNode({ stageHost:true, videoWorkflow:n.stageWorkflow, id:n.id });
            if(relay){
                relay.id = `${n.id}:stage-workflow`;
                relay.label = '上游片场布局';
                (n.images || []).slice(-1).forEach(item => {
                    const url = outputUrlValue(item);
                    if(url && !relay.refs.some(ref => ref.url === url)) relay.refs.push({url, name:'上游输出', kind:mediaKindForOutputItem(item)});
                });
                return relay;
            }
        }
        ...
        if((n.type === 'image' || n.type === 'audio') && n.url) {
        ...
        if(n.type === 'assetCard' && n.url) {
            return {id:n.id, type:'image', label:n.name || tr('canvas.assetCard'), preview:n.url, refs:[{url:n.url, name:n.name || 'asset', kind:'image'}], prompt:''};
        }
        if(n.type === 'group') {
...
async function runGenerator(genId, opts={}){
    ...
    const imageProviderId = resolveImageProviderId(gen.apiProvider || '');
    if(!imageProviderId || window.VideoWorkflowSchema?.isLocalEngine?.(imageProviderId)){
        gen.runStatus = 'preview';
        gen.runError = !imageProviderId ? '未选择图像引擎，当前仅生成预览。' : '本地图像引擎请使用对应的 ComfyUI/本地工作流节点。';
        setStatus(gen.runError);
...
async function runVideoNode(nodeId, opts={}){
    const node = nodes.find(n => n.id === nodeId);
    if(node?.stageHost){
        // A director desk is a layout/data source, not a video generator. It
        // may still be connected to a downstream video node, but Smart
        // Execute and cascade must never submit it as a remote job.
        node.runStatus = 'preview';
        node.runError = '片场节点仅提供布局数据，请连接到视频节点执行。';
        setStatus(node.runError);
    ...
    const cascadeTargetId = cascadeTargetIdFromOptions(opts);
    const sources = orderedSources(node, generatorSources(node));
    const promptRaw = sources.map(s => s.prompt).filter(Boolean).join('\n\n');
    ...
    const applied = await applyCanvasVideoWorkflow(node, promptRaw, mediaRefs, stageWorkflowsFromSources(sources));
    const prompt = applied.prompt;
    const refs = (applied.images && applied.images.length ? applied.images : imageRefsOnly(applied.refs)).map((ref, i) => {
        const next = {...ref};
        if(!next.role && (node.useFrameRoles || applied.useFrameRoles)){
            if(i === 0) next.role = 'first_frame';
            else if(i === 1) next.role = 'last_frame';
        }
        return next;
    });
    const videoRefs = videoRefsOnly(applied.refs);
    const audioRefs = audioRefsOnly(applied.refs);
    if(!prompt){ alert(tr('canvas.videoNeedsPrompt')); return; }
    const providerId = resolveVideoProviderId(node.apiProvider) || window.VideoWorkflowSchema?.engineOf?.(node.videoWorkflow, 'video')?.provider || '';
    const engine = window.VideoWorkflowSchema?.engineOf?.(node.videoWorkflow, 'video') || {};
    const modelId = String(node.model || engine.model || '').trim();
    if(!providerId || window.VideoWorkflowSchema?.isLocalEngine?.(providerId) || !modelId){
        node.runStatus = 'preview';
        node.runError = !providerId || window.VideoWorkflowSchema?.isLocalEngine?.(providerId)
            ? tr('videoWf.toolMock')
            : '请先选择或手动填写视频模型。';
        setStatus(tr('videoWf.toolMock'));
        await refreshNodeWorkflowPreview(node);
        scheduleSave();
        refreshRunNodes(node, outputForNode(node, 460));
        return;
    }
    let out = outputForNode(node, 460);
    const pendingId = uid('p');
    ...
    try {
        const result = await cascadeFetch('/api/canvas-video', {
            ...
            body:JSON.stringify({
                ...
                aspect_ratio:applied.leftover?.stageAspect || node.aspectRatio || '16:9',
                resolution:node.resolution || '',
                images:refs.map(ref => ({url:ref.url, name:ref.name || '', ...(ref.role ? {role:ref.role} : {})})),
                ...
                return_last_frame:Boolean(applied.returnLastFrame)
            })
...
async function runMiniMaxNode(nodeId, opts={}){
    ...
    const seg = miniMaxSelectedSegment(node);
    const promptRaw = String(seg?.prompt || '').trim() || sourceData.prompt;
    const rawRefs = miniMaxRefsForSegment(node, seg);
    const applied = await applyCanvasVideoWorkflow(node, promptRaw, rawRefs, stageWorkflowsFromSources(sourceData.sources));
    const prompt = applied.prompt;
    const refs = applied.refs || rawRefs;
    const media = {
...
async function uploadCanvasUrlToComfy(url){
    ...
    const filename = String(url || '').startsWith('data:') ? `stage-layout-${Date.now()}.png` : ((url || '').split('/').pop()?.split('?')[0] || `canvas_${Date.now()}.png`);
    const form = new FormData();
...
async function ltxDirectorBuildTimelinePayload(node, globalPromptFallback=''){
    ...
    for(const seg of segments){
        if(seg.type === 'image' && !seg.imageFile){
            ...
            if(url){
                const fullUrl = url.startsWith('http') || url.startsWith('data:') ? url : (location.origin + (url.startsWith('/') ? url : '/' + url));
                seg.imageFile = await uploadCanvasUrlToComfy(fullUrl);
            }
        }
        if(seg.imgObj) delete seg.imgObj;
    }
    if(node.ltxStage?.layoutUrl && !segments.some(seg => seg._stageLayout)){
        try {
            const file = await uploadCanvasUrlToComfy(node.ltxStage.layoutUrl);
            if(file) segments.push({id:uid('ltxlayout'), type:'image', start:0, length:Math.max(6, Number(node.durationFrames) || 24), prompt:'stage layout', imageFile:file, _stageLayout:true});
        } catch(e) {}
    }
...
function renderLTXDirectorBody(node){
    ...
    wrap.innerHTML = `
        ...
        <div class="ltx-stage-host" data-ltx-stage-host></div>
    ...
    const stageHost = wrap.querySelector('[data-ltx-stage-host]');
    if(stageHost && window.VideoWorkflowPanel){
        node.ltxStage = window.VideoWorkflowSchema
            ? window.VideoWorkflowSchema.normalize({stage: node.ltxStage}).stage
            : (node.ltxStage || {actors:[], camera:{x:0.5,y:0.82,facing:0}, layoutUrl:''});
        window.VideoWorkflowPanel.mountStage(stageHost, () => node.ltxStage, stage => {
            node.ltxStage = stage;
            scheduleSave();
        }, {
            ownerNodeId: node.id,
            ...
            openPageUrl: canvas?.id
                ? '/static/director-desk.html?id=' + encodeURIComponent(canvas.id) + '&node=' + encodeURIComponent(node.id)
                : '',
            deskOpen: Boolean(node._ltxStageDeskOpen),
            onDeskChange: on => { node._ltxStageDeskOpen = Boolean(on); },
            onExport: async (dataUrl, stage) => {
                node.ltxStage = stage;
                node.ltxStage.layoutUrl = dataUrl;
                scheduleSave();
                setStatus(tr('videoWf.layoutReady'));
            }
        });
...
async function runLTXDirectorNode(nodeId, opts={}){
    ...
    const pendingId = uid('p');
    const refs = sources.flatMap(s => s.refs || []);
    if(node.ltxStage?.layoutUrl) refs.push({url:node.ltxStage.layoutUrl, name:'片场布局', kind:'image', purpose:'layout'});
...
function runCascadeNodeByType(node, opts={}){
    if(!node || node.stageHost) return Promise.resolve();
...
function canvasWorkflowEdges(){
    ...
    connections.forEach(c => {
        const from = nodes.find(n => n.id === c.from);
        const to = nodes.find(n => n.id === c.to);
        if(!from || !to || !runTypes.includes(from.type) || from.stageHost) return;
        if(runTypes.includes(to.type) && !to.stageHost){
        ...
        if(to.type === 'output'){
            connections.filter(cc => cc.from === to.id).forEach(cc => {
                const next = nodes.find(n => n.id === cc.to);
                if(next && runTypes.includes(next.type) && !next.stageHost) direct.push([from.id, next.id]);
            });
        }
    });
    return direct;
...
function computeConnectedWorkflowOrder(anchorId){
    ...
    const runTypes = canvasRunTypes();
    if(!anchor || !runTypes.includes(anchor.type) || anchor.stageHost) return [];
    ...
    nodes.filter(n => connected.has(n.id) && runTypes.includes(n.type) && !n.stageHost).forEach(n => visit(n.id));
    return order;
}
...
function computeCascadeOrder(targetId){
    ...
    function dfs(id){
        ...
        connections.filter(c => c.to === id).forEach(c => {
            const from = nodes.find(n => n.id === c.from);
            if(!from) return;
            if(GEN_TYPES.includes(from.type) && !from.stageHost){
                dfs(from.id);
            } else if(from.type === 'output'){
                // output 节点的上游是生成器
                connections.filter(cc => cc.to === from.id).forEach(cc => {
                    const ff = nodes.find(n => n.id === cc.from);
                    if(ff && GEN_TYPES.includes(ff.type) && !ff.stageHost) dfs(ff.id);
                });
            }
        });
        if(GEN_TYPES.includes(node.type) && !node.stageHost) order.push(id);
    }
    dfs(targetId);
    return order;
}
...
canvasAssetAddCategoryBtn?.addEventListener('click', async () => {
    try {
        ...
        if(!String(name || '').trim()) return;
        const response = await fetch('/api/asset-library/categories', {
            method:'POST',
            headers:{'Content-Type':'application/json'},
            body:JSON.stringify({library_id:activeCanvasAssetLibraryId, name:String(name).trim(), type:'image'})
        });
        const data = await response.json().catch(() => ({}));
        if(!response.ok) throw new Error(data.detail || '新建分组失败');
        canvasAssetLibrary = data.library || canvasAssetLibrary;
        activeCanvasAssetCategoryId = data.category?.id || activeCanvasAssetCategoryId;
        renderCanvasAssetLibrary();
    } catch(err) {
        console.error('canvas asset category create failed', err);
        setStatus(err?.message || '新建分组失败');
    }
});
...
assetManagerModal?.addEventListener('click', async event => {
    // This delegated handler owns every CRUD action in the manager.  Keep a
    // single rejection boundary around the dispatcher so a transient API or
    // browser prompt failure never escapes as an unhandled promise.
    try {
    ...
    if(event.target.closest?.('[data-manager-asset-delete]')){
        ...
        const ids = [...managerSelectedAssetIds];
        if(!window.confirm(`确认删除所选 ${ids.length} 个素材？仍被画布或历史记录引用的文件会自动保留。`)) return;
        const response = await fetch('/api/asset-library/items/delete', {method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({library_id:activeCanvasAssetLibraryId, ids})});
        const data = await response.json().catch(() => ({}));
        if(!response.ok){ setStatus(data.detail || '批量删除素材失败'); return; }
        canvasAssetLibrary = data.library || canvasAssetLibrary;
        applyCanvasManagerBatchDeleteResult(data, managerSelectedAssetIds, '素材');
        renderAssetManager(); renderCanvasAssetLibrary(); return;
    }
    ...
    if(event.target.closest?.('[data-manager-workflow-delete]')){
        ...
        const ids = [...managerSelectedWorkflowIds];
        if(!window.confirm(`确认删除所选 ${ids.length} 个工作流？`)) return;
        const response = await fetch('/api/asset-library/items/delete', {method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({library_id:activeCanvasAssetLibraryId, ids})});
        const data = await response.json().catch(() => ({}));
        if(!response.ok){ setStatus(data.detail || '批量删除工作流失败'); return; }
        canvasAssetLibrary = data.library || canvasAssetLibrary;
        applyCanvasManagerBatchDeleteResult(data, managerSelectedWorkflowIds, '工作流');
        renderAssetManager(); renderCanvasAssetLibrary(); return;
    }
    ...
    if(event.target.closest?.('[data-manager-prompt-delete]')){
        ...
        const ids = [...managerSelectedPromptIds];
        if(!window.confirm(`确认删除所选 ${ids.length} 个提示词？`)) return;
        const response = await fetch('/api/prompt-libraries/items/delete', {method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({library_id:activePromptLibraryId, ids})});
        const data = await response.json().catch(() => ({}));
        if(!response.ok){ setStatus(data.detail || '批量删除提示词失败'); return; }
        canvasPromptLibraries = data.library?.libraries || canvasPromptLibraries;
        applyCanvasManagerBatchDeleteResult(data, managerSelectedPromptIds, '提示词');
        refreshCanvasPromptTemplatesFromLibraries();
        renderAssetManager(); return;
    }
    } catch(err) {
        console.error('canvas asset manager action failed', err);
        try {
            setStatus(err?.message || '资产管理操作失败');
        } catch (_) {}
    }
}, true);
...
function resetCanvasHistory(){
    // History snapshots are document-local.  Never carry an undo entry from a
    // previous canvas into the next one (or into a newly created document).
    undoStack.length = 0;
    redoStack.length = 0;
}
...
function pushUndo(){
    ...
    // A new edit after undo starts a new branch; the old redo branch is no
    // longer reachable and must not be replayed into the changed document.
    redoStack.length = 0;
}
function restoreCanvasHistorySnapshot(state){
...
function performUndo(){
    ...
    // Save the visible state before restoring the previous snapshot so redo
    // can faithfully return to it.
    redoStack.push(canvasHistorySnapshot());
    if(redoStack.length > UNDO_MAX) redoStack.shift();
    restoreCanvasHistorySnapshot(undoStack.pop());
}
function performRedo(){
    if(!canvas || !redoStack.length) return;
    // Symmetric with undo: a redo can itself be undone.
    undoStack.push(canvasHistorySnapshot());
    if(undoStack.length > UNDO_MAX) undoStack.shift();
    restoreCanvasHistorySnapshot(redoStack.pop());
}
...
function startNodeResize(e, node){
    ...
    resizeNode = {
        ...
        sx:e.clientX,
        sy:e.clientY,
        sw:(rect?.width ? rect.width / viewport.scale : node.w || defaultNodeSize(node.type, node).w),
        sh:(rect?.height ? rect.height / viewport.scale : node.h || defaultNodeSize(node.type, node).h || 160)
    };
...
function onNodeResize(e){
    ...
    const min = defaultNodeSize(resizeNode.node.type, resizeNode.node);
    // The embedded desk has a deliberate desktop floor so its four-column
    // studio remains usable.  Match the CSS floor here; otherwise dragging
    // below it would persist a smaller logical width/height while the browser
    // still paints a larger min-sized card on the next frame.
    const minW = resizeNode.node?.stageHost
        ? CANVAS_DIRECTOR_STAGE_MIN_SIZE.w
        : Math.min(min.w, 220);
    const minH = resizeNode.node?.stageHost
        ? CANVAS_DIRECTOR_STAGE_MIN_SIZE.h
        : 96;
    const nextW = Math.max(minW, resizeNode.sw + (e.clientX - resizeNode.sx) / viewport.scale);
    const nextH = Math.max(minH, resizeNode.sh + (e.clientY - resizeNode.sy) / viewport.scale);
...
function canConnect(fromId, toId){
    ...
    return CANVAS_GENERATOR_TYPES.includes(to.type) && ['image','prompt','assetCard','loop','group','promptGroup','output','llm','audio'].includes(from.type);
}
...
function canvasLinkCoord(value){
    const n = Number(value);
    return Number.isFinite(n) ? n : 0;
}
function canvasLinkPathD(x1, y1, x2, y2){
    const sx = canvasLinkCoord(x1), sy = canvasLinkCoord(y1);
    const ex = canvasLinkCoord(x2), ey = canvasLinkCoord(y2);
    const dx = Math.max(80, Math.abs(ex - sx) * .45);
    return `M ${sx} ${sy} C ${sx + dx} ${sy}, ${ex - dx} ${ey}, ${ex} ${ey}`;
}
function canvasLinkHash(value){
    // FNV-1a gives stable per-connection variation without Math.random(), so
    // a link keeps its phase when renderLinks is called during a drag.
    let hash = 2166136261;
...
function canvasLinkMotionTiming(connection, index=0, kind='flow'){
    // Prefer the connection identity over its array position so deleting or
    // reordering a neighboring edge does not make every remaining edge jump
    // to a new phase.  The index is only a fallback for transient paths.
    const identity = connection?.id != null ? String(connection.id) : `index:${index}`;
    const seed = canvasLinkHash(`${identity}:${kind}`);
    const duration = kind === 'temp'
        ? .92
        : kind === 'knife'
            ? .72
            // Keep the normal tube calm and legible instead of making each
            // edge look like a fast progress indicator.
            : 2.25 + ((seed % 1000) / 1000) * .95;
    const perfNow = typeof performance !== 'undefined' && performance && typeof performance.now === 'function'
        ? performance.now()
        : Date.now();
    const now = Number.isFinite(perfNow) ? perfNow / 1000 : Date.now() / 1000;
    const offset = ((seed >>> 8) % 1000) / 1000 * duration;
    const phase = (now + offset) % duration;
    const breatheDuration = kind === 'flow' ? duration * 1.42 : duration;
    const breathePhase = (now + offset * .73) % breatheDuration;
    const sparkDuration = kind === 'flow'
        ? Math.max(1.85, duration * (.76 + ((seed >>> 24) % 22) / 100))
        : duration;
    const sparkOffset = ((seed >>> 4) % 1000) / 1000 * sparkDuration;
    const sparkPhase = (now + sparkOffset) % sparkDuration;
    return {
        duration,
        delay: -phase,
        breatheDuration,
        breatheDelay: -breathePhase,
        sparkDuration,
        sparkDelay: -sparkPhase
    };
}
function applyCanvasLinkMotionTiming(path, timing){
    if(!path || !timing) return;
    path.style.setProperty('--canvas-link-duration', `${timing.duration.toFixed(3)}s`);
    path.style.setProperty('--canvas-link-delay', `${timing.delay.toFixed(3)}s`);
    path.style.setProperty('--canvas-link-breathe-duration', `${timing.breatheDuration.toFixed(3)}s`);
    path.style.setProperty('--canvas-link-breathe-delay', `${timing.breatheDelay.toFixed(3)}s`);
    path.setAttribute('pointer-events', 'none');
    path.setAttribute('aria-hidden', 'true');
}
function canvasLinkMotionPath(x1, y1, x2, y2, cls, timing, id){
    const path = pathEl(x1, y1, x2, y2, cls);
    if(id != null) path.dataset.connectionId = String(id);
    applyCanvasLinkMotionTiming(path, timing);
    return path;
}
function canvasLinkMotionSpark(x1, y1, x2, y2, timing, connection, extraClass=''){
    if(!timing) return null;
    const ns = 'http://www.w3.org/2000/svg';
    const group = document.createElementNS(ns, 'g');
    const state = canvasLinkMotionStateClasses(connection);
    group.setAttribute('class', `link-motion-spark${state ? ` ${state}` : ''}${extraClass ? ` ${extraClass}` : ''}`);
    if(connection?.id != null) group.dataset.connectionId = String(connection.id);
    group.setAttribute('pointer-events', 'none');
    group.setAttribute('aria-hidden', 'true');
    group.style.setProperty('--canvas-link-spark-color', state.includes('link-active') ? '#ecfeff' : '#d9fffb');
    group.style.setProperty('--canvas-link-spark-shadow', state.includes('link-active') ? 'rgba(45,212,191,.96)' : 'rgba(20,184,166,.84)');
    const motion = document.createElementNS(ns, 'animateMotion');
    motion.setAttribute('dur', `${Number(timing.sparkDuration || timing.duration || 1).toFixed(2)}s`);
    motion.setAttribute('begin', `${Number(timing.sparkDelay || 0).toFixed(2)}s`);
    motion.setAttribute('repeatCount', 'indefinite');
    motion.setAttribute('rotate', 'auto');
    motion.setAttribute('path', canvasLinkPathD(x1, y1, x2, y2));
    group.appendChild(motion);
    const streak = document.createElementNS(ns, 'ellipse');
    streak.setAttribute('class', 'link-motion-spark-streak');
    streak.setAttribute('cx', '0'); streak.setAttribute('cy', '0');
    streak.setAttribute('rx', '8'); streak.setAttribute('ry', '1.35');
    streak.setAttribute('fill', 'var(--canvas-link-spark-color)');
    streak.setAttribute('opacity', '.6');
    // Use a soft elongated sheen instead of a round particle.  The moving
    // highlight still communicates direction, but does not leave a bright
    // dot on every cable.
    const sheen = document.createElementNS(ns, 'ellipse');
    sheen.setAttribute('class', 'link-motion-spark-sheen');
    sheen.setAttribute('cx', '0'); sheen.setAttribute('cy', '0');
    sheen.setAttribute('rx', '18'); sheen.setAttribute('ry', '1.2');
    sheen.setAttribute('fill', 'var(--canvas-link-spark-color)');
    sheen.setAttribute('opacity', '.46');
    streak.setAttribute('rx', '10');
    streak.setAttribute('ry', '.72');
    streak.setAttribute('opacity', '.7');
    group.append(sheen, streak);
    return group;
}
function canvasLinkMotionStateClasses(connection){
    const classes = [];
    if(isConnectionSelected(connection)) classes.push('link-active');
    if(String(hoveredConnectionId || '') === String(connection?.id ?? '')) classes.push('link-hovered');
    return classes.join(' ');
}
function renderLinks(){
    ...
    linksEl.innerHTML = '';
    linkControlsEl.innerHTML = '';
    // A single shared gradient gives the classic board the same liquid-glass
    // material as smart canvas without allocating one paint definition per
    // edge.  It is attached after clearing the SVG so hot renders never leave
    // stale IDs behind.
    linksEl.insertAdjacentHTML('afterbegin', '<defs aria-hidden="true"><linearGradient id="canvas-link-gradient" x1="-0.5" y1="0" x2="1.5" y2="0" gradientUnits="objectBoundingBox"><stop offset="0" stop-color="#0f766e" stop-opacity=".62"></stop><stop offset=".28" stop-color="#2dd4bf" stop-opacity=".9"></stop><stop offset=".45" stop-color="#99f6e4" stop-opacity=".95"></stop><stop offset=".5" stop-color="#f0fffc" stop-opacity="1"></stop><stop offset=".56" stop-color="#2dd4bf" stop-opacity=".94"></stop><stop offset="1" stop-color="#0ea5e9" stop-opacity=".62"></stop><animateTransform attributeName="gradientTransform" type="translate" values="-.62 0;.62 0;-.62 0" dur="3.8s" repeatCount="indefinite"></animateTransform></linearGradient></defs>');
    ...
    segments.forEach(({c, a, b}, index) => {
        const stateClasses = canvasLinkMotionStateClasses(c);
        const relClass = stateClasses ? ` ${stateClasses}` : '';
        const timing = canvasLinkMotionTiming(c, index, 'flow');
        // Keep the original path as the base layer.  The additional
        // pointer-transparent layers form a continuous liquid tube; a single
        // animateMotion sheen adds direction and a flash without turning
        // the pipe into a row of synthetic dashes.
        const base = pathEl(a.x, a.y, b.x, b.y, `link link-base${relClass}`);
        base.dataset.connectionId = String(c.id);
        base.dataset.linkRole = 'base';
        base.setAttribute('pointer-events', 'none');
        base.setAttribute('aria-hidden', 'true');
        linksEl.appendChild(base);
        linksEl.appendChild(canvasLinkMotionPath(a.x, a.y, b.x, b.y, `link-motion-glow${relClass}`, timing, c.id));
        linksEl.appendChild(canvasLinkMotionPath(a.x, a.y, b.x, b.y, `link-motion-flow${relClass}`, timing, c.id));
        linksEl.appendChild(canvasLinkMotionPath(a.x, a.y, b.x, b.y, `link-motion-core${relClass}`, timing, c.id));
        const spark = canvasLinkMotionSpark(a.x, a.y, b.x, b.y, timing, c);
        if(spark) linksEl.appendChild(spark);
        linkControlsEl.appendChild(linkDeleteButton(c, a, b));
        // The hit path is deliberately appended last and remains untouched:
        // it is the only SVG layer that receives pointer events.
        linksEl.appendChild(linkHitEl(a.x, a.y, b.x, b.y, c.id));
    });
    if(tempLink){
        const tempTiming = canvasLinkMotionTiming({id:'temporary'}, 0, 'temp');
        linksEl.appendChild(pathEl(tempLink.x1, tempLink.y1, tempLink.x2, tempLink.y2, 'link temp'));
        linksEl.appendChild(canvasLinkMotionPath(tempLink.x1, tempLink.y1, tempLink.x2, tempLink.y2, 'link-motion-glow link-motion-temp', tempTiming));
        linksEl.appendChild(canvasLinkMotionPath(tempLink.x1, tempLink.y1, tempLink.x2, tempLink.y2, 'link-motion-flow link-motion-temp', tempTiming));
        linksEl.appendChild(canvasLinkMotionPath(tempLink.x1, tempLink.y1, tempLink.x2, tempLink.y2, 'link-motion-core link-motion-temp', tempTiming));
        const tempSpark = canvasLinkMotionSpark(tempLink.x1, tempLink.y1, tempLink.x2, tempLink.y2, tempTiming, {id:'temporary'}, 'link-motion-temp');
        if(tempSpark) linksEl.appendChild(tempSpark);
    }
    renderKnifeTrail();
}
function renderKnifeTrail(){
    ...
    poly.setAttribute('class', 'link knife-trail link-motion-knife');
    applyCanvasLinkMotionTiming(poly, canvasLinkMotionTiming({id:'knife'}, 0, 'knife'));
...
function setHoveredConnection(id){
    ...
    // Hover is intentionally updated in place instead of rebuilding the SVG;
    // moving the pointer over a line must not reset its animation or interrupt
    // a node drag.
    linksEl?.querySelectorAll?.('.link-base[data-connection-id], .link-motion-glow[data-connection-id], .link-motion-flow[data-connection-id], .link-motion-core[data-connection-id], .link-motion-spark[data-connection-id]')?.forEach(path => {
        path.classList.toggle('link-hovered', String(path.dataset.connectionId || '') === String(hoveredConnectionId || ''));
    });
}
...
function pathEl(x1,y1,x2,y2,cls){
    ...
    p.setAttribute('d', canvasLinkPathD(x1, y1, x2, y2));
...
function canvasHistoryShortcutBlocked(target){
    if(isEditableTarget(target) || isEditableTarget(document.activeElement)) return true;
    if(target?.closest?.('#imageEditModal, #createMenu, #linkCreateMenu, #nodeInputMenu, #nodeOutputMenu, #canvasAssetPanel, #assetManagerModal, #workflowTransferModal, #logModal, #promptTemplateModal, #outputLightbox')) return true;
    // Do not mutate the underlying graph while an editor/utility sheet owns
    // the keyboard focus.  This mirrors the guards used by the other canvas
    // shortcuts and keeps Ctrl+Y/Ctrl+Shift+Z native inside dialogs.
    return [
        document.getElementById('imageEditModal'),
        document.getElementById('createMenu'),
        document.getElementById('linkCreateMenu'),
        document.getElementById('nodeInputMenu'),
        document.getElementById('nodeOutputMenu'),
        document.getElementById('canvasAssetPanel'),
        promptTemplateModal,
        outputLightbox,
        assetManagerModal,
...
minimap?.addEventListener('mousedown', e => {
    ...
    if(e.target.closest?.('#canvasArrangeBtn, #canvasDeleteSelectedBtn')) return;
    e.preventDefault();
    e.stopPropagation();
    minimapDrag = true;
...
canvasDeleteSelectedBtn?.addEventListener('mousedown', e => e.stopPropagation());
canvasDeleteSelectedBtn?.addEventListener('click', e => {
    e.preventDefault();
    e.stopPropagation();
    if(!selected.size) return;
    const count = selected.size;
    if(!window.confirm(`确认删除选中的 ${count} 个节点？此操作可通过撤销恢复。`)) return;
    deleteSelectedNodes();
});
function isZoomPreviewIgnoredTarget(target){
    return !!target?.closest?.('#createMenu, #linkCreateMenu, #nodeInputMenu, #nodeOutputMenu, #imageNodeMenu, .minimap, #canvasAssetPanel, #assetManagerModal, #workflowTransferModal, #logModal, #promptTemplateModal, #imageEditModal, #outputLightbox, #canvasDeleteSelectedBtn');
}
...
window.addEventListener('keydown', e => {
    ...
    if((e.ctrlKey || e.metaKey) && (key === 'z' || key === 'y') && !canvasHistoryShortcutBlocked(e.target)) {
        if(e.repeat) return;
        e.preventDefault();
        // Ctrl/Cmd+Shift+Z and Ctrl/Cmd+Y are both conventional redo aliases.
        if(key === 'y' || e.shiftKey) performRedo();
        else performUndo();
        return;
    }
...
/*
 * Public presentation bridges.  The shared LibTV rail is loaded after this
 * editor and must be able to invoke the existing commands even when the page
 * is hosted in an embedded WebView where top-level lexical declarations are
 * not reflected on `window`.  These are command aliases only; canvas state,
 * persistence, and generation remain owned by this script.
 */
window.openCreateMenu = openCreateMenu;
window.fitAllNodesViewport = fitAllNodesViewport;
window.menuAdd = menuAdd;
window.toggleQuickToolbar = toggleQuickToolbar;
window.openAssetManager = openAssetManager;
window.onload = async () => {
    try {
        ...
        setStatus(langIsEn() ? 'Canvas startup failed' : '画布启动失败');
    }
};
