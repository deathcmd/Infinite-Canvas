...
const workflowBulkManageBtn = document.getElementById('workflowBulkManage');
const workflowBulkCountEl = document.getElementById('workflowBulkCount');
const workflowBulkSelectAllBtn = document.getElementById('workflowBulkSelectAll');
const workflowBulkClearBtn = document.getElementById('workflowBulkClear');
const workflowBulkDeleteBtn = document.getElementById('workflowBulkDelete');
const workflowBulkExitBtn = document.getElementById('workflowBulkExit');
...
function mediaPreviewHtml(kind, url, name='', compact=false){
    ...
    if(kind === 'video') return `<video src="${safeUrl}" muted preload="metadata" playsinline controls aria-label="${safeName}"></video>`;
    if(kind === 'audio') return `<div class="media-file-chip"><i data-lucide="file-audio" class="${compact ? 'w-5 h-5' : 'w-6 h-6'}"></i><span class="media-file-name">${safeName}</span><audio src="${safeUrl}" controls preload="metadata" aria-label="${safeName}"></audio></div>`;
...
function renderComfyInstances(){
    ...
    el.innerHTML = comfyInstances.map((addr, i) => `
        <div style="display:flex;align-items:center;gap:6px;padding:4px;border:1px solid var(--line);border-radius:9px;background:var(--soft)">
            ...
            <input class="small-input" type="text" value="${escapeAttr(addr)}" placeholder="host:port" aria-label="ComfyUI 实例地址 ${i + 1}" oninput="updateComfyInstance(${i}, this.value)" style="flex:1;height:28px;padding:0 8px;border:1px solid var(--line);border-radius:6px;background:var(--panel);color:var(--text);font-size:12px;font-family:ui-monospace,Menlo,monospace">
...
async function loadList(){
    try {
        ...
        const names = new Set(workflows.filter(w => !w.builtin).map(w => w.name));
        [...selectedWorkflowNames].forEach(name => { if(!names.has(name)) selectedWorkflowNames.delete(name); });
        renderList();
...
function renderList(){
    document.body.classList.toggle('workflow-bulk-mode', workflowBulkMode);
    listEl.innerHTML = workflows.map(w => {
        const selected = selectedWorkflowNames.has(w.name);
        return `
            <div class="workflow-card-row ${selected ? 'bulk-selected' : ''}" data-workflow-row="${escapeAttr(w.name)}">
                ${w.builtin ? '' : `<input class="workflow-bulk-check" type="checkbox" data-workflow-check="${escapeAttr(w.name)}" ${selected ? 'checked' : ''} aria-label="选择工作流 ${escapeAttr(w.title || w.name)}" ${workflowBulkMode ? '' : 'hidden'}>`}
                <button class="workflow-card ${w.name===selectedName?'active':''}" type="button" data-workflow-open="${escapeAttr(w.name)}">
    ...
    listEl.querySelectorAll('[data-workflow-open]').forEach(btn => {
        btn.addEventListener('click', event => {
            const name = btn.dataset.workflowOpen || '';
            if(workflowBulkMode){
                event.preventDefault();
                const item = workflows.find(w => w.name === name);
                if(item && !item.builtin) toggleWorkflowSelection(name);
                return;
            }
            selectWorkflow(name);
        });
    });
    listEl.querySelectorAll('[data-workflow-check]').forEach(input => {
        input.addEventListener('click', event => event.stopPropagation());
        input.addEventListener('change', () => toggleWorkflowSelection(input.dataset.workflowCheck || '', input.checked));
    });
    refreshWorkflowBulkToolbar();
    refreshIcons();
}

function refreshWorkflowBulkToolbar(){
    if(!workflowBulkManageBtn) return;
    const editable = workflows.filter(item => !item.builtin);
    const selectedCount = selectedWorkflowNames.size;
    const allSelected = editable.length > 0 && selectedCount === editable.length;
    workflowBulkManageBtn.hidden = workflowBulkMode;
    workflowBulkManageBtn.setAttribute('aria-pressed', workflowBulkMode ? 'true' : 'false');
    workflowBulkCountEl.hidden = !workflowBulkMode;
    workflowBulkCountEl.textContent = currentLang() === 'en' ? `${selectedCount} selected` : `已选 ${selectedCount} 个`;
    workflowBulkSelectAllBtn.hidden = !workflowBulkMode;
    workflowBulkSelectAllBtn.disabled = editable.length === 0;
    workflowBulkSelectAllBtn.querySelector('span').textContent = allSelected ? (currentLang() === 'en' ? 'Deselect all' : '取消全选') : (currentLang() === 'en' ? 'Select all' : '全选');
    workflowBulkClearBtn.hidden = !workflowBulkMode;
    workflowBulkClearBtn.disabled = selectedCount === 0;
    workflowBulkDeleteBtn.hidden = !workflowBulkMode;
    workflowBulkDeleteBtn.disabled = selectedCount === 0;
    workflowBulkExitBtn.hidden = !workflowBulkMode;
    refreshIcons();
}

function toggleWorkflowBulkMode(force){
    workflowBulkMode = typeof force === 'boolean' ? force : !workflowBulkMode;
    if(!workflowBulkMode) selectedWorkflowNames.clear();
    renderList();
}

function toggleWorkflowSelection(name, value){
    if(!name || workflows.find(item => item.name === name)?.builtin) return;
    const shouldSelect = typeof value === 'boolean' ? value : !selectedWorkflowNames.has(name);
    if(shouldSelect) selectedWorkflowNames.add(name);
    else selectedWorkflowNames.delete(name);
    renderList();
}

async function deleteSelectedWorkflows(){
    const names = [...selectedWorkflowNames].filter(name => workflows.some(item => item.name === name && !item.builtin));
    if(!names.length) return;
    if(!confirm(currentLang() === 'en' ? `Delete ${names.length} selected workflows? This cannot be undone.` : `确认删除选中的 ${names.length} 个工作流？此操作不可恢复。`)) return;
    workflowBulkDeleteBtn.disabled = true;
    try {
        ...
        if(response.status === 404 || response.status === 405){
            const results = await Promise.allSettled(names.map(name => fetch(`/api/workflows/${encodeURIComponent(name)}`, {method:'DELETE'}).then(r => { if(!r.ok) throw new Error('delete failed'); return true; })));
            results.forEach((item, index) => {
                if(item.status === 'fulfilled') removedNames.add(String(names[index]));
                else skippedNames.add(String(names[index]));
            });
            removed = removedNames.size;
        } else {
            const data = await response.json().catch(() => ({}));
            if(!response.ok) throw new Error(data.detail || tr('comfy.deleteFailed'));
            const readNames = value => new Set(Array.isArray(value)
                ? value.map(item => String(item?.name ?? item?.id ?? item ?? '')).filter(Boolean)
                : []);
            removedNames = readNames(data.removed ?? data.deleted);
            skippedNames = new Set((Array.isArray(data.skipped) ? data.skipped : [])
                .map(item => String(item?.name ?? item?.id ?? item ?? '')).filter(Boolean));
            // Older compatible servers may return only a counter.  Infer the
            // first N non-skipped requested names without treating an array
            // as a number (Number([...]) would yield NaN for this API).
            if(!removedNames.size){
                let rawCount = data.removed_count ?? data.deleted_count;
                if(rawCount === undefined && !Array.isArray(data.removed) && !Array.isArray(data.deleted)) {
                    rawCount = data.removed ?? data.deleted;
                }
                const count = Number(rawCount);
                if(Number.isFinite(count) && count > 0){
                    names.filter(name => !skippedNames.has(String(name))).slice(0, count)
                        .forEach(name => removedNames.add(String(name)));
                } else if(!('removed' in data) && !('deleted' in data) && !('removed_count' in data) && !('deleted_count' in data)) {
                    names.filter(name => !skippedNames.has(String(name))).forEach(name => removedNames.add(String(name)));
                }
            }
            // Any requested name not explicitly removed is retained as a
            // skipped item so a partial result is visible and retryable.
            names.forEach(name => {
                const key = String(name);
                if(!removedNames.has(key)) skippedNames.add(key);
            });
            removed = removedNames.size;
        }
        const deletedSelected = selectedName && removedNames.has(String(selectedName));
        selectedWorkflowNames.clear();
        skippedNames.forEach(name => {
            if(names.includes(name) && workflows.some(item => item.name === name && !item.builtin)) selectedWorkflowNames.add(name);
        });
        workflowBulkMode = selectedWorkflowNames.size > 0;
        if(deletedSelected){ selectedName = ''; currentWorkflow = null; currentConfig = null; renderEditor(); renderPreview(); renderWorkspaceView(); }
        await loadList();
        const skippedCount = skippedNames.size;
        setStatus(currentLang() === 'en'
            ? `Deleted ${removed} workflows${skippedCount ? `; ${skippedCount} skipped` : ''}`
            : `已删除 ${removed} 个工作流${skippedCount ? `，${skippedCount} 个未删除` : ''}`);
        try { new BroadcastChannel('studio-api').postMessage({ type: 'workflows-changed' }); } catch(e) {}
    } catch(err){
        workflowBulkDeleteBtn.disabled = false;
        alert(err.message || tr('comfy.deleteFailed'));
    }
}
...
function renderInputRow(nodeId, inputKey, rawValue){
    ...
    return `
        <div class="input-row ${active?'is-active':''} ${showExtras?'has-extras':''}">
            ...
            <input class="small-input" type="text" placeholder="${tr('comfy.displayName')}" aria-label="${escapeAttr(friendlyName)} 显示名称" value="${active?escapeAttr(f.name):escapeAttr(friendlyName)}" ${active?'':'disabled'} oninput="updateField('${active?f.id:''}','name',this.value)">
            <select class="small-select" aria-label="${escapeAttr(friendlyName)} 字段类型" ${active?'':'disabled'} onchange="updateField('${active?f.id:''}','type',this.value)">
...
function renderExtras(f){
    if(f.type === 'slider' || f.type === 'number'){
        const randomToggle = f.type === 'number'
            ? `<label class="random-toggle" onclick="event.stopPropagation()"><input type="checkbox" aria-label="${escapeAttr(f.name || f.input || '数字字段')} 启用随机数" ${f.random_enabled === true ? 'checked' : ''} onchange="updateField('${f.id}','random_enabled',this.checked)">随机数</label>`
            : '';
        return `<div class="extras-row">
            <div class="extra-pair">min<input class="small-input" type="number" aria-label="${escapeAttr(f.name || f.input || '字段')} 最小值" value="${f.min ?? ''}" oninput="updateField('${f.id}','min',this.value===''?null:parseFloat(this.value))"></div>
            <div class="extra-pair">max<input class="small-input" type="number" aria-label="${escapeAttr(f.name || f.input || '字段')} 最大值" value="${f.max ?? ''}" oninput="updateField('${f.id}','max',this.value===''?null:parseFloat(this.value))"></div>
            <div class="extra-pair">step<input class="small-input" type="number" aria-label="${escapeAttr(f.name || f.input || '字段')} 步长" value="${f.step ?? ''}" oninput="updateField('${f.id}','step',this.value===''?null:parseFloat(this.value))"></div>
            <div class="extra-pair">${tr('comfy.defaultValue')}<input class="small-input" type="number" aria-label="${escapeAttr(f.name || f.input || '字段')} 默认值" value="${f.default ?? ''}" oninput="updateField('${f.id}','default',this.value===''?null:parseFloat(this.value))"></div>
            ${randomToggle}
        </div>`;
    }
    if(f.type === 'dropdown'){
        ...
        const rows = opts.map((o, i) => {
            ...
            return `
                <div class="dropdown-opt-row">
                    ...
                    <input class="small-input" type="text" aria-label="${escapeAttr(f.name || f.input || '下拉字段')} 选项 ${i + 1}" placeholder="选项 ${i + 1}" value="${escapeAttr(o)}"
...
function randomButtonHtml(f){
    ...
    return `<button class="random-btn ${active ? 'active' : ''}" type="button" onclick="togglePreviewRandom('${f.id}')" title="${title}" aria-label="${escapeAttr(title)}"><i data-lucide="dice-5" class="w-4 h-4"></i></button>`;
}
...
function renderPreview(){
    ...
    const resultHtml = runResult
        ? `<div class="run-result"><img src="${escapeAttr(runResult)}" alt="ComfyUI 运行结果" onclick="openImagePreview('${escapeAttr(runResult)}')"><div class="run-status">${tr('comfy.runSuccess')}</div></div>`
        : '';
...
function renderPreviewField(f){
    ...
    if(f.type === 'textarea'){
        return `<div class="pfield">${label}<textarea class="pfield-textarea" aria-label="${escapeAttr(f.name || f.input || '多行文本')}" oninput="setPreviewValue('${f.id}',this.value)">${escapeHtml(v)}</textarea></div>`;
    }
    if(f.type === 'number'){
        const randomBtn = randomButtonHtml(f);
        return `<div class="pfield">${label}<div class="pfield-random-row" style="${randomBtn ? '' : 'grid-template-columns:1fr'}"><input class="pfield-input" type="number" aria-label="${escapeAttr(f.name || f.input || '数字')}" value="${escapeAttr(v)}" oninput="setPreviewValue('${f.id}',parseFloat(this.value)||0)">${randomBtn}</div></div>`;
    }
    if(f.type === 'slider'){
        ...
        return `<div class="pfield">${label}<div class="pfield-random-row" style="grid-template-columns:1fr"><div class="pfield-slider">
            <input type="range" aria-label="${escapeAttr(f.name || f.input || '滑块')}" min="${min}" max="${max}" step="${step}" value="${escapeAttr(v)}" oninput="setPreviewValue('${f.id}',parseFloat(this.value))">
    ...
    if(f.type === 'dropdown'){
        ...
        return `<div class="pfield">${label}<select class="pfield-select" aria-label="${escapeAttr(f.name || f.input || '下拉选项')}" onchange="setPreviewValue('${f.id}',this.value)">${opts || `<option value="">${tr('comfy.noOptions')}</option>`}</select></div>`;
    }
    if(isMediaField(f)){
        ...
        return `<div class="pfield">${label}<div class="pfield-image-drop ${displayUrl?'has-image':''}" role="button" tabindex="0" aria-label="选择${escapeAttr(f.name || f.input || '媒体')}" onclick="pickImage('${f.id}')" onkeydown="if(event.key==='Enter'||event.key===' '){event.preventDefault();pickImage('${f.id}')}" >
    ...
    return `<div class="pfield">${label}<input class="pfield-input" type="text" aria-label="${escapeAttr(f.name || f.input || '文本')}" value="${escapeAttr(v)}" oninput="setPreviewValue('${f.id}',this.value)"></div>`;
}
...
function renderMiniCanvasPreview(target = previewCard, large = false){
    ...
    const resultHtml = runResult
        ? `<div class="mini-result"><img src="${escapeAttr(runResult)}" alt="ComfyUI 运行结果" onclick="openImagePreview('${escapeAttr(runResult)}')"><div class="run-status">${tr('comfy.runSuccess')}</div></div>`
            ...
            <div id="miniWorld" class="mini-world" style="transform:translate(${miniView.x}px,${miniView.y}px) scale(${miniView.k})">
                ...
                ${prompts.map((n,i) => `
                    <div class="mini-card" data-node="${n.id}" style="left:${n.x}px;top:${n.y}px">
                        ...
                        <div class="mini-card-body"><textarea class="mini-textarea" aria-label="${escapeAttr(tr('comfy.promptNode'))} ${i + 1}" oninput="updateMiniNode('${n.id}','text',this.value)" placeholder="${escapeAttr(tr('comfy.promptPlaceholder'))}">${escapeHtml(n.text || '')}</textarea></div>
                    </div>`).join('')}
                ${mediaNodes.map((n,i) => `
                    <div class="mini-card" data-node="${n.id}" style="left:${n.x}px;top:${n.y}px">
                        <span class="mini-port out"></span>
...
function renderMiniField(f){
    ...
    if(isMediaField(f)){
        const displayUrl = previewImageUrls[f.id] || (typeof v === 'string' && /^(\/|https?:|blob:|data:)/.test(v) ? v : '');
        return `<div class="pfield">${label}<div class="mini-image-drop" role="button" tabindex="0" aria-label="选择${escapeAttr(f.name || f.input || '媒体')}" onclick="pickImage('${f.id}')" onkeydown="if(event.key==='Enter'||event.key===' '){event.preventDefault();pickImage('${f.id}')}" >${mediaPreviewHtml(fieldKind(f), displayUrl, v, true)}</div></div>`;
    }
    if(f.type === 'textarea'){
        return `<div class="pfield">${label}<textarea class="mini-textarea" aria-label="${escapeAttr(f.name || f.input || '多行文本')}" oninput="setPreviewValue('${f.id}',this.value)">${escapeHtml(v)}</textarea></div>`;
    }
    if(f.type === 'number'){
        const randomBtn = randomButtonHtml(f);
        return `<div class="pfield">${label}<div class="pfield-random-row" style="${randomBtn ? '' : 'grid-template-columns:1fr'}"><input class="mini-input" type="number" aria-label="${escapeAttr(f.name || f.input || '数字')}" value="${escapeAttr(v)}" oninput="setPreviewValue('${f.id}',parseFloat(this.value)||0)">${randomBtn}</div></div>`;
    }
    if(f.type === 'slider'){
        const min = f.min ?? 0, max = f.max ?? 10, step = f.step ?? 1;
        return `<div class="pfield">${label}<div class="pfield-random-row" style="grid-template-columns:1fr"><div class="pfield-slider">
    ...
    return `<div class="pfield">${label}<input class="mini-input" type="text" aria-label="${escapeAttr(f.name || f.input || '文本')}" value="${escapeAttr(v)}" oninput="setPreviewValue('${f.id}',this.value)"></div>`;
}
...
async function pickMiniImage(nodeId){
    ...
    input.type = 'file';
    input.setAttribute('aria-label', '上传测试素材');
...
async function pickImage(fieldId){
    const input = document.createElement('input');
    input.type = 'file';
    input.setAttribute('aria-label', '上传工作流素材');
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    workflowBulkManageBtn?.addEventListener('click', () => toggleWorkflowBulkMode(true));
    workflowBulkSelectAllBtn?.addEventListener('click', () => {
        const editable = workflows.filter(item => !item.builtin);
        const allSelected = editable.length > 0 && selectedWorkflowNames.size === editable.length;
        if(allSelected) selectedWorkflowNames.clear();
        else editable.forEach(item => selectedWorkflowNames.add(item.name));
        renderList();
    });
    workflowBulkClearBtn?.addEventListener('click', () => { selectedWorkflowNames.clear(); renderList(); });
    workflowBulkDeleteBtn?.addEventListener('click', deleteSelectedWorkflows);
    workflowBulkExitBtn?.addEventListener('click', () => toggleWorkflowBulkMode(false));
    refreshWorkflowBulkToolbar();
    loadList();
    loadComfyInstances();
});
