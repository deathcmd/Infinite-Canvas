(function(){
    ...
    function attach(opts){
        ...
        async function doDelete(){
            ...
            const timestamps = sel.map(card => card.dataset.historyTs).filter(Boolean);
            let removed = 0;
            try {
                ...
                if(response.status === 404 || response.status === 405){
                    // Keep compatibility with an older local server while the
                    // new endpoint is rolled out.
                    const results = await Promise.allSettled(sel.map(card => fetch('/api/history/delete', {
                        method: 'POST',
                        headers: { 'Content-Type': 'application/json' },
                    ...
                    removed = results.length - failed;
                    if(failed > 0) alert(failed + ' / ' + sel.length + ' ✗');
                } else {
                    const payload = await response.json().catch(() => ({}));
                    if(!response.ok) throw new Error(payload.detail || 'delete failed');
                    const deleted = Array.isArray(payload.deleted_timestamps)
                        ? new Set(payload.deleted_timestamps.map(String))
                        : (Array.isArray(payload.deleted) ? new Set(payload.deleted.map(String)) : null);
                    sel.forEach(card => {
                        if(!deleted || deleted.has(String(card.dataset.historyTs))) card.remove();
                    });
                    removed = deleted ? sel.filter(card => !card.isConnected).length : Number(payload.removed ?? timestamps.length);
                }
            } catch(err){
