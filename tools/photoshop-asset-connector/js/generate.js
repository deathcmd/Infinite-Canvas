...
(function () {
  ...
  async function setImageField(fi, btn) {
    ...
    try {
      ...
      if (g.mode === 'rh') {
        // The open-source connector never selects wallet billing. Keep the
        // compatibility field explicit so older servers receive a safe value.
        const res = await net.apiSend('POST', '/api/runninghub/upload-asset', { url, useWallet: false });
  ...
  async function runRh() {
    ...
    // Wallet billing is intentionally unavailable in the open-source build.
    const res = await net.apiSend('POST', '/api/runninghub/workflow-submit', { workflowId: g.rhWorkflowId, nodeInfoList, useWallet: false });
